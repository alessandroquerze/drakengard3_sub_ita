`Il caro vecchio Octa può scaldarti{LF}le viscere fino al punto di ebollizione...{#Name="Octa"}`
Dear old Octa can heat your{LF}insides to the boiling point...{#Name="Octa"}

`Credo che sia il tuo cervello a bollire, vecchio.{#Name="Zero"}`
I think your brain is boiling, old man.{#Name="Zero"}

`C-Che freddo... N-Non riesco a parlare... D-Denti che battono...{#Name="Dito"}`
S-So cold... C-Can't talk... T-Teeth chattery...{#Name="Dito"}

`Certo che parli tanto per uno{LF}che dice di non riuscire a parlare. {#Name="Zero"}`
You sure talk a lot for a guy{LF}who says he can't talk. {#Name="Zero"}

`Infatti!{#Name="Octa"}`
Indeed!{#Name="Octa"}

`Sì, ti prego, sta’ zitto.{#Name="Decadus"}`
Yes, do please be quiet.{#Name="Decadus"}

`Come ti pare, brutto fi—{#Name="Dito"}`
Whatever, you fu—{#Name="Dito"}

`Sembri impassibile, Decadus. Non hai freddo?{#Name="Zero"}`
You seem unfazed, Decadus. Aren't you cold?{#Name="Zero"}

`Sì, mia signora. Ho freddo e sono fisicamente esausto.{#Name="Decadus"}`
I am, my lady. Cold and physically exhausted.{#Name="Decadus"}

`Allooora perché sembri così felice?{#Name="Zero"}`
Theeen why do you look so happy?{#Name="Zero"}

`Sai perché? Perché è un pervertito{LF}a cui piace farsi prendere il pacco—{#Name="Dito"}`
You know why? 'Cause he's a freak{LF}who likes having his junk—{#Name="Dito"}

`Scusa, non ho capito bene.{#Name="Zero"}`
Sorry, I didn't quite get that.{#Name="Zero"}

`Nascondere loro i miei veri desideri...{LF}Che gioco perverso!{#Name="Decadus"}`
Hiding my true desires from them...{LF}Such a devious game!{#Name="Decadus"}

`Sai, io in realtà non ho affatto freddo.{#Name="Cent"}`
You know, I'm actually not cold at all.{#Name="Cent"}

`Questo tipo di freddo semplicemente non basta{LF}a influenzare un uomo del mio calibro.{#Name="Cent"}`
This kind of cold simply isn't enough{LF}to affect a man of my caliber.{#Name="Cent"}

`E allora perché ti cola il naso{LF}come una fontana, eh?{#Name="Dito"}`
So why's your nose runnin'{LF}like a fountain, then, huh?{#Name="Dito"}

`Oh ho! Scommetto che questi stregoni{LF}sono la causa di questa ondata di freddo!{#Name="Octa"}`
Oh ho! I wager these sorcerers{LF}are the source of our cold snap!{#Name="Octa"}

`Allora andiamo a spaccargli il culo!{#Name="Zero"}`
Then let's go tear 'em a new one!{#Name="Zero"}

`然り{#Name="オクタ"}`
然り{#Name="オクタ"}

`ゼロ、殺っちゃえ！　殺っちゃえ！{#Name="ディト"}`
ゼロ、殺っちゃえ！　殺っちゃえ！{#Name="ディト"}

`ああ。さっさと済まそう{#Name="ゼロ"}`
ああ。さっさと済まそう{#Name="ゼロ"}

`Finalmente a terra...{#Name="Zero"}`
Finally down...{#Name="Zero"}

`Questo dovrebbe occuparsi di questo incantesimo di freddo.{#Name="Octa"}`
That should take care of this cold spell.{#Name="Octa"}

`Ugh! Mi sto stancando di questa storia!{#Name="Zero"}`
Ugh! I'm getting sick of this!{#Name="Zero"}

`Suppongo che degli spiriti abbiano preso{LF}il controllo di questi soldati non morti.{#Name="Octa"}`
I surmise that spirits have taken{LF}control of these undead soldiers.{#Name="Octa"}

`Devono esserci dei necromanti qui vicino.{#Name="Octa"}`
There must be necromancers nearby.{#Name="Octa"}

`Diventa più irritante a ogni minuto.{#Name="Zero"}`
This gets more annoying by the minute.{#Name="Zero"}

`Diventa più irritante a ogni minuto.{#Name="Zero"}`
This gets more annoying by the minute.{#Name="Zero"}

`Che state lì impalati a fare!? Uccideteli!{#Name="Zero"}`
What are you standing around for!? Kill 'em!{#Name="Zero"}

`Tutti questi sbalzi di temperatura{LF}mi stanno rovinando la salute...{#Name="Dito"}`
All these temperature fluxes{LF}are messin' with my health...{#Name="Dito"}

`IO ti rovino la salute se non ti muovi!{#Name="Zero"}`
I'LL mess with your health if you don't get moving!{#Name="Zero"}

`Sto combattendo! Sto combattendo davvero! Guardami!{#Name="Dito"}`
I'm fighting! I'm totally fighting! Watch me go!{#Name="Dito"}

`Che cos’è tutta questa agitazione, Octa?{#Name="Zero"}`
What's all the fuss about, Octa?{#Name="Zero"}

`Hmm? Non so di nessuna agitazione.{#Name="Octa"}`
Hmm? I don't know of any fuss?{#Name="Octa"}

`Oh, non lo sai, eh?{#Name="Zero"}`
Oh, you don't, do you?{#Name="Zero"}

`Allora che cos’è quell’improvviso{LF}manico di scopa nei pantaloni?{#Name="Zero"}`
Then what's with the sudden{LF}broom handle in your pants?{#Name="Zero"}

`Oh ho! Hai scoperto il mio segreto, vero!?{#Name="Octa"}`
Oh ho! You discovered my secret, did you!?{#Name="Octa"}

`Stavo solo intrattenendo qualche{LF}pensiero sconcio mentre combattevo.{#Name="Octa"}`
I was just engaging in some{LF}dirty thoughts while fighting.{#Name="Octa"}

`Beh, piantala e concentrati!{#Name="Zero"}`
Well, knock it off and concentrate already!{#Name="Zero"}

`Costretti a combattere anche dopo la morte.{LF}Che destino orribile.{#Name="Decadus"}`
Forced to fight even after death.{LF}Such a horrid fate.{#Name="Decadus"}

`Davvero? Pensavo che cose del genere ti piacessero.{#Name="Dito"}`
Really? I'd think you'd be into that sort of thing.{#Name="Dito"}

`Pensaci! Zero potrebbe continuare a prenderti a calci,{LF}pugni e fendenti anche DOPO la morte!{#Name="Dito"}`
Think about it! Zero could keep kicking and{LF}punching and slicing you even AFTER you die!{#Name="Dito"}

`Anche...dopo la morte? Unnngh...{#Name="Decadus"}`
Even...after I die? Unnngh...{#Name="Decadus"}

`Sai, potrei abbattere questi nemici{LF}con un solo pollice. Un solo pollice!{#Name="Cent"}`
You know, I could take these foes down{LF}with a single thumb. A single thumb!{#Name="Cent"}

`Peròòò non lo farò. Rovinerebbe il divertimento. {#Name="Cent"}`
Buuut I won't. That would just spoil the fun. {#Name="Cent"}

`Se puoi farlo, allora fallo e basta!{#Name="Zero"}`
If you can do it, then do it already!{#Name="Zero"}

`Nngh...rrgh...{#Name="Mikhail"}`
Nngh...rrgh...{#Name="Mikhail"}

`Che c’è? Hai freddo?{#Name="Zero"}`
What's wrong? You cold?{#Name="Zero"}

`Io...non lo so.{#Name="Mikhail"}`
I...I don't know.{#Name="Mikhail"}

`Non sembra essere in buona salute.{#Name="Decadus"}`
He does not seem to be in the best of health.{#Name="Decadus"}

`Perché sono tutti preoccupati per quel grosso frignone?{#Name="Dito"}`
Why's everybody worried about that big crybaby?{#Name="Dito"}

`Io sto molto peggio di lui in questo momento!{#Name="Dito"}`
I'm a hell of a lot worse off than him right now!{#Name="Dito"}

`Direi che è solo una questione di carattere.{#Name="Octa"}`
It's purely a matter of character, I'd say.{#Name="Octa"}

`Infatti. Mikhail ha una{LF}innocenza affascinante. Tu...no.{#Name="Decadus"}`
Indeed. Mikhail has a charming{LF}innocence about him. You...do not.{#Name="Decadus"}

`Oh, per l’amor del cielo! Non sa{LF}nemmeno dire se ha freddo o no!{#Name="Dito"}`
Oh, for hell's sake! He can't{LF}even tell if he's cold or not!{#Name="Dito"}

`Quella stupida lucertola se la caverà benissimo.{#Name="Dito"}`
That dumb lizard's gonna be just fine.{#Name="Dito"}

`Quante volte dobbiamo passarci!?{#Name="Zero"}`
How many times do we have to go through this!?{#Name="Zero"}

`Ancora e ANCORA... Ne ho abbastanza!{#Name="Zero"}`
Over and OVER again... I'm sick of it!{#Name="Zero"}

`Zero? Ehi, Zero! Credo di sentire freddo, Zero!{#Name="Mikhail"}`
Zero? Hey, Zero! I think I feel cold, Zero!{#Name="Mikhail"}

`Davvero? Mi sorprende che tu senta qualcosa{LF}attraverso quella testaccia dura.{#Name="Zero"}`
Really? I'm surprised you feel anything{LF}through that thick head of yours.{#Name="Zero"}

`Momento curiosità! Sapevate che i bagni nel ghiaccio{LF}in realtà ti scaldano se—{#Name="Cent"}`
Trivia time! Did you know ice baths{LF}actually warm you up if—{#Name="Cent"}

`Sta’ zitto, Cent.{#Name="Dito"}`
Shut up, Cent.{#Name="Dito"}

`Perché non ci ritiriamo per un po’ così possiamo recuperare?{#Name="Octa"}`
Why don't we retreat for a bit so we can recover?{#Name="Octa"}

`Non sono sicuro...che possiamo...{#Name="Mikhail"}`
I'm not sure...we can...{#Name="Mikhail"}

`Ooof. Sembra che quest’area sia{LF}chiusa da un potere magico...{#Name="Octa"}`
Ooof. It does seem this area is{LF}enclosed by magical power...{#Name="Octa"}

`Dev’essere quello che trattiene il freddo.{#Name="Decadus"}`
That must be what's holding in the cold.{#Name="Decadus"}

`Dannazione. Ucciderò ogni{LF}singolo stregone qui intorno!{#Name="Zero"}`
Goddammit. I'll kill every{LF}last sorcerer around here!{#Name="Zero"}

`Zero...{#Name="Mikhail"}`
Zero...{#Name="Mikhail"}

`Va tutto bene. Non ci vorrà molto.{#Name="Zero"}`
It's all right. This won't take long.{#Name="Zero"}

`くっ……くくく。{#Name="ゼロ"}`
くっ……くくく。{#Name="ゼロ"}

`Uccideremo ogni stronzo{LF}che ha avuto parte in questo.{#Name="Zero"}`
We're gonna kill every asshole{LF}who played a part in this.{#Name="Zero"}

`Stregoni... Soldati... Intoner... Tutti.{#Name="Zero"}`
Sorcerers... Soldiers... Intoners... Everyone.{#Name="Zero"}

`Se scappano, muoiono.{LF}Se si arrendono, muoiono lo stesso!{#Name="Zero"}`
If they run, they die.{LF}If they surrender, they still die!{#Name="Zero"}

`Dipingeremo le pareti col loro sangue!{#Name="Zero"}`
We're gonna paint the walls with their blood!{#Name="Zero"}

`487{#Name="487"}`
487{#Name="487"}

`Ne ho avuto abbastanza di voi stronzi{LF}e dei vostri stupidi cerchi magici!{#Name="Zero"}`
I've had enough of you jackasses{LF}and your stupid magic circles!{#Name="Zero"}

`Avanti, scappate pure! Morirete tutti comunque!{#Name="Zero"}`
Go ahead and run! You're all gonna die either way!{#Name="Zero"}

`GYAAAAAAAAAH!{#Name="Zero"}`
GYAAAAAAAAAH!{#Name="Zero"}

`Santo cielo! Che rozza{LF}dimostrazione di aggressività!{#Name="Cent"}`
Good heavens! What an oafish{LF}display of aggression!{#Name="Cent"}

`Mia signora, forse potremmo lasciar andare i nemici{LF}in fuga? Non sono un pericolo per noi.{#Name="Decadus"}`
My lady, perhaps we could let the fleeing{LF}enemies go? They are no danger to us.{#Name="Decadus"}

`Non esiste.{#Name="Zero"}`
No can do.{#Name="Zero"}

`Mia signora, forse potremmo lasciar andare i nemici{LF}in fuga? Non sono un pericolo per noi.{#Name="Decadus"}`
My lady, perhaps we could let the fleeing{LF}enemies go? They are no danger to us.{#Name="Decadus"}

`Non esiste.{#Name="Zero"}`
No can do.{#Name="Zero"}

`541{#Name="541"}`
541{#Name="541"}

`Sbrigatevi a morire! Ho fretta!{#Name="Zero"}`
Hurry up and die! I'm in a rush here!{#Name="Zero"}

`Voi figli di puttana proprio non sapete quando mollare.{#Name="Zero"}`
You sons of bitches just don't know when to quit.{#Name="Zero"}

`Il freddo? Il caldo? La sabbia?{LF}È tutta colpa vostra, cazzo!{#Name="Zero"}`
The cold? The heat? The sand?{LF}It's all your fuckin' fault!{#Name="Zero"}

`Non credo che siano responsabili{LF}per la SABBIA, in realtà...{#Name="Dito"}`
I don't think they're responsible{LF}for the SAND, actually...{#Name="Dito"}

`Ehi, ora va tutto bene. Okay?{#Name="Zero"}`
Hey, we're all good now. Okay?{#Name="Zero"}

`Okay...{#Name="Mikhail"}`
Okay...{#Name="Mikhail"}

`{06}>{#Name="Mikhail"}`
{06}>{#Name="Mikhail"}

`Che c’è?{#Name="Zero"}`
What's wrong?{#Name="Zero"}

`{06}>{#Name="Mikhail"}`
{06}>{#Name="Mikhail"}

`Che diavolo è questo?{#Name="Zero"}`
What the hell is this?{#Name="Zero"}

`Divergenza: Diramazione C{#Name="Accord"}`
Divergence: Branch C{#Name="Accord"}

`13 aprile dell’anno 1000 d.C.{#Name="Accord"}`
April the 13th, 1000 A.D.{#Name="Accord"}

`Ho osservato una diramazione estremamente preoccupante.{#Name="Accord"}`
I've observed an extremely worrisome branch.{#Name="Accord"}

`Quando la singolarità nota come Two catturò il drago Mikhail,{#Name="Accord"}`
When the singularity known as Two captured the dragon Mikhail,{#Name="Accord"}

`ho rilevato che gli stava imponendo una sorta di influenza magica.{#Name="Accord"}`
I detected her imposing some kind of magical influence upon him.{#Name="Accord"}

`Finora, solo Zero poteva esercitare tale influenza.{LF}Questa è la prima eccezione nota,{#Name="Accord"}`
Until now, only Zero could enact such influence.{LF}This is the first known exception,{#Name="Accord"}

`e in quanto tale aumenta la probabilità{LF}che si verifichi un incidente imprevisto.{#Name="Accord"}`
and as such, increases the likelihood{LF}of an unforeseen accident occurring.{#Name="Accord"}

`Questa questione deve avere priorità per un’osservazione attenta.{#Name="Accord"}`
This matter must be prioritized for careful observation.{#Name="Accord"}

`M4110_E0020_DM: Inizio registrazione{#Name="Accord"}`
M4110_E0020_DM: Begin Recording{#Name="Accord"}

`487{#Name="487"}`
487{#Name="487"}

`Eccoli qui. La radice di tutto questo male.{#Name="Zero"}`
Here they are. The root of all this evil.{#Name="Zero"}

