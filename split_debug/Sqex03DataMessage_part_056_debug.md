`Soldato F`
Soldier F

`Soldato E`
Soldier E

`Soldato G`
Soldier G

`Soldato D`
Soldier D

`Soldato C`
Soldier C

`Accord`
Accord

`テスト`
テスト

`Stare così in alto è bello, eh?{#Name="Mikhail"}`
Being high up like this is nice, huh?{#Name="Mikhail"}

`Se ti piace congelare.{#Name="Zero"}`
If you like freezing.{#Name="Zero"}

`Davvero? Hai freddo? A me sembra fantastico.{#Name="Mikhail"}`
Really? You're cold? It feels great to me.{#Name="Mikhail"}

`Già, beh, io non sono ricoperta di squame e grasso.{#Name="Zero"}`
Yeah, well, I'm not covered in scales and blubber.{#Name="Zero"}

`Uuurp... Questa è brutta...{#Name="Zero"}`
Uuurp... This is bad...{#Name="Zero"}

`Che c'è, Zero, ti ha preso una viverna?{LF}...Stupide viverne!{#Name="Mikhail"}`
What's wrong, Zero, did a wyvern get you?{LF}...Stupid wyverns!{#Name="Mikhail"}

`No. Puzzi così tanto che credo{LF}che vomiterò il pranzo.{#Name="Zero"}`
No. You smell so bad, I think{LF}I'm gonna horf my lunch.{#Name="Zero"}

`Puoi vomitarmi addosso, Zero!{LF}Non mi dà fastidio!{#Name="Mikhail"}`
It's okay to barf on me, Zero!{LF}I don't mind!{#Name="Mikhail"}

`Mi stai prendendo in giro?{#Name="Zero"}`
Are you kidding me?{#Name="Zero"}

`Ecco perché puzzi così tanto.{#Name="Zero"}`
No wonder you reek so bad.{#Name="Zero"}

`Ah ah! Grazie!{#Name="Mikhail"}`
Ha ha! Thanks!{#Name="Mikhail"}

`Non era un complimento!{#Name="Zero"}`
That wasn't a compliment!{#Name="Zero"}

`Wow. Te la stai cavando davvero{LF}contro queste viverne.{#Name="Dito"}`
Wow. You're actually holding your{LF}own against these wyverns.{#Name="Dito"}

`Chi, io? Mi stai facendo un complimento!?{#Name="Mikhail"}`
Who, me? Are you complimenting me!?{#Name="Mikhail"}

`Mi sa che i draghi sono davvero forti, dopotutto.{#Name="Dito"}`
I guess dragons are pretty strong after all, huh?{#Name="Dito"}

`Eh eh eeeeeh! Puoi dirlo forte!{#Name="Mikhail"}`
Hee heeeeee! You got that right!{#Name="Mikhail"}

`Ehi! Smettila di scodinzolare!{LF}Perderai l'equilibrio.{#Name="Zero"}`
Hey! Quit wagging your tail!{LF}You'll go off-balance.{#Name="Zero"}

`E Dito, chiudi quella bocca!{LF}Non stai aiutando.{#Name="Zero"}`
And Dito, shut the hell up down there!{LF}You're not helping.{#Name="Zero"}

`Ah ah ah ah ah ah!{#Name="Dito"}`
Ha ha ha ha ha ha!{#Name="Dito"}

`Cavolo, quelle nuvole davanti sono belle fitte.{#Name="Mikhail"}`
Boy, those are some thick clouds up ahead.{#Name="Mikhail"}

`Strano. ...Un po' TROPPO strano.{#Name="Zero"}`
Weird. ...A little TOO weird.{#Name="Zero"}

`Zero! Nevica troppo forte per vedere davanti!{#Name="Mikhail"}`
Zero! It's snowing too hard to see up ahead!{#Name="Mikhail"}

`Nnngh...{#Name="Zero"}`
Nnngh...{#Name="Zero"}

`Proviamo ad aggirarle?{LF}Che facciamo, Zero? ...Zero!?{#Name="Mikhail"}`
Should we try going around it?{LF}What do we do, Zero? ...Zero!?{#Name="Mikhail"}

`Agh! ...Dannazione. Fa così freddo,{LF}penso il mio cervello si sia appena congelato.{#Name="Zero"}`
Agh! ...Dammit. It's so cold,{LF}I think my brain just froze.{#Name="Zero"}

`Beh, riprenditi! Abbiamo ancora{LF}delle viverne sporche e puzzolenti da uccidere!{#Name="Mikhail"}`
Well, snap out of it! We've still got{LF}dirty stinking wyverns to kill!{#Name="Mikhail"}

`Ehi, scemo... Perché voli{LF}con la bocca aperta?{#Name="Zero"}`
Hey, dummy... Why are you flying{LF}with your mouth open?{#Name="Zero"}

`Ehm... perché così ci entra la neve?{#Name="Mikhail"}`
Um...because then the snow goes in there?{#Name="Mikhail"}

`...E quindi?{#Name="Zero"}`
...And?{#Name="Zero"}

`E, ehm... è fredda, è buona ed è... divertente?{#Name="Mikhail"}`
And, um...it's cold and it's yummy and it's...fun?{#Name="Mikhail"}

`...E QUINDI?{#Name="Zero"}`
...AND?{#Name="Zero"}

`Io... mi dispiace! Mi concentrerò sulla battaglia!{#Name="Mikhail"}`
I...I'm sorry! I'll focus on the battle!{#Name="Mikhail"}

`Dannazione, qui fuori non vedo niente...{#Name="Zero"}`
Dammit, I can't see anything out here...{#Name="Zero"}

`あ。ゼロさん、あぶない！{#Name="ディト"}`
あ。ゼロさん、あぶない！{#Name="ディト"}

`ふー。間一髪か……。空中戦は気配を察することが難しいな{#Name="ゼロ"}`
ふー。間一髪か……。空中戦は気配を察することが難しいな{#Name="ゼロ"}

`Immagino che vedremo presto Four, eh?{#Name="Mikhail"}`
I guess we'll be seeing Four soon, huh?{#Name="Mikhail"}

`Già. Non le resta più nessun posto dove scappare.{#Name="Zero"}`
Yeah. She's got nowhere left to run.{#Name="Zero"}

`Beh, prova a parlarle, okay?{#Name="Mikhail"}`
Well, try to talk to her, okay?{#Name="Mikhail"}

`Cosa?{#Name="Zero"}`
What?{#Name="Zero"}

`Siete sorelle! Se parlate, potreste{LF}capirvi meglio!{#Name="Mikhail"}`
You're sisters! If you guys talk, you might{LF}be able to understand each other better!{#Name="Mikhail"}

`Non ABBIAMO bisogno di capirci meglio.{#Name="Zero"}`
We don't NEED to understand each other better.{#Name="Zero"}

`Four sarà con il suo discepolo, giusto?{#Name="Dito"}`
Four's gonna be with her disciple, right?{#Name="Dito"}

`Possibile.{#Name="Zero"}`
Probably.{#Name="Zero"}

`Immagino che quel tizio abbia dei problemi seri.{#Name="Zero"}`
I imagine that guy's got some serious issues.{#Name="Zero"}

`Non so, dici? Magari è solo{LF}costretto a stare lì, come me.{#Name="Dito"}`
I dunno, you think? Maybe he's just{LF}forced to be there like I was.{#Name="Dito"}

`僕みたいに{LF}しぶしぶウタヒメと一緒にいるのかもよ？{#Name="ディト"}`
僕みたいに{LF}しぶしぶウタヒメと一緒にいるのかもよ？{#Name="ディト"}

`...Sì, ma tu l'hai fatto con Five, giusto?{#Name="Zero"}`
...Yeah, but you did the deed with Five, right?{#Name="Zero"}

`Uuuugh...{#Name="Dito"}`
Uuuugh...{#Name="Dito"}

`È stato terribile, venire sballottato da{LF}quei giganteschi sacchi da boxe ogni notte...{#Name="Dito"}`
It was awful, being knocked around by{LF}those giant punching bags every night...{#Name="Dito"}

`Bugiardo. Ti piaceva.{#Name="Zero"}`
Liar. You loved it.{#Name="Zero"}

`Ti piaceva averla in ginocchio, a supplicarti{LF}di continuare. Ce l'hai scritto in faccia.{#Name="Zero"}`
You loved having her on her knees, begging you{LF}for more. It's written all over your face.{#Name="Zero"}

`Ma col cazzo!{#Name="Dito"}`
Bullshit it is!{#Name="Dito"}

`Eh... Hai proprio un bel caratterino, eh?{#Name="Zero"}`
Heh... He's got a temper, eh?{#Name="Zero"}

`Perché sei arrabbiata con Four, Zero? È personale?{#Name="Mikhail"}`
Why are you mad at Four, Zero? Is it personal?{#Name="Mikhail"}

`Personale?{#Name="Zero"}`
Personal?{#Name="Zero"}

`Tipo, ti ha rubato il gelato{LF}da piccola o roba così?{#Name="Mikhail"}`
Like, did she steal your ice cream{LF}as a kid or whatever?{#Name="Mikhail"}

`Non c'entra niente. Semplicemente la voglio morta.{#Name="Zero"}`
It's not about that. I just want her dead.{#Name="Zero"}

`Ma perché!?{#Name="Mikhail"}`
But why!?{#Name="Mikhail"}

`Tutte le sorelle vogliono uccidersi a vicenda.{LF}La maggior parte non passa ai fatti.{#Name="Zero"}`
All sisters want to kill each other.{LF}Most just don't follow through.{#Name="Zero"}

`Sul serio? QUELLO è il demone di Four?{#Name="Zero"}`
Seriously? THAT'S Four's daemon?{#Name="Zero"}

`Sembra più un dannato castello.{#Name="Zero"}`
Looks more like a goddamn castle.{#Name="Zero"}

`Guarda tutti quei cannoni sul perimetro...{#Name="Zero"}`
Look at all those cannons on the perimeter...{#Name="Zero"}

`Aggiralo dal basso!{#Name="Zero"}`
Circle in from below!{#Name="Zero"}

`Non ci riesco! Non ci riesco!{#Name="Mikhail"}`
I can't! I can't!{#Name="Mikhail"}

`Perché no!?{#Name="Zero"}`
Why not!?{#Name="Zero"}

`Quei cosi quadrati ad anello sono magia!{#Name="Mikhail"}`
Those square ring thingies are magic!{#Name="Mikhail"}

`Il potere della sua canzone mi rende difficile volare!{#Name="Mikhail"}`
Her song's power is making it hard for me to fly!{#Name="Mikhail"}

`Mi sta attirando dentro!{#Name="Mikhail"}`
It's pulling me in!{#Name="Mikhail"}

`Fantastico. Ora dobbiamo eliminare{LF}tutti questi dannati cannoni...{#Name="Zero"}`
Great. Now we gotta take out{LF}all these freaking cannons...{#Name="Zero"}

`Ti piace proprio rompermi le palle,{LF}vero, Four!?{#Name="Zero"}`
You just love messing with me,{LF}don't you, Four!{#Name="Zero"}

`Perché, sorella mia? Perché mi odi così tanto?{#Name="Four"}`
Why, my sister? Why do you hate me so?{#Name="Four"}

`Non ti odio. Voglio solo ucciderti.{#Name="Zero"}`
I don't hate you. I just want to kill you.{#Name="Zero"}

`Ma perché!? Non capisco!{#Name="Four"}`
But, why!? I don't understand!{#Name="Four"}

`Mi fa un po' pena...{#Name="Mikhail"}`
I feel kinda bad for her...{#Name="Mikhail"}

`Vuoi chiuderla!?{#Name="Zero"}`
Shut it, would you!?{#Name="Zero"}

`Zero, ti prego! Rinsavisci!{#Name="Four"}`
Zero, please! Snap out of this!{#Name="Four"}

`Dov'è la Zero gentile e buona che conoscevo?{#Name="Four"}`
Where's the kind, gentle Zero I once knew?{#Name="Four"}

`Che ne so. Sono sempre stata così.{#Name="Zero"}`
Beats me. I've always been like this.{#Name="Zero"}

`Smettila e basta, Zero!{#Name="Four"}`
Just stop this already, Zero!{#Name="Four"}

`Mi dispiace. Non posso.{#Name="Zero"}`
Sorry. No can do.{#Name="Zero"}

`Non posso combatterti, Zero.{LF}Non voglio combatterti!{#Name="Four"}`
I can't fight you, Zero.{LF}I don't want to fight you!{#Name="Four"}

`Per me va bene. Allora ti ucciderò senza combattere.{#Name="Zero"}`
Fine by me. Then I'll kill you without a fight.{#Name="Zero"}

`Oh, Dio... Qualcuno, vi prego...{LF}Svegliatemi da questo incubo...{#Name="Four"}`
Oh, God... Someone, please...{LF}Wake me from this nightmare...{#Name="Four"}

`Four e Decadus sembrano entrambi brave persone...{LF}Non voglio ucciderli.{#Name="Mikhail"}`
Four and Decadus both seem like good people...{LF}I don't want to kill them.{#Name="Mikhail"}

`Dillo a qualcuno a cui importi qualcosa.{#Name="Zero"}`
Save it for someone who gives a shit.{#Name="Zero"}

`No, davvero! Guardaci! Se qui c'è{LF}un cattivo, siamo noi!{#Name="Mikhail"}`
No, really! Look at us! If anyone's{LF}a bad guy here, it's us!{#Name="Mikhail"}

`E allora? Adesso posso uccidere solo i malvagi?{LF}È questo che stai dicendo?{#Name="Zero"}`
So, what? I can only kill evil people now?{LF}Is that what you're saying?{#Name="Zero"}

`Indovina un po'? Nella vita vera,{LF}i cattivi vincono di continuo.{#Name="Zero"}`
Guess what? In real life,{LF}bad guys win all the time.{#Name="Zero"}

`Quindi smettila di rompere con il tuo{LF}mondo felice di fantasia, prima che uccida anche te.{#Name="Zero"}`
So quit bitching about your happy{LF}fantasy world before I kill you, too.{#Name="Zero"}

`うらあああっ！フォウ、どこだァ？出てこい！{#Name="ゼロ"}`
うらあああっ！フォウ、どこだァ？出てこい！{#Name="ゼロ"}

`砲台は邪魔だ！邪魔だ！邪魔だ！{LF}邪魔だ！邪魔だ！邪魔だ～～～ッ！{#Name="ゼロ"}`
砲台は邪魔だ！邪魔だ！邪魔だ！{LF}邪魔だ！邪魔だ！邪魔だ～～～ッ！{#Name="ゼロ"}

`どんだけ逃げりゃ気がすむんだ？{#Name="ゼロ"}`
どんだけ逃げりゃ気がすむんだ？{#Name="ゼロ"}

`おとなしく死ねッ！{#Name="ゼロ"}`
おとなしく死ねッ！{#Name="ゼロ"}

`Ci siamo quasi!{#Name="Mikhail"}`
Almost there!{#Name="Mikhail"}

`Resta ferma, sorella. Non ti resta più dove scappare!{#Name="Zero"}`
Sit still, Sister. There's nowhere left to run!{#Name="Zero"}

`ナントカ山上空{#Name="説明文"}`
ナントカ山上空{#Name="説明文"}

`Distruggi tutti gli occhi di Armaros!{#Name="説明文"}`
Destroy all of Armaros' eyes!{#Name="説明文"}

`Sconfiggi Armaros!{#Name="説明文"}`
Defeat Armaros!{#Name="説明文"}

`全てのアルマロスの瞳を撃破した！{#Name="説明文"}`
全てのアルマロスの瞳を撃破した！{#Name="説明文"}

