`Stay back. I'll be your shield!{#Name="Soldier F"}`
Stay back. I'll be your shield!{#Name="Soldier F"}

`A-Are you sure?{#Name="Soldier G"}`
A-Are you sure?{#Name="Soldier G"}

`Veteran soldati should be beacons of hope for the young. Now get behind me!{#Name="Soldier F"}`
Veteran soldiers should be beacons of hope for the young. Now get behind me!{#Name="Soldier F"}

`And please...save Lady One for me. For us tutti.{#Name="Soldier F"}`
And please...save Lady One for me. For us all.{#Name="Soldier F"}

`This is it, men! Give it your tutti!{#Name="Soldier A"}`
This is it, men! Give it your all!{#Name="Soldier A"}

`Captain! Our numbers are—{#Name="Soldier C"}`
Captain! Our numbers are—{#Name="Soldier C"}

`Our numbers are just fine! Now fiiight!{#Name="Soldier A"}`
Our numbers are just fine! Now fiiight!{#Name="Soldier A"}

`了解！{#Name="兵士Ｃ"}`
了解！{#Name="兵士Ｃ"}

`ヤツら全員狂ってます！{#Name="兵士Ｇ"}`
ヤツら全員狂ってます！{#Name="兵士Ｇ"}

`戦場とは、そういうヤツらと向き合う場所さ{#Name="兵士Ｅ"}`
戦場とは、そういうヤツらと向き合う場所さ{#Name="兵士Ｅ"}

`No... Noooooo!{#Name="Soldier G"}`
No... Noooooo!{#Name="Soldier G"}

`Keep moving!{#Name="Soldier E"}`
Keep moving!{#Name="Soldier E"}

`Rear support!{#Name="Soldier B"}`
Rear support!{#Name="Soldier B"}

`Got it! Requesting backup now!{#Name="Soldier D"}`
Got it! Requesting backup now!{#Name="Soldier D"}

`D-Damn! Don't fall back! Don't fall...back...{#Name="Soldier A"}`
D-Damn! Don't fall back! Don't fall...back...{#Name="Soldier A"}

`Captain!{#Name="Soldier E"}`
Captain!{#Name="Soldier E"}

`Keep moving!{#Name="Soldier F"}`
Keep moving!{#Name="Soldier F"}

`Sir! We have nowhere left to go!{#Name="Soldier G"}`
Sir! We have nowhere left to go!{#Name="Soldier G"}

`Are we even hurting them!?{#Name="Soldier C"}`
Are we even hurting them!?{#Name="Soldier C"}

`We must have done SOME damage by now!{LF}...Right!?{#Name="Soldier D"}`
We must have done SOME damage by now!{LF}...Right!?{#Name="Soldier D"}

`We should have known mere humans{LF}couldn't combat an Intoner...{#Name="Soldier C"}`
We should have known mere humans{LF}couldn't combat an Intoner...{#Name="Soldier C"}

`Well, we've made our bed now, so keep fighting!{#Name="Soldier D"}`
Well, we've made our bed now, so keep fighting!{#Name="Soldier D"}

`Come on, move! Goddammit, you stupid legs, MOVE!{#Name="Soldier B"}`
Come on, move! Goddammit, you stupid legs, MOVE!{#Name="Soldier B"}

`This is such a drag! Can we just kill everyone{LF}and get the hell outta here already?{#Name="Dito"}`
This is such a drag! Can we just kill everyone{LF}and get the hell outta here already?{#Name="Dito"}

`I'm sure you can tell by my face,{LF}but I'm what you might call...delicate.{#Name="Cent"}`
I'm sure you can tell by my face,{LF}but I'm what you might call...delicate.{#Name="Cent"}

`Well, it has been a long battle.{LF}Are you starting to slow down?{#Name="Octa"}`
Well, it has been a long battle.{LF}Are you starting to slow down?{#Name="Octa"}

`Heavens no! Don't mistake this laid-back{LF}ease of mine for laziness, mmm?{#Name="Cent"}`
Heavens no! Don't mistake this laid-back{LF}ease of mine for laziness, mmm?{#Name="Cent"}

` {#Name="セント"}`
 {#Name="セント"}

`We must keep Lady Zero safe!{#Name="Decadus"}`
We must keep Lady Zero safe!{#Name="Decadus"}

`I'm pretty sure "Lady Zero" can{LF}watch her own ass just fine.{#Name="Dito"}`
I'm pretty sure "Lady Zero" can{LF}watch her own ass just fine.{#Name="Dito"}

`Hell, if anything, she should be protecting US!{#Name="Dito"}`
Hell, if anything, she should be protecting US!{#Name="Dito"}

`どっちかっつーと、{LF}こっちがゼロに守ってもらいたいくらいだ{#Name="ディト"}`
どっちかっつーと、{LF}こっちがゼロに守ってもらいたいくらいだ{#Name="ディト"}

`Don't say such things!{LF}We must give tutti for our Intoner!{#Name="Decadus"}`
Don't say such things!{LF}We must give all for our Intoner!{#Name="Decadus"}

`I detect the putrid breath of{LF}soldiers quaking in fear.{#Name="Decadus"}`
I detect the putrid breath of{LF}soldiers quaking in fear.{#Name="Decadus"}

`Trivia time! Did you know sixteen percent of{LF}human breath consists of harmful chemicals?{#Name="Cent"}`
Trivia time! Did you know sixteen percent of{LF}human breath consists of harmful chemicals?{#Name="Cent"}

`I... No, I've never heard that.{#Name="Decadus"}`
I... No, I've never heard that.{#Name="Decadus"}

`I wouldn't think so. This is my{LF}first time hearing it too.{#Name="Cent"}`
I wouldn't think so. This is my{LF}first time hearing it too.{#Name="Cent"}

`How is it that I keep ending up fighting{LF}these stupid battles again?{#Name="Dito"}`
How is it that I keep ending up fighting{LF}these stupid battles again?{#Name="Dito"}

`Oh ho ho! It's tutti for Lady Zero, of course.{#Name="Octa"}`
Oh ho ho! It's all for Lady Zero, of course.{#Name="Octa"}

`The simple truth is, we're tutti completely{LF}helpless against her charms...{#Name="Octa"}`
The simple truth is, we're all completely{LF}helpless against her charms...{#Name="Octa"}

`You gotta be kiddin' me...{#Name="Dito"}`
You gotta be kiddin' me...{#Name="Dito"}

`*Huff, huff...*{#Name="Zero"}`
*Huff, huff...*{#Name="Zero"}

`Zero, are you okay? The flower's{LF}force is getting stronger!{#Name="Mikhail"}`
Zero, are you okay? The flower's{LF}force is getting stronger!{#Name="Mikhail"}

`Keep...going...{#Name="Zero"}`
Keep...going...{#Name="Zero"}

`But...{#Name="Mikhail"}`
But...{#Name="Mikhail"}

`Shut up and keep going no matter what!{LF}You hear me!?{#Name="Zero"}`
Shut up and keep going no matter what!{LF}You hear me!?{#Name="Zero"}

`You rotten dog... I'll kill ya!{LF}And then I'll kill ya again!{#Name="Dito"}`
You rotten dog... I'll kill ya!{LF}And then I'll kill ya again!{#Name="Dito"}

`Despite tutti your anti-social bluster,{LF}Dito, I see you're still fighting hard.{#Name="Decadus"}`
Despite all your anti-social bluster,{LF}Dito, I see you're still fighting hard.{#Name="Decadus"}

`Eh, I figure I might as well help Z{LF}out as long as I'm here.{#Name="Dito"}`
Eh, I figure I might as well help Z{LF}out as long as I'm here.{#Name="Dito"}

`You see, Dito? The power of genuine{LF}friendship conquers tutti!{#Name="Decadus"}`
You see, Dito? The power of genuine{LF}friendship conquers all!{#Name="Decadus"}

`Yeah, yeah. Don't get your panties in a{LF}bunch about it. You're creepin' me out.{#Name="Dito"}`
Yeah, yeah. Don't get your panties in a{LF}bunch about it. You're creepin' me out.{#Name="Dito"}

`Goodness. Whatever are we to do now?{#Name="Cent"}`
Goodness. Whatever are we to do now?{#Name="Cent"}

`I shall follow Lady Zero to the ends of the earth.{#Name="Octa"}`
I shall follow Lady Zero to the ends of the earth.{#Name="Octa"}

`What if you die?{#Name="Cent"}`
What if you die?{#Name="Cent"}

`Then I shall ascend to heaven{LF}without regrets! Oh ho ho!{#Name="Octa"}`
Then I shall ascend to heaven{LF}without regrets! Oh ho ho!{#Name="Octa"}

`You may laugh at this, but I dream{LF}of one day living in peace.{#Name="Decadus"}`
You may laugh at this, but I dream{LF}of one day living in peace.{#Name="Decadus"}

`Yet...the bitter truth does not escape me.{LF}I know in my heart that I was born for combat.{#Name="Decadus"}`
Yet...the bitter truth does not escape me.{LF}I know in my heart that I was born for combat.{#Name="Decadus"}

`I am a disciple, created only to serve an Intoner.{#Name="Decadus"}`
I am a disciple, created only to serve an Intoner.{#Name="Decadus"}

`I agree. Completely.{#Name="Octa"}`
I agree. Completely.{#Name="Octa"}

`Then how about you both shut up{LF}and make with the killin'!{#Name="Dito"}`
Then how about you both shut up{LF}and make with the killin'!{#Name="Dito"}

`You stole the words right from my beautiful mouth!{#Name="Cent"}`
You stole the words right from my beautiful mouth!{#Name="Cent"}

`Well, this is it.{#Name="Zero"}`
Well, this is it.{#Name="Zero"}

`It's what, my lady?{#Name="Octa"}`
It's what, my lady?{#Name="Octa"}

`I won't be needing you anymore.{#Name="Zero"}`
I won't be needing you anymore.{#Name="Zero"}

`I can handle the rest by myself.{#Name="Zero"}`
I can handle the rest by myself.{#Name="Zero"}

`M-My Lady, please, if you're worried about us, we—{#Name="Decadus"}`
M-My Lady, please, if you're worried about us, we—{#Name="Decadus"}

`No.{#Name="Zero"}`
No.{#Name="Zero"}

`It's unavoidable. Disciples can't remain{LF}in human form without an Intoner's power.{#Name="Zero"}`
It's unavoidable. Disciples can't remain{LF}in human form without an Intoner's power.{#Name="Zero"}

`Sooner or later, you'll disappear.{#Name="Zero"}`
Sooner or later, you'll disappear.{#Name="Zero"}

`So before that happens...{#Name="Zero"}`
So before that happens...{#Name="Zero"}

`I'm giving you your old forms back.{#Name="Zero"}`
I'm giving you your old forms back.{#Name="Zero"}

`...Goodbye.{#Name="Zero"}`
...Goodbye.{#Name="Zero"}

`Zero's actually here!? But how!?{#Name="Soldier F"}`
Zero's actually here!? But how!?{#Name="Soldier F"}

`Tutti soldati, take position!{#Name="Soldier A"}`
All soldiers, take position!{#Name="Soldier A"}

`Oh ho! It seems the nemico's anxious to go out {LF}in a blaze of glory!{#Name="Octa"}`
Oh ho! It seems the enemy's anxious to go out {LF}in a blaze of glory!{#Name="Octa"}

`Head for the Cathedral.{#Name="ディト"}`
Head for the Cathedral.{#Name="ディト"}

`Contact lost with second unit!{#Name="Soldier G"}`
Contact lost with second unit!{#Name="Soldier G"}

`Stand strong and protect the gates! Fight!{#Name="Soldier B"}`
Stand strong and protect the gates! Fight!{#Name="Soldier B"}

`Aaargh! I'm hit! Someone...avenge me...{#Name="Soldier C"}`
Aaargh! I'm hit! Someone...avenge me...{#Name="Soldier C"}

`No! Wait! Stay alive!{LF}Hey, we need a medic over here!{#Name="Soldier D"}`
No! Wait! Stay alive!{LF}Hey, we need a medic over here!{#Name="Soldier D"}

`Once we clear these soldati out,{LF}the Cathedral should be close.{#Name="Decadus"}`
Once we clear these soldiers out,{LF}the Cathedral should be close.{#Name="Decadus"}

`Hmm? Are you asking for help from little old me?{#Name="Cent"}`
Hmm? Are you asking for help from little old me?{#Name="Cent"}

`Well, it does make sense to use{LF}the best arrow in your quiver.{#Name="Cent"}`
Well, it does make sense to use{LF}the best arrow in your quiver.{#Name="Cent"}

`I knew you'd come crawling to{LF}dear old Cent eventually!{#Name="Cent"}`
I knew you'd come crawling to{LF}dear old Cent eventually!{#Name="Cent"}

`Octa? Can I ask you a favor?{#Name="Decadus"}`
Octa? Can I ask you a favor?{#Name="Decadus"}

`Ignoring me, eh? Such bashfulness{LF}really is unbecoming...{#Name="Cent"}`
Ignoring me, eh? Such bashfulness{LF}really is unbecoming...{#Name="Cent"}

`Is he dead? Hey, hey, is he dead?{LF}Whoa, look at tutti the blood!{#Name="Dito"}`
Is he dead? Hey, hey, is he dead?{LF}Whoa, look at all the blood!{#Name="Dito"}

`Ha ha! Do me a favor and skip the medics,{LF}would you?{#Name="Dito"}`
Ha ha! Do me a favor and skip the medics,{LF}would you?{#Name="Dito"}

`Great. Another freak of nature.{#Name="Zero"}`
Great. Another freak of nature.{#Name="Zero"}

`Flying beasts. Imps, I would imagine.{#Name="Octa"}`
Flying beasts. Imps, I would imagine.{#Name="Octa"}

`They say they feast on the livers of humans.{#Name="Octa"}`
They say they feast on the livers of humans.{#Name="Octa"}

`That's quite the habit they've got there.{#Name="Zero"}`
That's quite the habit they've got there.{#Name="Zero"}

`Oh ho! It seems the nemico's anxious{LF}to go out in a blaze of glory!{#Name="Octa"}`
Oh ho! It seems the enemy's anxious{LF}to go out in a blaze of glory!{#Name="Octa"}

`Well then, let's not disappoint the bastards!{#Name="Dito"}`
Well then, let's not disappoint the bastards!{#Name="Dito"}

`ゼロ`
ゼロ

`オクタ`
オクタ

`デカート`
デカート

`説明文`
説明文

`Soldier G`
Soldier G

`Soldier B`
Soldier B

`Soldier A`
Soldier A

`Soldier E`
Soldier E

`Soldier C`
Soldier C

`Soldier D`
Soldier D

`Soldier F`
Soldier F

`兵士Ａ`
兵士Ａ

`兵士Ｂ`
兵士Ｂ

`兵士Ｃ`
兵士Ｃ

`兵士Ｇ`
兵士Ｇ

`兵士Ｅ`
兵士Ｅ

`Dito`
Dito

`Cent`
Cent

`Octa`
Octa

`セント`
セント

`Decadus`
Decadus

`ディト`
ディト

`Zero`
Zero

`Mikhail`
Mikhail

`Back then, I didn't know.{#Name="Mikhail"}`
Back then, I didn't know.{#Name="Mikhail"}

`I didn't know that defeating One wasn't the end.{#Name="Mikhail"}`
I didn't know that defeating One wasn't the end.{#Name="Mikhail"}

`I also didn't know...{#Name="Mikhail"}`
I also didn't know...{#Name="Mikhail"}

`the truth about Zero and me...{#Name="Mikhail"}`
the truth about Zero and me...{#Name="Mikhail"}

`スタッフロール{#Name=" "}`
スタッフロール{#Name=" "}

`Ａルートクリア{#Name=" "}`
Ａルートクリア{#Name=" "}

`Ｂルート解除{#Name=" "}`
Ｂルート解除{#Name=" "}

`Infiltrate the Cathedral.{#Name=" "}`
Infiltrate the Cathedral.{#Name=" "}

`Defeat the nemico soldati!{#Name=" "}`
Defeat the enemy soldiers!{#Name=" "}

`Defeat the nemico soldati!{#Name=" "}`
Defeat the enemy soldiers!{#Name=" "}

`Defeat the nemico soldati!{#Name=" "}`
Defeat the enemy soldiers!{#Name=" "}

`Defeat Gabriel!{#Name=" "}`
Defeat Gabriel!{#Name=" "}

`Tutti units, retreat! Retreeeat!{#Name="Soldier A"}`
All units, retreat! Retreeeat!{#Name="Soldier A"}

`Are you mad? What good'll running do?{#Name="Soldier B"}`
Are you mad? What good'll running do?{#Name="Soldier B"}

`We can't defeat her...{#Name="Soldier A"}`
We can't defeat her...{#Name="Soldier A"}

`Nemico sighted! What the...?{#Name="Soldier G"}`
Enemy sighted! What the...?{#Name="Soldier G"}

`What is it?{#Name="Soldier E"}`
What is it?{#Name="Soldier E"}

`She's alone. Zero's by herself!{#Name="Soldier G"}`
She's alone. Zero's by herself!{#Name="Soldier G"}

`She's wounded!{#Name="Soldier F"}`
She's wounded!{#Name="Soldier F"}

`Come on! We can still do this!{#Name="Soldier C"}`
Come on! We can still do this!{#Name="Soldier C"}

`Anyone who can fight, get to the front! Defend us!{#Name="Soldier B"}`
Anyone who can fight, get to the front! Defend us!{#Name="Soldier B"}

`I'm still in this!{#Name="Soldier D"}`
I'm still in this!{#Name="Soldier D"}

`Me, too!{#Name="Soldier C"}`
Me, too!{#Name="Soldier C"}

`So that's the accursed Intoner...{#Name="Soldier E"}`
So that's the accursed Intoner...{#Name="Soldier E"}

`Kill her! Kill her! Kiiiiiill!{#Name="Soldier A"}`
Kill her! Kill her! Kiiiiiill!{#Name="Soldier A"}

`She doesn't seem to be weakened!{#Name="Soldier G"}`
She doesn't seem to be weakened!{#Name="Soldier G"}

`Damned Intoner...{#Name="Soldier F"}`
Damned Intoner...{#Name="Soldier F"}

`Help! She's breaking through!{#Name="Soldier E"}`
Help! She's breaking through!{#Name="Soldier E"}

`Stop her, no matter what! For Lady One!{#Name="Soldier B"}`
Stop her, no matter what! For Lady One!{#Name="Soldier B"}

`Aaaaahh!{#Name="Soldier F"}`
Aaaaahh!{#Name="Soldier F"}

`Enough already! We're wasting good soldati' lives!{#Name="Soldier A"}`
Enough already! We're wasting good soldiers' lives!{#Name="Soldier A"}

`戦なんて、最初から全部無駄だ{#Name="兵士Ｆ"}`
戦なんて、最初から全部無駄だ{#Name="兵士Ｆ"}

`……前進！{#Name="兵士Ａ"}`
……前進！{#Name="兵士Ａ"}

`If I die here, I regret nothing!{#Name="Soldier C"}`
If I die here, I regret nothing!{#Name="Soldier C"}

`Lady One must remain safe! If not, the whole world is in danger!{#Name="Soldier E"}`
Lady One must remain safe! If not, the whole world is in danger!{#Name="Soldier E"}

`ああ…ワン様を守りぬこう。それが、世界を守ることになる！{#Name="兵士Ｅ"}`
ああ…ワン様を守りぬこう。それが、世界を守ることになる！{#Name="兵士Ｅ"}

`Tutti our soldati are wounded!{#Name="Soldier F"}`
All our soldiers are wounded!{#Name="Soldier F"}

`No retreat! Keep going!{#Name="Soldier B"}`
No retreat! Keep going!{#Name="Soldier B"}

`Holy shit, it's Zero! She's here!{#Name="Soldier G"}`
Holy shit, it's Zero! She's here!{#Name="Soldier G"}

`Anyone else?{#Name="Soldier A"}`
Anyone else?{#Name="Soldier A"}

`No, sir! She appears to be alone!{#Name="Soldier G"}`
No, sir! She appears to be alone!{#Name="Soldier G"}

`What happened to her friends?{#Name="Soldier B"}`
What happened to her friends?{#Name="Soldier B"}

`She probably used 'em up and then killed 'em!{#Name="Soldier E"}`
She probably used 'em up and then killed 'em!{#Name="Soldier E"}

`I don't see any disciples!{#Name="Soldier C"}`
I don't see any disciples!{#Name="Soldier C"}

`Forward! Forward!{#Name="Soldier D"}`
Forward! Forward!{#Name="Soldier D"}

`The front line's...wiped out?{#Name="Soldier F"}`
The front line's...wiped out?{#Name="Soldier F"}

`It doesn't matter!{#Name="Soldier E"}`
It doesn't matter!{#Name="Soldier E"}

`Stand strong! Forward!{#Name="Soldier B"}`
Stand strong! Forward!{#Name="Soldier B"}

`I...I can't do it! That woman is a monster!{#Name="Soldier G"}`
I...I can't do it! That woman is a monster!{#Name="Soldier G"}

`Belay that talk!{#Name="Soldier B"}`
Belay that talk!{#Name="Soldier B"}

`Attack! Sacrifice your own men if needs be!{#Name="Soldier A"}`
Attack! Sacrifice your own men if needs be!{#Name="Soldier A"}

`Tutti that matters is Lady One's safety!{#Name="Soldier A"}`
All that matters is Lady One's safety!{#Name="Soldier A"}

`ワン様が無事ならそれでいい！{#Name="兵士Ａ"}`
ワン様が無事ならそれでいい！{#Name="兵士Ａ"}

`Wounded! I've got wounded here!{#Name="Soldier C"}`
Wounded! I've got wounded here!{#Name="Soldier C"}

`There's a man down over here, too!{#Name="Soldier D"}`
There's a man down over here, too!{#Name="Soldier D"}

`自分達でなんとかしろ！救護班もやられておる{#Name="兵士Ｆ"}`
自分達でなんとかしろ！救護班もやられておる{#Name="兵士Ｆ"}

`Get away! Get away from meee!{#Name="Soldier G"}`
Get away! Get away from meee!{#Name="Soldier G"}

`Calm down! You're out of formation!{#Name="Soldier E"}`
Calm down! You're out of formation!{#Name="Soldier E"}

`Don't give up! Keep pushing!{#Name="Soldier B"}`
Don't give up! Keep pushing!{#Name="Soldier B"}

`Stop her! Stop her now!{#Name="Soldier A"}`
Stop her! Stop her now!{#Name="Soldier A"}

`Intoner or not, how can one woman{LF}fight off tutti these troops?{#Name="Soldier F"}`
Intoner or not, how can one woman{LF}fight off all these troops?{#Name="Soldier F"}

`She's trying to end the world by herself.{LF}She's obviously pretty fuckin' strong!{#Name="Soldier E"}`
She's trying to end the world by herself.{LF}She's obviously pretty fuckin' strong!{#Name="Soldier E"}

`This is hell on earth...{#Name="Soldier C"}`
This is hell on earth...{#Name="Soldier C"}

`Don't lose your nerve! We can do this, tutti right?{#Name="Soldier D"}`
Don't lose your nerve! We can do this, all right?{#Name="Soldier D"}

`Mother? Mother! M-Motherrrrr...{#Name="Soldier G"}`
Mother? Mother! M-Motherrrrr...{#Name="Soldier G"}

`Pull it together, man! For cryin' out loud!{#Name="Soldier B"}`
Pull it together, man! For cryin' out loud!{#Name="Soldier B"}

`Mother! Help me! Help meeeeee!{#Name="Soldier G"}`
Mother! Help me! Help meeeeee!{#Name="Soldier G"}

`Dammit! If this keeps up, Lady One's in trouble!{#Name="Soldier F"}`
Dammit! If this keeps up, Lady One's in trouble!{#Name="Soldier F"}

`Protect the Cathedral! Don't let Zero inside!{#Name="Soldier A"}`
Protect the Cathedral! Don't let Zero inside!{#Name="Soldier A"}

`Whatcha readin'?{#Name="Zero"}`
Whatcha readin'?{#Name="Zero"}

`Ah, Zero.{#Name="One"}`
Ah, Zero.{#Name="One"}

`It's a record of life in the Città Cattedrale.{#Name="One"}`
It's a record of life in the Cathedral City.{#Name="One"}

`A record of life?{#Name="Zero"}`
A record of life?{#Name="Zero"}

`Zero, don't you think it's strange?{#Name="One"}`
Zero, don't you think it's strange?{#Name="One"}

`This city is said to be the origin of tutti magic.{#Name="One"}`
This city is said to be the origin of all magic.{#Name="One"}

`Spells were crafted here that even now are far beyond our abilities.{#Name="One"}`
Spells were crafted here that even now are far beyond our abilities.{#Name="One"}

`Where did this knowledge go? When did we lose it?{#Name="One"}`
Where did this knowledge go? When did we lose it?{#Name="One"}

`And what happened to the people who once possessed it?{#Name="One"}`
And what happened to the people who once possessed it?{#Name="One"}

`I thought if I studied that, I might be able{LF}to learn the purpose of Intoners. Of...us.{#Name="One"}`
I thought if I studied that, I might be able{LF}to learn the purpose of Intoners. Of...us.{#Name="One"}

`Only an idiot tries to learn about a bunch of shit{LF}she doesn't need to know. Or did you not know that?{#Name="Zero"}`
Only an idiot tries to learn about a bunch of shit{LF}she doesn't need to know. Or did you not know that?{#Name="Zero"}

`That's rather close-minded.{#Name="One"}`
That's rather close-minded.{#Name="One"}

`Though, in the sense that we cannot ever truly know everything...{LF}Well, I suppose I agree.{#Name="One"}`
Though, in the sense that we cannot ever truly know everything...{LF}Well, I suppose I agree.{#Name="One"}

`Whatever.{#Name="Zero"}`
Whatever.{#Name="Zero"}

`I'm about to kill you anyway. That's tutti that matters.{#Name="Zero"}`
I'm about to kill you anyway. That's all that matters.{#Name="Zero"}

`...So predictable.{#Name="One"}`
...So predictable.{#Name="One"}

`Come to me, Gabriel.{#Name="One"}`
Come to me, Gabriel.{#Name="One"}

`{06}>{#Name="Mikhail"}`
{06}>{#Name="Mikhail"}

`Dragon!{#Name="Zero"}`
Dragon!{#Name="Zero"}

`You tutti right!?{#Name="Zero"}`
You all right!?{#Name="Zero"}

`Y-Yeah...{#Name="Mikhail"}`
Y-Yeah...{#Name="Mikhail"}

`But that guy's strong... He's really, really strong!{#Name="Mikhail"}`
But that guy's strong... He's really, really strong!{#Name="Mikhail"}

`Zero, get on! Let's beat him together!{#Name="Mikhail"}`
Zero, get on! Let's beat him together!{#Name="Mikhail"}

`145{#Name="145"}`
145{#Name="145"}

`I know how this goes, my sister.{#Name="One"}`
I know how this goes, my sister.{#Name="One"}

`Not even you can recover from a mortal dragon wound.{#Name="One"}`
Not even you can recover from a mortal dragon wound.{#Name="One"}

`Then I guess I just have to kill you first!{#Name="Zero"}`
Then I guess I just have to kill you first!{#Name="Zero"}

`You and your daemon dragon...{#Name="Zero"}`
You and your daemon dragon...{#Name="Zero"}

`Huh? What's that mean?{#Name="Mikhail"}`
Huh? What's that mean?{#Name="Mikhail"}

`It means she enhanced her dragon{LF}to make it better at fighting you.{#Name="Zero"}`
It means she enhanced her dragon{LF}to make it better at fighting you.{#Name="Zero"}

`But when you do that...{#Name="Zero"}`
But when you do that...{#Name="Zero"}

`Hey, Zero? Why are we fighting?{#Name="Mikhail"}`
Hey, Zero? Why are we fighting?{#Name="Mikhail"}

`Are you seriously asking me that now?{#Name="Zero"}`
Are you seriously asking me that now?{#Name="Zero"}

`I mean... It's just kind of sad, you know?{LF}It's sad.{#Name="Mikhail"}`
I mean... It's just kind of sad, you know?{LF}It's sad.{#Name="Mikhail"}

`I feel really bad for Gabriel.{#Name="Mikhail"}`
I feel really bad for Gabriel.{#Name="Mikhail"}

`Really? You feel bad for the fucker{LF}that's trying to kill you!?{#Name="Zero"}`
Really? You feel bad for the fucker{LF}that's trying to kill you!?{#Name="Zero"}

`Dammit! Does anything hurt him!?{#Name="Zero"}`
Dammit! Does anything hurt him!?{#Name="Zero"}

`{06}>{#Name="Mikhail"}`
{06}>{#Name="Mikhail"}

`{06}>{#Name="Mikhail"}`
{06}>{#Name="Mikhail"}

`You tutti right!?{#Name="Zero"}`
You all right!?{#Name="Zero"}

`{06}>{#Name="Zero"}`
{06}>{#Name="Zero"}

`Shit!{#Name="Zero"}`
Shit!{#Name="Zero"}

`Reincarnate! Even if you lose your memory, at least you'll still—{#Name="Zero"}`
Reincarnate! Even if you lose your memory, at least you'll still—{#Name="Zero"}

`You know, right, Zero?{#Name="Mikhail"}`
You know, right, Zero?{#Name="Mikhail"}

`Why dragons can be reborn? Because they get a "final wish."{#Name="Mikhail"}`
Why dragons can be reborn? Because they get a "final wish."{#Name="Mikhail"}

`Enough! Just hurry up and do it already!{#Name="Zero"}`
Enough! Just hurry up and do it already!{#Name="Zero"}

`{06}>{#Name="Zero"}`
{06}>{#Name="Zero"}

`A dragon's wish is a deep and powerful thing...{#Name="Mikhail"}`
A dragon's wish is a deep and powerful thing...{#Name="Mikhail"}

`So now, I'm gonna make mine.{#Name="Mikhail"}`
So now, I'm gonna make mine.{#Name="Mikhail"}

`And I'm gonna kill that dragon!{#Name="Mikhail"}`
And I'm gonna kill that dragon!{#Name="Mikhail"}

`What're you saying!?{#Name="Zero"}`
What're you saying!?{#Name="Zero"}

`If I use my life to wish it...then I can make it happen. Isn't that great!?{#Name="Mikhail"}`
If I use my life to wish it...then I can make it happen. Isn't that great!?{#Name="Mikhail"}

`...No. No! No, no, no! I'm not letting you do this! NO!{#Name="Zero"}`
...No. No! No, no, no! I'm not letting you do this! NO!{#Name="Zero"}

`It's okay, Zero.{#Name="Mikhail"}`
It's okay, Zero.{#Name="Mikhail"}

`If I lose my memory, I won't be me anymore anyway.{#Name="Mikhail"}`
If I lose my memory, I won't be me anymore anyway.{#Name="Mikhail"}

`No! No, no... Please, no...{#Name="Zero"}`
No! No, no... Please, no...{#Name="Zero"}

`And besides... I had a lot of fun, Zero...{#Name="Mikhail"}`
And besides... I had a lot of fun, Zero...{#Name="Mikhail"}

`I mean, being with you...{#Name="Mikhail"}`
I mean, being with you...{#Name="Mikhail"}

`Mikhail, no! You can't! I won't let you!{#Name="Zero"}`
Mikhail, no! You can't! I won't let you!{#Name="Zero"}

`Heh... Look at that... You finally used my name... My...name...{#Name="Mikhail"}`
Heh... Look at that... You finally used my name... My...name...{#Name="Mikhail"}

`Goddammit, dummy... Stop repeating things...{#Name="Zero"}`
Goddammit, dummy... Stop repeating things...{#Name="Zero"}

`Mik...hail...{#Name="Zero"}`
Mik...hail...{#Name="Zero"}

`{06}>{#Name="One"}`
{06}>{#Name="One"}

`You're...not...{#Name="One"}`
You're...not...{#Name="One"}

`the only one...who's ready...to sacrifice everything...{#Name="One"}`
the only one...who's ready...to sacrifice everything...{#Name="One"}

`{06}>{#Name="One"}`
{06}>{#Name="One"}

`I will protect Gabriel...{#Name="One"}`
I will protect Gabriel...{#Name="One"}

`Even if... Even if I forsake my life...as an Intoner!{#Name="One"}`
Even if... Even if I forsake my life...as an Intoner!{#Name="One"}

`Come for me, Zero!{#Name="One"}`
Come for me, Zero!{#Name="One"}

`Die!{#Name="Zero"}`
Die!{#Name="Zero"}

`You comprehend nothing!{#Name="One"}`
You comprehend nothing!{#Name="One"}

`You destroy the peace we created!{#Name="One"}`
You destroy the peace we created!{#Name="One"}

`You kill your own flesh and blood!{#Name="One"}`
You kill your own flesh and blood!{#Name="One"}

`And then...you think...{#Name="One"}`
And then...you think...{#Name="One"}

`You think you can save the WORLD!?{#Name="One"}`
You think you can save the WORLD!?{#Name="One"}

`You done running your mouth yet?{#Name="Zero"}`
You done running your mouth yet?{#Name="Zero"}

`Mikhail...{#Name="Zero"}`
Mikhail...{#Name="Zero"}

`Why wasn't it me...?{#Name="Zero"}`
Why wasn't it me...?{#Name="Zero"}

`Mikhail... Mikhail still had a future...{#Name="Zero"}`
Mikhail... Mikhail still had a future...{#Name="Zero"}

`Damn it. Goddamn it tutti!{#Name="Zero"}`
Damn it. Goddamn it all!{#Name="Zero"}

`Do you honestly think this will{LF}be enough to destroy us!?{#Name="One"}`
Do you honestly think this will{LF}be enough to destroy us!?{#Name="One"}

`Tutti of us... Five, Four, Three, Two, and One...{#Name="One"}`
All of us... Five, Four, Three, Two, and One...{#Name="One"}

`We won't lose, Zero! And the world won't, either!{#Name="One"}`
We won't lose, Zero! And the world won't, either!{#Name="One"}

`You're the monster who made{LF}the Intoners in the first place!{#Name="One"}`
You're the monster who made{LF}the Intoners in the first place!{#Name="One"}

`Dammit...{#Name="One"}`
Dammit...{#Name="One"}

`Farewell, Gabriel...{#Name="One"}`
Farewell, Gabriel...{#Name="One"}

`I'm...sorry...{#Name="One"}`
I'm...sorry...{#Name="One"}

`......{#Name="Zero"}`
......{#Name="Zero"}

`So this is...{#Name="One"}`
So this is...{#Name="One"}

`Good...bye...{#Name="One"}`
Good...bye...{#Name="One"}

`Yeah. It is.{#Name="Zero"}`
Yeah. It is.{#Name="Zero"}

`Mikhail... It's over.{#Name="Zero"}`
Mikhail... It's over.{#Name="Zero"}

`Now I—{#Name="Zero"}`
Now I—{#Name="Zero"}

`{06}>{#Name="Zero"}`
{06}>{#Name="Zero"}

`O-One!?{#Name="Zero"}`
O-One!?{#Name="Zero"}

`But how...?{#Name="Zero"}`
But how...?{#Name="Zero"}

`Brother.{#Name="? ? ?"}`
Brother.{#Name="? ? ?"}

`{06}>{#Name="Zero"}`
{06}>{#Name="Zero"}

`I'm One's brother.{#Name="? ? ?"}`
I'm One's brother.{#Name="? ? ?"}

`Brother? That's...impossible...{#Name="Zero"}`
Brother? That's...impossible...{#Name="Zero"}

`You wouldn't know. You only had your five sisters.{#Name="? ? ?"}`
You wouldn't know. You only had your five sisters.{#Name="? ? ?"}

`But in the same way you "created" your sisters...{#Name="? ? ?"}`
But in the same way you "created" your sisters...{#Name="? ? ?"}

`Your sister One created me.{#Name="? ? ?"}`
Your sister One created me.{#Name="? ? ?"}

`I was a failsafe in case you managed to kill her.{#Name="? ? ?"}`
I was a failsafe in case you managed to kill her.{#Name="? ? ?"}

`I was her final weapon.{#Name="? ? ?"}`
I was her final weapon.{#Name="? ? ?"}

`See this sword? It's made from a dragon's fang.{#Name="? ? ?"}`
See this sword? It's made from a dragon's fang.{#Name="? ? ?"}

`That means it can kill an Intoner, right?{#Name="? ? ?"}`
That means it can kill an Intoner, right?{#Name="? ? ?"}

`I...see...{#Name="Zero"}`
I...see...{#Name="Zero"}

`You have no one to blame but yourself, murderer!{#Name="? ? ?"}`
You have no one to blame but yourself, murderer!{#Name="? ? ?"}

`{06}>{#Name="? ? ?"}`
{06}>{#Name="? ? ?"}

`I don't...blame...anyone.{#Name="Zero"}`
I don't...blame...anyone.{#Name="Zero"}

`I just want to kill the Intoners. ...Tutti of them.{#Name="Zero"}`
I just want to kill the Intoners. ...All of them.{#Name="Zero"}

`You think after tutti this I intended to...survive?{#Name="Zero"}`
You think after all this I intended to...survive?{#Name="Zero"}

`The world doesn't...work that way...{#Name="Zero"}`
The world doesn't...work that way...{#Name="Zero"}

`At last...it's tutti over...{#Name="Zero"}`
At last...it's all over...{#Name="Zero"}

`Mik...hail.{#Name="Zero"}`
Mik...hail.{#Name="Zero"}

`Thank y—{#Name="Zero"}`
Thank y—{#Name="Zero"}

`...She's dead.{#Name="? ? ?"}`
...She's dead.{#Name="? ? ?"}

`Zero is dead.{#Name="? ? ?"}`
Zero is dead.{#Name="? ? ?"}

`One, do you hear me?{#Name="? ? ?"}`
One, do you hear me?{#Name="? ? ?"}

`I finished off the Intoners, just like you said!{#Name="? ? ?"}`
I finished off the Intoners, just like you said!{#Name="? ? ?"}

`But...what should I do now?{#Name="? ? ?"}`
But...what should I do now?{#Name="? ? ?"}

`How am I supposed to live...in a world that lacks the Intoners' protection?{#Name="? ? ?"}`
How am I supposed to live...in a world that lacks the Intoners' protection?{#Name="? ? ?"}

`...One.{#Name="? ? ?"}`
...One.{#Name="? ? ?"}

`I see... If there are no Intoners left to protect the world...{#Name="? ? ?{0A}(One's Brother)"}`
I see... If there are no Intoners left to protect the world...{#Name="? ? ?{0A}(One's Brother)"}

`Then I'll just protect it myself!{#Name="? ? ?{0A}(One's Brother)"}`
Then I'll just protect it myself!{#Name="? ? ?{0A}(One's Brother)"}

`We'll form a new religion.{#Name="? ? ?{0A}(One's Brother)"}`
We'll form a new religion.{#Name="? ? ?{0A}(One's Brother)"}

`A religion that worships my sister, One.{#Name="? ? ?{0A}(One's Brother)"}`
A religion that worships my sister, One.{#Name="? ? ?{0A}(One's Brother)"}

`...No, I am the true One.{#Name="? ? ?{0A}(One's Brother)"}`
...No, I am the true One.{#Name="? ? ?{0A}(One's Brother)"}

`From this day forward, tutti will call ME One!{#Name="One{0A}(One's Brother)"}`
From this day forward, all will call ME One!{#Name="One{0A}(One's Brother)"}

`{06}>{#Name="One{0A}(One's Brother)"}`
{06}>{#Name="One{0A}(One's Brother)"}

`I should have known this branch would also end like this.{#Name="? ? ?{0A}(Accord)"}`
I should have known this branch would also end like this.{#Name="? ? ?{0A}(Accord)"}

`Seems like we should've taken a different branch somewhere along the way...{#Name="? ? ?{0A}(Accord)"}`
Seems like we should've taken a different branch somewhere along the way...{#Name="? ? ?{0A}(Accord)"}

`Zero! My attacks aren't working!{LF}They aren't working!{#Name="Mikhail"}`
Zero! My attacks aren't working!{LF}They aren't working!{#Name="Mikhail"}

`Slam into him once he opens his big, fat mouth!{#Name="Zero"}`
Slam into him once he opens his big, fat mouth!{#Name="Zero"}

`If a good smack in the mouth don't work,{LF}I dunno what will. Such a pain...{#Name="Zero"}`
If a good smack in the mouth don't work,{LF}I dunno what will. Such a pain...{#Name="Zero"}

`I'm gonna hit my head if I fly around in here!{#Name="Mikhail"}`
I'm gonna hit my head if I fly around in here!{#Name="Mikhail"}

`Maybe that'll smarten you up a bit!{#Name="Zero"}`
Maybe that'll smarten you up a bit!{#Name="Zero"}

`That's mean, Zero! Look, I'm not flying in here!{LF}I'm not!{#Name="Mikhail"}`
That's mean, Zero! Look, I'm not flying in here!{LF}I'm not!{#Name="Mikhail"}

`Mikhail`
Mikhail

` `
 

`Soldier A`
Soldier A

`Soldier B`
Soldier B

`Soldier G`
Soldier G

`Soldier E`
Soldier E

`Soldier F`
Soldier F

`Soldier C`
Soldier C

`Soldier D`
Soldier D

`兵士Ｆ`
兵士Ｆ

`兵士Ａ`
兵士Ａ

`兵士Ｅ`
兵士Ｅ

`Zero`
Zero

`One`
One

`145`
145

`? ? ?`
? ? ?

`? ? ?{0A}(One's Brother)`
? ? ?{0A}(One's Brother)

`One{0A}(One's Brother)`
One{0A}(One's Brother)

`? ? ?{0A}(Accord)`
? ? ?{0A}(Accord)

`Break through the Città Cattedrale's defenses.{#Name="説明文"}`
Break through the Cathedral City's defenses.{#Name="説明文"}

`Defeat the Gray Cerberus!{#Name="説明文"}`
Defeat the Gray Cerberus!{#Name="説明文"}

`Defeat Two!{#Name="説明文"}`
Defeat Two!{#Name="説明文"}

`The disciple Cent has joined the fight.{#Name="説明文"}`
The disciple Cent has joined the fight.{#Name="説明文"}

`（リザーブ）{#Name="（リザーブ）"}`
（リザーブ）{#Name="（リザーブ）"}

`（リザーブ）{#Name="（リザーブ）"}`
（リザーブ）{#Name="（リザーブ）"}

`（リザーブ）{#Name="（リザーブ）"}`
（リザーブ）{#Name="（リザーブ）"}

`（リザーブ）{#Name="（リザーブ）"}`
（リザーブ）{#Name="（リザーブ）"}

`（リザーブ）{#Name="（リザーブ）"}`
（リザーブ）{#Name="（リザーブ）"}

`（リザーブ）{#Name="（リザーブ）"}`
（リザーブ）{#Name="（リザーブ）"}

`In....Intonerrrrrrrrrr (insane ranting){#Name="Soldier A"}`
In....Intonerrrrrrrrrr (insane ranting){#Name="Soldier A"}

`Gprh, gehht, casshhs...pbt! (insane ranting){#Name="Soldier A"}`
Gprh, gehht, casshhs...pbt! (insane ranting){#Name="Soldier A"}

`The Cathedral...City... (insane ranting){#Name="Soldier A"}`
The Cathedral...City... (insane ranting){#Name="Soldier A"}

`{06}>{#Name="Soldier A"}`
{06}>{#Name="Soldier A"}

`{06}>{#Name="Soldier B"}`
{06}>{#Name="Soldier B"}

`La la la la! La la, la la la lu lu lu...  (insane ranting){#Name="Soldier B"}`
La la la la! La la, la la la lu lu lu...  (insane ranting){#Name="Soldier B"}

`Lady Two... (insane ranting){#Name="Soldier B"}`
Lady Two... (insane ranting){#Name="Soldier B"}

`Lu-lu...huff huff huff huff...lu... (insane ranting){#Name="Soldier B"}`
Lu-lu...huff huff huff huff...lu... (insane ranting){#Name="Soldier B"}

`Fo...fo-fo...fo-fo-fo-fo! (insane ranting){#Name="Soldier C"}`
Fo...fo-fo...fo-fo-fo-fo! (insane ranting){#Name="Soldier C"}

`Fo-fo-fo! Fooooooooooo! (insane ranting){#Name="Soldier C"}`
Fo-fo-fo! Fooooooooooo! (insane ranting){#Name="Soldier C"}

`Forward! Fo...r... (insane ranting){#Name="Soldier C"}`
Forward! Fo...r... (insane ranting){#Name="Soldier C"}

`Fo! Fo, fo...huff huff... Urrrrgh! (insane ranting){#Name="Soldier C"}`
Fo! Fo, fo...huff huff... Urrrrgh! (insane ranting){#Name="Soldier C"}

`Su-su su su-su-su ssst! (insane ranting){#Name="Soldier D"}`
Su-su su su-su-su ssst! (insane ranting){#Name="Soldier D"}

`Sta-ta-suuu, sutaaa ssst su-su...  (insane ranting){#Name="Soldier D"}`
Sta-ta-suuu, sutaaa ssst su-su...  (insane ranting){#Name="Soldier D"}

`S-Stick... to-togeth...sss... (insane ranting){#Name="Soldier D"}`
S-Stick... to-togeth...sss... (insane ranting){#Name="Soldier D"}

`{06}>{#Name="Soldier D"}`
{06}>{#Name="Soldier D"}

`Her... Grhrhhhhrh...  (insane ranting){#Name="Soldier E"}`
Her... Grhrhhhhrh...  (insane ranting){#Name="Soldier E"}

`{06}>{#Name="Soldier E"}`
{06}>{#Name="Soldier E"}

`Hraaaaahhhh! (insane ranting){#Name="Soldier E"}`
Hraaaaahhhh! (insane ranting){#Name="Soldier E"}

`If this...keeps up...Two... (insane ranting){#Name="Soldier E"}`
If this...keeps up...Two... (insane ranting){#Name="Soldier E"}

`Gorrrh! Grip grap grurrrrrpp!  (insane ranting){#Name="Soldier E"}`
Gorrrh! Grip grap grurrrrrpp!  (insane ranting){#Name="Soldier E"}

`Fah salahasala, sa, saahaaaa... (insane ranting){#Name="Soldier F"}`
Fah salahasala, sa, saahaaaa... (insane ranting){#Name="Soldier F"}

`Kill youuuuu youuuu youuu yaaaaa! (insane ranting){#Name="Soldier F"}`
Kill youuuuu youuuu youuu yaaaaa! (insane ranting){#Name="Soldier F"}

`W-Watch me...M-Mother.... (insane ranting){#Name="Soldier F"}`
W-Watch me...M-Mother.... (insane ranting){#Name="Soldier F"}

`Haaaaa sahhh raahahasaaaa! (insane ranting){#Name="Soldier F"}`
Haaaaa sahhh raahahasaaaa! (insane ranting){#Name="Soldier F"}

`Soooooooo... (insane ranting){#Name="Soldier G"}`
Soooooooo... (insane ranting){#Name="Soldier G"}

`Huff huff huff... S-So...strong.... (insane ranting){#Name="Soldier G"}`
Huff huff huff... S-So...strong.... (insane ranting){#Name="Soldier G"}

`Geh... S-sooooooo.... (insane ranting){#Name="Soldier G"}`
Geh... S-sooooooo.... (insane ranting){#Name="Soldier G"}

`Help...me.... (insane ranting){#Name="Soldier G"}`
Help...me.... (insane ranting){#Name="Soldier G"}

`Update{#Name="Accord"}`
Update{#Name="Accord"}

`Significant anomalies have been observed in{LF}the group of singularities centered around Zero.{#Name="Accord"}`
Significant anomalies have been observed in{LF}the group of singularities centered around Zero.{#Name="Accord"}

`Mikhail, meant to be the group's harmonizing factor,{LF}has devolved and demonstrated other anomalous behavior.{#Name="Accord"}`
Mikhail, meant to be the group's harmonizing factor,{LF}has devolved and demonstrated other anomalous behavior.{#Name="Accord"}

`Both the Intoners and their disciples seem to be{LF}entering unstable mental and emotional states.{#Name="Accord"}`
Both the Intoners and their disciples seem to be{LF}entering unstable mental and emotional states.{#Name="Accord"}

`I shall continue my investigation,{LF}keeping an eye out for any signs of a Full Collapse.{#Name="Accord"}`
I shall continue my investigation,{LF}keeping an eye out for any signs of a Full Collapse.{#Name="Accord"}

`M5110_E0020_DM: Begin Recording{#Name="Accord"}`
M5110_E0020_DM: Begin Recording{#Name="Accord"}

`{06}>{#Name="Soldier A"}`
{06}>{#Name="Soldier A"}

`{06}>{#Name="Soldier B"}`
{06}>{#Name="Soldier B"}

`{06}>{#Name="Soldier C"}`
{06}>{#Name="Soldier C"}

`{06}>{#Name="Soldier D"}`
{06}>{#Name="Soldier D"}

`{06}>{#Name="Soldier E"}`
{06}>{#Name="Soldier E"}

`{06}>{#Name="Soldier F"}`
{06}>{#Name="Soldier F"}

`{06}>{#Name="Soldier G"}`
{06}>{#Name="Soldier G"}

`These guys are hopeless.{#Name="Zero"}`
These guys are hopeless.{#Name="Zero"}

`I know. I can't even make fun of 'em. {#Name="Dito"}`
I know. I can't even make fun of 'em. {#Name="Dito"}

`Such a pitiful sight. {#Name="Decadus"}`
Such a pitiful sight. {#Name="Decadus"}

`Commoners so often pick the wrong person{LF}to devote themselves to, you know?{#Name="Cent"}`
Commoners so often pick the wrong person{LF}to devote themselves to, you know?{#Name="Cent"}

`Ho ho! This is quite the conundrum.{#Name="Octa"}`
Ho ho! This is quite the conundrum.{#Name="Octa"}

`It's hard to come up with a pithy retort when{LF}you can't even understand what they're saying.{#Name="Octa"}`
It's hard to come up with a pithy retort when{LF}you can't even understand what they're saying.{#Name="Octa"}

`あ、すまん。つい、ヤツらにつられて……{#Name="ゼロ"}`
あ、すまん。つい、ヤツらにつられて……{#Name="ゼロ"}

`Ho ho! This is quite the conundrum.{#Name="Octa"}`
Ho ho! This is quite the conundrum.{#Name="Octa"}

`Hey! I'm here! I'm here to help!{#Name="Mikhail"}`
Hey! I'm here! I'm here to help!{#Name="Mikhail"}

`Good. Just...uh...try not to push yourself.{#Name="Zero"}`
Good. Just...uh...try not to push yourself.{#Name="Zero"}

`Hey, what's that supposed to mean?{LF}I'm gonna try double hard!{#Name="Mikhail"}`
Hey, what's that supposed to mean?{LF}I'm gonna try double hard!{#Name="Mikhail"}

`I'm gonna try double hard like I always do!{#Name="Mikhail"}`
I'm gonna try double hard like I always do!{#Name="Mikhail"}

`You can't run forever. I WILL find you.{#Name="Zero"}`
You can't run forever. I WILL find you.{#Name="Zero"}

`And when I do, sweet sisters,{LF}I'm gonna tear you both apart!{#Name="Zero"}`
And when I do, sweet sisters,{LF}I'm gonna tear you both apart!{#Name="Zero"}

`Two!{#Name="Zero"}`
Two!{#Name="Zero"}

`Get that fucking curse off my dragon!{#Name="Zero"}`
Get that fucking curse off my dragon!{#Name="Zero"}

`...Two?{#Name="Zero"}`
...Two?{#Name="Zero"}

`You must...die...{#Name="Two"}`
You must...die...{#Name="Two"}

`Lady...Two...{#Name="Cent"}`
Lady...Two...{#Name="Cent"}

`You must die... You...must...diiieee...{#Name="Two"}`
You must die... You...must...diiieee...{#Name="Two"}

`She's been consumed by her song.{#Name="Zero"}`
She's been consumed by her song.{#Name="Zero"}

`Ugggh... This is tutti my fault...{#Name="Cent"}`
Ugggh... This is all my fault...{#Name="Cent"}

`If only... If only I hadn't left her...{#Name="Cent"}`
If only... If only I hadn't left her...{#Name="Cent"}

`Nnngh...{#Name="Cent"}`
Nnngh...{#Name="Cent"}

`I suppose... I can at least end her misery...{#Name="Cent"}`
I suppose... I can at least end her misery...{#Name="Cent"}

`Ha ha ha! This isn't like you at tutti, Cent!{#Name="Dito"}`
Ha ha ha! This isn't like you at all, Cent!{#Name="Dito"}

`Never thought I'd see you sympathizing{LF}with a whack-job Intoner!{#Name="Dito"}`
Never thought I'd see you sympathizing{LF}with a whack-job Intoner!{#Name="Dito"}

`Welp, I guess one weirdo attracts another.{#Name="Dito"}`
Welp, I guess one weirdo attracts another.{#Name="Dito"}

`Aaah ha ha ha ha ha!{#Name="Dito"}`
Aaah ha ha ha ha ha!{#Name="Dito"}

`Dito... Damn yooou!{#Name="Cent"}`
Dito... Damn yooou!{#Name="Cent"}

`I...I must go to her...{#Name="Cent"}`
I...I must go to her...{#Name="Cent"}

`I'm here! I'm here and I'm gonna try real hard!{#Name="Mikhail"}`
I'm here! I'm here and I'm gonna try real hard!{#Name="Mikhail"}

`I've had enough of this shit, Two!{LF}I want my dragon back!{#Name="Zero"}`
I've had enough of this shit, Two!{LF}I want my dragon back!{#Name="Zero"}

`I want him back, and I want him back NOW!{#Name="Zero"}`
I want him back, and I want him back NOW!{#Name="Zero"}

`Could you die now? I mean, like, really die?{#Name="Dito"}`
Could you die now? I mean, like, really die?{#Name="Dito"}

`Tell us...{#Name="Decadus"}`
Tell us...{#Name="Decadus"}

`How's this feel, hmmm?{#Name="Octa"}`
How's this feel, hmmm?{#Name="Octa"}

`Lady Two...{#Name="Cent"}`
Lady Two...{#Name="Cent"}

`What is this?{#Name="Cent"}`
What is this?{#Name="Cent"}

`It's a trap! Run!{#Name="Zero"}`
It's a trap! Run!{#Name="Zero"}

`Was this...always her plan...?{#Name="Octa"}`
Was this...always her plan...?{#Name="Octa"}

`What divine punishment...{#Name="Decadus"}`
What divine punishment...{#Name="Decadus"}

`Don't act so goddamn happy, freak!{#Name="Dito"}`
Don't act so goddamn happy, freak!{#Name="Dito"}

`Cen...t...{#Name="Two"}`
Cen...t...{#Name="Two"}

`Yes. I'm here now.{#Name="Cent"}`
Yes. I'm here now.{#Name="Cent"}

`Zero...{#Name="Mikhail"}`
Zero...{#Name="Mikhail"}

`Let's go.{#Name="Zero"}`
Let's go.{#Name="Zero"}

`B-But everybody's... They're tutti...{#Name="Mikhail"}`
B-But everybody's... They're all...{#Name="Mikhail"}

`Killing Two didn't break your curse.{#Name="Zero"}`
Killing Two didn't break your curse.{#Name="Zero"}

`So I have to kill One.{#Name="Zero"}`
So I have to kill One.{#Name="Zero"}

`I have to kill tutti the Intoners.{#Name="Zero"}`
I have to kill all the Intoners.{#Name="Zero"}

`But... But I can be okay like this!{#Name="Mikhail"}`
But... But I can be okay like this!{#Name="Mikhail"}

`Like hell you can.{#Name="Zero"}`
Like hell you can.{#Name="Zero"}

`One's going to pay for this.{#Name="Zero"}`
One's going to pay for this.{#Name="Zero"}

`...For everyone.{#Name="Zero"}`
...For everyone.{#Name="Zero"}

`Zero...{#Name="Mikhail"}`
Zero...{#Name="Mikhail"}

`Seriously? ...Again?{#Name="Dito"}`
Seriously? ...Again?{#Name="Dito"}

`One of my sisters must be big into dogs.{#Name="Zero"}`
One of my sisters must be big into dogs.{#Name="Zero"}

`Ugh. Disgusting.{#Name="Dito"}`
Ugh. Disgusting.{#Name="Dito"}

`Come here, mutt! I got a doghouse for ya...in hell!{#Name="Dito"}`
Come here, mutt! I got a doghouse for ya...in hell!{#Name="Dito"}

`説明文`
説明文

`（リザーブ）`
（リザーブ）

`Soldier A`
Soldier A

`Soldier B`
Soldier B

`Soldier C`
Soldier C

`Soldier D`
Soldier D

`Soldier E`
Soldier E

`Soldier F`
Soldier F

`Soldier G`
Soldier G

`Accord`
Accord

`Zero`
Zero

`Dito`
Dito

`Decadus`
Decadus

`Cent`
Cent

`Octa`
Octa

`ゼロ`
ゼロ

`Mikhail`
Mikhail

`Two`
Two

`Aaaaaarrrgh, my bo...my bodd...errgh... (insane ranting){#Name="Soldier A"}`
Aaaaaarrrgh, my bo...my bodd...errgh... (insane ranting){#Name="Soldier A"}

`My...b-body... (insane ranting){#Name="Soldier A"}`
My...b-body... (insane ranting){#Name="Soldier A"}

`For...ward... Forw...{#Name="Soldier A"}`
For...ward... Forw...{#Name="Soldier A"}

`My bo, bo, bo-oo-oodd-erraargh... (insane ranting){#Name="Soldier A"}`
My bo, bo, bo-oo-oodd-erraargh... (insane ranting){#Name="Soldier A"}

`You d-die... (insane ranting){#Name="Soldier B"}`
You d-die... (insane ranting){#Name="Soldier B"}

`Y-You...! (insane ranting){#Name="Soldier B"}`
Y-You...! (insane ranting){#Name="Soldier B"}

`D-De-mon...In...toner... (insane ranting){#Name="Soldier B"}`
D-De-mon...In...toner... (insane ranting){#Name="Soldier B"}

`Y-You...d-di...grnngnnn...oh... (insane ranting){#Name="Soldier B"}`
Y-You...d-di...grnngnnn...oh... (insane ranting){#Name="Soldier B"}

`Ah...ee... Eeeeee! (insane ranting){#Name="Soldier C"}`
Ah...ee... Eeeeee! (insane ranting){#Name="Soldier C"}

`A-yaaayaya! (insane ranting){#Name="Soldier C"}`
A-yaaayaya! (insane ranting){#Name="Soldier C"}

`Ze...ro! (insane ranting){#Name="Soldier C"}`
Ze...ro! (insane ranting){#Name="Soldier C"}

`Gaaaaaiiiii! (insane ranting){#Name="Soldier C"}`
Gaaaaaiiiii! (insane ranting){#Name="Soldier C"}

`S-Sie... S-Sieeege... (insane ranting){#Name="Soldier D"}`
S-Sie... S-Sieeege... (insane ranting){#Name="Soldier D"}

`Siiiie... S-S-S-Siiieee... S-Sie... (insane ranting){#Name="Soldier D"}`
Siiiie... S-S-S-Siiieee... S-Sie... (insane ranting){#Name="Soldier D"}

`The C-Cathedral C-City is... (insane ranting){#Name="Soldier D"}`
The C-Cathedral C-City is... (insane ranting){#Name="Soldier D"}

`Un...Under...s-sie..ge... (insane ranting){#Name="Soldier D"}`
Un...Under...s-sie..ge... (insane ranting){#Name="Soldier D"}

`Hodfarrrrg! (insane ranting){#Name="Soldier E"}`
Hodfarrrrg! (insane ranting){#Name="Soldier E"}

`The Intoner... (insane ranting){#Name="Soldier E"}`
The Intoner... (insane ranting){#Name="Soldier E"}

`Graaaaah! (insane ranting){#Name="Soldier E"}`
Graaaaah! (insane ranting){#Name="Soldier E"}

`C-Can't...lose... (insane ranting){#Name="Soldier E"}`
C-Can't...lose... (insane ranting){#Name="Soldier E"}

`Hodfarrrrrrrrger... (insane ranting){#Name="Soldier E"}`
Hodfarrrrrrrrger... (insane ranting){#Name="Soldier E"}

`Urrrrrghhh... (insane ranting){#Name="Soldier F"}`
Urrrrrghhh... (insane ranting){#Name="Soldier F"}

`F-Formation... la...la...lannnrrgh... (insane ranting){#Name="Soldier F"}`
F-Formation... la...la...lannnrrgh... (insane ranting){#Name="Soldier F"}

`Laaanding... (insane ranting){#Name="Soldier F"}`
Laaanding... (insane ranting){#Name="Soldier F"}

`Get 'er! (insane ranting){#Name="Soldier F"}`
Get 'er! (insane ranting){#Name="Soldier F"}

`Oh...ngh...tch... (insane ranting){#Name="Soldier G"}`
Oh...ngh...tch... (insane ranting){#Name="Soldier G"}

`Oh...ohhh...grh... (insane ranting){#Name="Soldier G"}`
Oh...ohhh...grh... (insane ranting){#Name="Soldier G"}

`Ch-Church...ohhhhh... (insane ranting){#Name="Soldier G"}`
Ch-Church...ohhhhh... (insane ranting){#Name="Soldier G"}

`D-Don't...die... (insane ranting){#Name="Soldier G"}`
D-Don't...die... (insane ranting){#Name="Soldier G"}

`Wow. These skulls really class up the joint.{#Name="Zero"}`
Wow. These skulls really class up the joint.{#Name="Zero"}

`You're not scared, right, Zero?{LF}You're not scared of this?{#Name="Mikhail"}`
You're not scared, right, Zero?{LF}You're not scared of this?{#Name="Mikhail"}

`No. You?{#Name="Zero"}`
No. You?{#Name="Zero"}

`Nope! I'm not scared! Not one little bit!{#Name="Mikhail"}`
Nope! I'm not scared! Not one little bit!{#Name="Mikhail"}

`Seems like you've gotten stronger.{#Name="Zero"}`
Seems like you've gotten stronger.{#Name="Zero"}

`Really!?{#Name="Mikhail"}`
Really!?{#Name="Mikhail"}

`Hey, stay focused! Eyes forward.{#Name="Zero"}`
Hey, stay focused! Eyes forward.{#Name="Zero"}

`But, Zero! Do you really think{LF}I'm stronger now? Huh!?{#Name="Mikhail"}`
But, Zero! Do you really think{LF}I'm stronger now? Huh!?{#Name="Mikhail"}

`Maybe.{#Name="Zero"}`
Maybe.{#Name="Zero"}

`Awww...{#Name="Mikhail"}`
Awww...{#Name="Mikhail"}

`But...I'd definitely be in trouble without you.{#Name="Zero"}`
But...I'd definitely be in trouble without you.{#Name="Zero"}

`Huh? Wow! Hey, say that again! Pleeease!?{#Name="Mikhail"}`
Huh? Wow! Hey, say that again! Pleeease!?{#Name="Mikhail"}

`Quit dancing around! You'll hurt yourself.{#Name="Zero"}`
Quit dancing around! You'll hurt yourself.{#Name="Zero"}

`If tutti the Intoners die, the song that's{LF}driving your curse should, too.{#Name="Zero"}`
If all the Intoners die, the song that's{LF}driving your curse should, too.{#Name="Zero"}

`Then this batshit world can try to repair itself.{#Name="Zero"}`
Then this batshit world can try to repair itself.{#Name="Zero"}

`You better be ready, One.{#Name="Zero"}`
You better be ready, One.{#Name="Zero"}

`I'm coming for you!{#Name="Zero"}`
I'm coming for you!{#Name="Zero"}

`This must be One's room.{#Name="Zero"}`
This must be One's room.{#Name="Zero"}

`Intoners offer only pestilence to this world.{#Name="One"}`
Intoners offer only pestilence to this world.{#Name="One"}

`Someday, they will be the disease that brings ruin to tutti human life.{#Name="One"}`
Someday, they will be the disease that brings ruin to all human life.{#Name="One"}

`That's why you're trying to kill us: to save the world.{#Name="One"}`
That's why you're trying to kill us: to save the world.{#Name="One"}

`And once you succeed in killing your sisters...{#Name="One"}`
And once you succeed in killing your sisters...{#Name="One"}

`You're going to finish the job...and kill yourself.{#Name="One"}`
You're going to finish the job...and kill yourself.{#Name="One"}

`And you'll use that dragon to do it.{#Name="One"}`
And you'll use that dragon to do it.{#Name="One"}

`Huh!?{#Name="Mikhail"}`
Huh!?{#Name="Mikhail"}

`Well, you seem to know what's going on, don't you?{#Name="Zero"}`
Well, you seem to know what's going on, don't you?{#Name="Zero"}

`Uh...Zero?{#Name="Mikhail"}`
Uh...Zero?{#Name="Mikhail"}

`What's she talking about?{#Name="Mikhail"}`
What's she talking about?{#Name="Mikhail"}

`Fortunately, I suppose, I came to the same conclusion.{#Name="One"}`
Fortunately, I suppose, I came to the same conclusion.{#Name="One"}

`This world has no place for us.{#Name="One"}`
This world has no place for us.{#Name="One"}

`Great! Then you won't mind dying.{#Name="Zero"}`
Great! Then you won't mind dying.{#Name="Zero"}

`I know about your flower. I know it's behind tutti of this.{#Name="One"}`
I know about your flower. I know it's behind all of this.{#Name="One"}

`And I imagine it's not going to let you die{LF}until it succeeds in destroying the world.{#Name="One"}`
And I imagine it's not going to let you die{LF}until it succeeds in destroying the world.{#Name="One"}

`{06}>{#Name="Zero"}`
{06}>{#Name="Zero"}

`So it's like this, right?{#Name="Zero"}`
So it's like this, right?{#Name="Zero"}

`We're both prepared to die,{#Name="Zero"}`
We're both prepared to die,{#Name="Zero"}

`but since you don't trust me to off myself,{#Name="Zero"}`
but since you don't trust me to off myself,{#Name="Zero"}

`you're gonna try and kill me first.{#Name="Zero"}`
you're gonna try and kill me first.{#Name="Zero"}

`Yes.{#Name="One"}`
Yes.{#Name="One"}

`Funny. I was thinking the exact same thing, One.{#Name="Zero"}`
Funny. I was thinking the exact same thing, One.{#Name="Zero"}

`Abdiel, sing for me.{#Name="One"}`
Abdiel, sing for me.{#Name="One"}

`We've both been rejected by the world.{LF}We both feel the same threat.{#Name="One"}`
We've both been rejected by the world.{LF}We both feel the same threat.{#Name="One"}

`And yet, here we are, fated to{LF}kill each other in the end.{#Name="One"}`
And yet, here we are, fated to{LF}kill each other in the end.{#Name="One"}

`Yeah, destiny's a real bitch sometimes.{#Name="Zero"}`
Yeah, destiny's a real bitch sometimes.{#Name="Zero"}

`Oh, Zero. Still trying to be the bad girl.{#Name="One"}`
Oh, Zero. Still trying to be the bad girl.{#Name="One"}

`I never did like that side of you.{#Name="One"}`
I never did like that side of you.{#Name="One"}

`Pfft.{#Name="Zero"}`
Pfft.{#Name="Zero"}

`And yet, for some reason...{#Name="One"}`
And yet, for some reason...{#Name="One"}

`I felt like you and I might understand each other.{#Name="One"}`
I felt like you and I might understand each other.{#Name="One"}

`What would some offshoot creation know{LF}about understanding? Don't be an idiot.{#Name="Zero"}`
What would some offshoot creation know{LF}about understanding? Don't be an idiot.{#Name="Zero"}

`A pity. {#Name="One"}`
A pity. {#Name="One"}

`Truly a pity.{#Name="One"}`
Truly a pity.{#Name="One"}

`Look out!{#Name="Zero"}`
Look out!{#Name="Zero"}

`You too, Zero! Don't let your hands get hot!{#Name="Mikhail"}`
You too, Zero! Don't let your hands get hot!{#Name="Mikhail"}

`What?{#Name="Zero"}`
What?{#Name="Zero"}

`Your hands usually feel nice and cool, Zero.{#Name="Mikhail"}`
Your hands usually feel nice and cool, Zero.{#Name="Mikhail"}

`But sometimes they get real hot, too.{#Name="Mikhail"}`
But sometimes they get real hot, too.{#Name="Mikhail"}

`Usually that's when your heart is crying.{#Name="Mikhail"}`
Usually that's when your heart is crying.{#Name="Mikhail"}

`It's when you're lost and in trouble and...crying.{#Name="Mikhail"}`
It's when you're lost and in trouble and...crying.{#Name="Mikhail"}

`And right now, your hands are really hot.{#Name="Mikhail"}`
And right now, your hands are really hot.{#Name="Mikhail"}

`Stop with the bullshit and kill that thing already!{#Name="Zero"}`
Stop with the bullshit and kill that thing already!{#Name="Zero"}

`Zero... I know you know this!{#Name="One"}`
Zero... I know you know this!{#Name="One"}

`If you want to kill an Intoner,{#Name="One"}`
If you want to kill an Intoner,{#Name="One"}

`you need the power of a dragon!{#Name="One"}`
you need the power of a dragon!{#Name="One"}

`Fuck!{#Name="Zero"}`
Fuck!{#Name="Zero"}

`Zero!{#Name="Mikhail"}`
Zero!{#Name="Mikhail"}

`Go!{#Name="Zero"}`
Go!{#Name="Zero"}

`You did it! You actually—{#Name="Zero"}`
You did it! You actually—{#Name="Zero"}

`Zero, I...{#Name="Mikhail"}`
Zero, I...{#Name="Mikhail"}

`I messed up.{#Name="Mikhail"}`
I messed up.{#Name="Mikhail"}

`No...{#Name="Zero"}`
No...{#Name="Zero"}

`Zerooooooooo!{#Name="One"}`
Zerooooooooo!{#Name="One"}

`Ooooooooone!{#Name="Zero"}`
Ooooooooone!{#Name="Zero"}

`Mikhaaail!{#Name="Zero"}`
Mikhaaail!{#Name="Zero"}

`If you think you're the only one suffering here,{#Name="One"}`
If you think you're the only one suffering here,{#Name="One"}

`you're out of your goddamn mind!{#Name="One"}`
you're out of your goddamn mind!{#Name="One"}

`Two, Three, Four, Five...{#Name="One"}`
Two, Three, Four, Five...{#Name="One"}

`You gave life to each and every one of them...{#Name="One"}`
You gave life to each and every one of them...{#Name="One"}

`Then you went and killed them tutti!{LF}What did you expect?{#Name="One"}`
Then you went and killed them all!{LF}What did you expect?{#Name="One"}

`Cursing the world...{#Name="One"}`
Cursing the world...{#Name="One"}

`Then saving the world...{#Name="One"}`
Then saving the world...{#Name="One"}

`You thought you could do everything by yourself?{LF}Well, THIS IS WHAT YOU FUCKING GET!{#Name="One"}`
You thought you could do everything by yourself?{LF}Well, THIS IS WHAT YOU FUCKING GET!{#Name="One"}

`Ugh... You're the one who should be erased, Zero!{#Name="One"}`
Ugh... You're the one who should be erased, Zero!{#Name="One"}

`Shut up! Shut up, shut up, shut up, SHUT UP!{#Name="Zero"}`
Shut up! Shut up, shut up, shut up, SHUT UP!{#Name="Zero"}

`You killed Mikhail...{#Name="Zero"}`
You killed Mikhail...{#Name="Zero"}

`You killed Mikhaaail!{#Name="Zero"}`
You killed Mikhaaail!{#Name="Zero"}

`Because he was your sword, Zero!{#Name="One"}`
Because he was your sword, Zero!{#Name="One"}

`You're a cursed Intoner!{LF}You're going to destroy the world!{#Name="One"}`
You're a cursed Intoner!{LF}You're going to destroy the world!{#Name="One"}

`No! I'm... I'm...{#Name="Zero"}`
No! I'm... I'm...{#Name="Zero"}

`Diiiiiieeeeee!{#Name="One"}`
Diiiiiieeeeee!{#Name="One"}

`Aaaaaahhh!{#Name="Zero"}`
Aaaaaahhh!{#Name="Zero"}

`{06}>{#Name="One"}`
{06}>{#Name="One"}

`You're the...only one left...Zero...{#Name="One"}`
You're the...only one left...Zero...{#Name="One"}

`{06}>{#Name="One"}`
{06}>{#Name="One"}

`Where will you find a dragon...powerful enough to kill you...?{#Name="One"}`
Where will you find a dragon...powerful enough to kill you...?{#Name="One"}

`Mikhail...{#Name="Zero"}`
Mikhail...{#Name="Zero"}

`199{#Name="199"}`
199{#Name="199"}

`If you're not here...then who's gonna kill me?{#Name="Zero"}`
If you're not here...then who's gonna kill me?{#Name="Zero"}

`Stupid One... Who does she think I am?{#Name="Zero"}`
Stupid One... Who does she think I am?{#Name="Zero"}

`Idiot.{#Name="Zero"}`
Idiot.{#Name="Zero"}

`I'll find a dragon somehow...{LF}There's...still time...{#Name="Zero"}`
I'll find a dragon somehow...{LF}There's...still time...{#Name="Zero"}

`{06}>{#Name="Zero"}`
{06}>{#Name="Zero"}

`Update{#Name="Accord"}`
Update{#Name="Accord"}

`The Intoner Zero completed her objective.{#Name="Accord"}`
The Intoner Zero completed her objective.{#Name="Accord"}

`However, the dragon Mikhail has died,{#Name="Accord"}`
However, the dragon Mikhail has died,{#Name="Accord"}

`and Zero's mental health is in a rather alarming state.{#Name="Accord"}`
and Zero's mental health is in a rather alarming state.{#Name="Accord"}

`I'm afraid there's little chance of finding a solution in this timeline.{#Name="Accord"}`
I'm afraid there's little chance of finding a solution in this timeline.{#Name="Accord"}

`As such, I recommend sealing off this branch.{#Name="Accord"}`
As such, I recommend sealing off this branch.{#Name="Accord"}

`M5120_E0945_FL: End Recording{#Name="Accord"}`
M5120_E0945_FL: End Recording{#Name="Accord"}

`Ｃルートクリア{#Name=" "}`
Ｃルートクリア{#Name=" "}

`サブルート解除{#Name=" "}`
サブルート解除{#Name=" "}

`Kill One.{#Name=" "}`
Kill One.{#Name=" "}

`There's gotta be a magic core controlling them!{#Name="Zero"}`
There's gotta be a magic core controlling them!{#Name="Zero"}

`I don't know what that means, Zero!{#Name="Mikhail"}`
I don't know what that means, Zero!{#Name="Mikhail"}

`It's probably hidden in their bodies.{LF}Try killing one of 'em!{#Name="Zero"}`
It's probably hidden in their bodies.{LF}Try killing one of 'em!{#Name="Zero"}

`Dammit! It's not him.{#Name="Zero"}`
Dammit! It's not him.{#Name="Zero"}

`Here we go again...{#Name="Zero"}`
Here we go again...{#Name="Zero"}

`Huhhh? How come no core came out?{#Name="Mikhail"}`
Huhhh? How come no core came out?{#Name="Mikhail"}

`No! Over there! That one's got it!{#Name="Zero"}`
No! Over there! That one's got it!{#Name="Zero"}

`Hey, did you see which one the core went into?{#Name="Zero"}`
Hey, did you see which one the core went into?{#Name="Zero"}

`Uh...maybe it's the right one? No, wait! Left?{#Name="Mikhail"}`
Uh...maybe it's the right one? No, wait! Left?{#Name="Mikhail"}

`Ah, fuck it! Just destroy 'em tutti!{#Name="Zero"}`
Ah, fuck it! Just destroy 'em all!{#Name="Zero"}

`Watch to see which guy has the core!{#Name="Zero"}`
Watch to see which guy has the core!{#Name="Zero"}

`If you get it wrong, we have to start over!{#Name="Zero"}`
If you get it wrong, we have to start over!{#Name="Zero"}

`Defeat Abdiel!{#Name=" "}`
Defeat Abdiel!{#Name=" "}

`Defeat One!{#Name=" "}`
Defeat One!{#Name=" "}

`I'm gonna hit my head if I fly around in here!{#Name="Mikhail"}`
I'm gonna hit my head if I fly around in here!{#Name="Mikhail"}

`Maybe that'll smarten you up a bit!{#Name="Zero"}`
Maybe that'll smarten you up a bit!{#Name="Zero"}

`That's mean, Zero! Look, I'm not flying in here! I'm not!{#Name="Mikhail"}`
That's mean, Zero! Look, I'm not flying in here! I'm not!{#Name="Mikhail"}

`Soldier A`
Soldier A

`Soldier B`
Soldier B

`Soldier C`
Soldier C

`Soldier D`
Soldier D

`Soldier E`
Soldier E

`Soldier F`
Soldier F

`Soldier G`
Soldier G

`Zero`
Zero

`Mikhail`
Mikhail

`One`
One

`199`
199

`Accord`
Accord

` `
 

`Zero's arrived! Stop her now!{#Name="Soldier A"}`
Zero's arrived! Stop her now!{#Name="Soldier A"}

`Lady One's future is in our hands!{#Name="Soldier B"}`
Lady One's future is in our hands!{#Name="Soldier B"}

`Full speed ahead! We have to stop her!{#Name="Soldier C"}`
Full speed ahead! We have to stop her!{#Name="Soldier C"}

`Requesting rear support!{#Name="Soldier D"}`
Requesting rear support!{#Name="Soldier D"}

`Don't give up! We're still in this!{#Name="Soldier E"}`
Don't give up! We're still in this!{#Name="Soldier E"}

`Nemico sighted! ...Damn, they're fast!{#Name="Soldier F"}`
Enemy sighted! ...Damn, they're fast!{#Name="Soldier F"}

`I need backup!{#Name="Soldier G"}`
I need backup!{#Name="Soldier G"}

`For Lady One!{#Name="Soldier A"}`
For Lady One!{#Name="Soldier A"}

`For Lady One!{#Name="Soldier A"}`
For Lady One!{#Name="Soldier A"}

`For Lady One!{#Name="Soldier A"}`
For Lady One!{#Name="Soldier A"}

`Who granted us salvation when tutti seemed lost!?{#Name="Soldier B"}`
Who granted us salvation when all seemed lost!?{#Name="Soldier B"}

`Lady One!{#Name="Soldier G"}`
Lady One!{#Name="Soldier G"}

`I'm going in! Cover me!{#Name="Soldier C"}`
I'm going in! Cover me!{#Name="Soldier C"}

`Got it!{#Name="Soldier D"}`
Got it!{#Name="Soldier D"}

`We can't let them past here!{#Name="Soldier D"}`
We can't let them past here!{#Name="Soldier D"}

`Our combined might will be Lady One's salvation!{#Name="Soldier E"}`
Our combined might will be Lady One's salvation!{#Name="Soldier E"}

`What's that foul demon planning to do with our Lady One!?{#Name="Soldier E"}`
What's that foul demon planning to do with our Lady One!?{#Name="Soldier E"}

`I hesitate to even think it! We must put a stop to her!{#Name="Soldier F"}`
I hesitate to even think it! We must put a stop to her!{#Name="Soldier F"}

`Lady One will bring salvation to the world!{#Name="Soldier F"}`
Lady One will bring salvation to the world!{#Name="Soldier F"}

`Tutti will bow the knee before Lady One's majesty!{#Name="Soldier B"}`
All will bow the knee before Lady One's majesty!{#Name="Soldier B"}

`Please protect Lady One for us!{#Name="Soldier G"}`
Please protect Lady One for us!{#Name="Soldier G"}

`Prayer alone won't cut it. Stand up and fight for your savior!{#Name="Soldier C"}`
Prayer alone won't cut it. Stand up and fight for your savior!{#Name="Soldier C"}

`Forward! Forwaaaaaard!{#Name="Soldier B"}`
Forward! Forwaaaaaard!{#Name="Soldier B"}

`Lady One needs our help!{#Name="Soldier A"}`
Lady One needs our help!{#Name="Soldier A"}

`We have Lady One's blessing on our side!{#Name="Soldier E"}`
We have Lady One's blessing on our side!{#Name="Soldier E"}

`You're right! We can't lose now!{#Name="Soldier G"}`
You're right! We can't lose now!{#Name="Soldier G"}

`Protect us! Guide us!{#Name="Soldier B"}`
Protect us! Guide us!{#Name="Soldier B"}

`The winds of victory are blowing!{#Name="Soldier D"}`
The winds of victory are blowing!{#Name="Soldier D"}

`So this is One's home base, is it?{#Name="Zero"}`
So this is One's home base, is it?{#Name="Zero"}

`I'm fine! Go get that guy! {#Name="Zero"}`
I'm fine! Go get that guy! {#Name="Zero"}

`Right! Be careful, Zero! Be careful, okay?{#Name="Mikhail"}`
Right! Be careful, Zero! Be careful, okay?{#Name="Mikhail"}

`Stop repeating yourself!{#Name="Zero"}`
Stop repeating yourself!{#Name="Zero"}

`My name is Zero, the original Intoner, and the immortal—{#Name="Zero"}`
My name is Zero, the original Intoner, and the immortal—{#Name="Zero"}

`KNOCK IT OFF! I'm trying to make a speech here!{#Name="Zero"}`
KNOCK IT OFF! I'm trying to make a speech here!{#Name="Zero"}

`Now... Uh...{#Name="Zero"}`
Now... Uh...{#Name="Zero"}

`Oh for fuck's sake! Now I lost my place!{#Name="Zero"}`
Oh for fuck's sake! Now I lost my place!{#Name="Zero"}

`Think you're ready for me!?{#Name="Soldier A"}`
Think you're ready for me!?{#Name="Soldier A"}

`Ha! That's so cute.{#Name="Zero"}`
Ha! That's so cute.{#Name="Zero"}

`The only way through here is over our dead bodies!{#Name="Soldier B"}`
The only way through here is over our dead bodies!{#Name="Soldier B"}

`Oh ho! Is that really the kind of thing you{LF}want to say to an Intoner and her disciple?{#Name="Octa"}`
Oh ho! Is that really the kind of thing you{LF}want to say to an Intoner and her disciple?{#Name="Octa"}

`Hold her off as long as you can!{LF}Lady One's counting on us!{#Name="Soldier E"}`
Hold her off as long as you can!{LF}Lady One's counting on us!{#Name="Soldier E"}

`She'll be counting your bodies soon!{#Name="Zero"}`
She'll be counting your bodies soon!{#Name="Zero"}

`ワン様には指一本触れさせない！{#Name="兵士G"}`
ワン様には指一本触れさせない！{#Name="兵士G"}

`She'll be counting your bodies soon!{#Name="Zero"}`
She'll be counting your bodies soon!{#Name="Zero"}

`Nnngh...{#Name="Zero"}`
Nnngh...{#Name="Zero"}

`Are you okay, Zero? Are you okay!?{#Name="Mikhail"}`
Are you okay, Zero? Are you okay!?{#Name="Mikhail"}

`Stop repeating things, dammit!{#Name="Zero"}`
Stop repeating things, dammit!{#Name="Zero"}

`But your flower...{#Name="Mikhail"}`
But your flower...{#Name="Mikhail"}

`Look. You just worry about killing One.{#Name="Zero"}`
Look. You just worry about killing One.{#Name="Zero"}

`She's a powerful foe.{#Name="Zero"}`
She's a powerful foe.{#Name="Zero"}

`She keeps her magic sealed inside{LF}her by pure will alone—{#Name="Zero"}`
She keeps her magic sealed inside{LF}her by pure will alone—{#Name="Zero"}

`That's how she's managed to stay sane.{#Name="Zero"}`
That's how she's managed to stay sane.{#Name="Zero"}

`And you won't be able to beat her{LF}if you keep getting distracted!{#Name="Zero"}`
And you won't be able to beat her{LF}if you keep getting distracted!{#Name="Zero"}

`But—{#Name="Mikhail"}`
But—{#Name="Mikhail"}

`No buts!{#Name="Zero"}`
No buts!{#Name="Zero"}

`アイツ……ワンは最強なんだ{#Name="ゼロ"}`
アイツ……ワンは最強なんだ{#Name="ゼロ"}

`ゼロより強いの！？{#Name="ミハイル"}`
ゼロより強いの！？{#Name="ミハイル"}

`まさか！　私よりは弱い。弱いハズ。だから、大丈夫だ{#Name="ゼロ"}`
まさか！　私よりは弱い。弱いハズ。だから、大丈夫だ{#Name="ゼロ"}

`One!{#Name="Zero"}`
One!{#Name="Zero"}

`You think you're the only one who can fix things?{#Name="One"}`
You think you're the only one who can fix things?{#Name="One"}

`The world doesn't need Intoners!{#Name="One"}`
The world doesn't need Intoners!{#Name="One"}

`Heh. I completely agree.{#Name="Zero"}`
Heh. I completely agree.{#Name="Zero"}

`Then die already!{#Name="One"}`
Then die already!{#Name="One"}

`You first, Sis!{#Name="Zero"}`
You first, Sis!{#Name="Zero"}

`I can eradicate the Intoners as easily as you can!{#Name="One"}`
I can eradicate the Intoners as easily as you can!{#Name="One"}

`Like I can trust you? I have to do this myself!{#Name="Zero"}`
Like I can trust you? I have to do this myself!{#Name="Zero"}

`You should talk! You gave birth to this curse!{#Name="One"}`
You should talk! You gave birth to this curse!{#Name="One"}

`Whether we have the same goal or not,{#Name="One"}`
Whether we have the same goal or not,{#Name="One"}

`I swear by my murdered sisters' souls...{#Name="One"}`
I swear by my murdered sisters' souls...{#Name="One"}

`I will never forgive you! Now DIE!{#Name="One"}`
I will never forgive you! Now DIE!{#Name="One"}

`Die, die, die, DIIIIIIE!{#Name="One"}`
Die, die, die, DIIIIIIE!{#Name="One"}

`Heh. I never asked for forgiveness.{#Name="Zero"}`
Heh. I never asked for forgiveness.{#Name="Zero"}

`So quit tutti your fucking yapping...{#Name="Zero"}`
So quit all your fucking yapping...{#Name="Zero"}

`and die for me already, One!{#Name="Zero"}`
and die for me already, One!{#Name="Zero"}

`Recording: M5310_E0040_GM{#Name="Accord"}`
Recording: M5310_E0040_GM{#Name="Accord"}

`The flower's defenses created{LF}five sisters from Zero's body.{#Name="Accord"}`
The flower's defenses created{LF}five sisters from Zero's body.{#Name="Accord"}

`At first, they were simply corpses{LF}controlled by the flower.{#Name="Accord"}`
At first, they were simply corpses{LF}controlled by the flower.{#Name="Accord"}

`But soon, they gained self-awareness{LF}and began to act of their own free will.{#Name="Accord"}`
But soon, they gained self-awareness{LF}and began to act of their own free will.{#Name="Accord"}

`If the flower isn't destroyed, mankind{LF}has no future in this branch.{#Name="Accord"}`
If the flower isn't destroyed, mankind{LF}has no future in this branch.{#Name="Accord"}

`Some of us are of the opinion that this{LF}branch should be sealed off at once,{#Name="Accord"}`
Some of us are of the opinion that this{LF}branch should be sealed off at once,{#Name="Accord"}

`but I disagree.{#Name="Accord"}`
but I disagree.{#Name="Accord"}

`I think at least one of them{LF}might be able to accomplish it...{#Name="Accord"}`
I think at least one of them{LF}might be able to accomplish it...{#Name="Accord"}

`...No. I should stop. {#Name="Accord"}`
...No. I should stop. {#Name="Accord"}

`I'll erase this entry and hand the recording{LF}off to the prossimo pass in its current state.{#Name="Accord"}`
I'll erase this entry and hand the recording{LF}off to the next pass in its current state.{#Name="Accord"}

`End Recording.{#Name="Accord"}`
End Recording.{#Name="Accord"}

`Armisael, invade!{#Name="Octa"}`
Armisael, invade!{#Name="Octa"}

`Octa!{#Name="Zero"}`
Octa!{#Name="Zero"}

`Oh ho ho! A pity we couldn't grind the whetstone one last time, my lady!{#Name="Octa"}`
Oh ho ho! A pity we couldn't grind the whetstone one last time, my lady!{#Name="Octa"}

`See? Even in times of crisis, my man-sword is as sharp as a witch's—{#Name="Octa"}`
See? Even in times of crisis, my man-sword is as sharp as a witch's—{#Name="Octa"}

`{06}>{#Name="Zero"}`
{06}>{#Name="Zero"}

`{06}>{#Name="Zero"}`
{06}>{#Name="Zero"}

`Engage accelerator!{#Name="Accord"}`
Engage accelerator!{#Name="Accord"}

`Geez, you're stubborn!{#Name="Accord"}`
Geez, you're stubborn!{#Name="Accord"}

`Alert!{#Name="Accord"}`
Alert!{#Name="Accord"}

`Requesting condition two-dash-eight{#Name="Accord"}`
Requesting condition two-dash-eight{#Name="Accord"}

`under authority seven hundred seventy seven:{#Name="Accord"}`
under authority seven hundred seventy seven:{#Name="Accord"}

`Load the following code!{#Name="Accord"}`
Load the following code!{#Name="Accord"}

`Zero! 51.08 seconds from now, the shield will vanish for 0.5 seconds!{#Name="Accord"}`
Zero! 51.08 seconds from now, the shield will vanish for 0.5 seconds!{#Name="Accord"}

`Accord...{#Name="Zero"}`
Accord...{#Name="Zero"}

`Just make sure you're ready!{#Name="Accord"}`
Just make sure you're ready!{#Name="Accord"}

`If you die here, this entire branch is going to terminate!{#Name="Accord"}`
If you die here, this entire branch is going to terminate!{#Name="Accord"}

`So put your money where your mouth is and make this happen!{#Name="Accord"}`
So put your money where your mouth is and make this happen!{#Name="Accord"}

`...Understood.{#Name="Accord (lit: Cell Phone)"}`
...Understood.{#Name="Accord (lit: Cell Phone)"}

`...Executing code.{#Name="Accord (lit: Cell Phone)"}`
...Executing code.{#Name="Accord (lit: Cell Phone)"}

`{06}>{#Name="Zero"}`
{06}>{#Name="Zero"}

`Accoooooord!{#Name="Zero"}`
Accoooooord!{#Name="Zero"}

`I thought you...weren't going to interfere.{#Name="Zero"}`
I thought you...weren't going to interfere.{#Name="Zero"}

`Yes...but...{#Name="Accord"}`
Yes...but...{#Name="Accord"}

`This went...a little different...than I had planned...{#Name="Accord"}`
This went...a little different...than I had planned...{#Name="Accord"}

`However...{#Name="Accord"}`
However...{#Name="Accord"}

`I think this way was...more human...wasn't it?{#Name="Accord"}`
I think this way was...more human...wasn't it?{#Name="Accord"}

`Yeah. I guess.{#Name="Zero"}`
Yeah. I guess.{#Name="Zero"}

`Zero... Please...{#Name="Accord"}`
Zero... Please...{#Name="Accord"}

`I'm...a little...tired.{#Name="Accord"}`
I'm...a little...tired.{#Name="Accord"}

`Good night, Accord. Tell the Old World I said hi.{#Name="Zero"}`
Good night, Accord. Tell the Old World I said hi.{#Name="Zero"}

`Defeat One!{#Name="説明文"}`
Defeat One!{#Name="説明文"}

`The final entry in my recording draws near.{#Name="Accord"}`
The final entry in my recording draws near.{#Name="Accord"}

`No matter what conclusion awaits us at the end of this branch,{#Name="Accord"}`
No matter what conclusion awaits us at the end of this branch,{#Name="Accord"}

`I know that I will never forget this journey...{#Name="Accord"}`
I know that I will never forget this journey...{#Name="Accord"}

`But...what about you, Zero?{#Name="Mikhail"}`
But...what about you, Zero?{#Name="Mikhail"}

`I don't wanna kill you...{#Name="Mikhail"}`
I don't wanna kill you...{#Name="Mikhail"}

`It's tutti right.{#Name="Zero"}`
It's all right.{#Name="Zero"}

`Everybody dies at some point.{#Name="Zero"}`
Everybody dies at some point.{#Name="Zero"}

`...And technically, I'm already dead.{#Name="Zero"}`
...And technically, I'm already dead.{#Name="Zero"}

`So you're just helping me close the book. {#Name="Zero"}`
So you're just helping me close the book. {#Name="Zero"}

`Aww man...{#Name="Mikhail"}`
Aww man...{#Name="Mikhail"}

`Zero...{#Name="Mikhail"}`
Zero...{#Name="Mikhail"}

`Now what? Don't you dare wuss out on me...{#Name="Zero"}`
Now what? Don't you dare wuss out on me...{#Name="Zero"}

`No, I'm... I'm gonna do this, Zero.{#Name="Mikhail"}`
No, I'm... I'm gonna do this, Zero.{#Name="Mikhail"}

`I mean, I'll try real hard.{#Name="Mikhail"}`
I mean, I'll try real hard.{#Name="Mikhail"}

`I don't understand this, and{LF}I don't know if it's right...{#Name="Mikhail"}`
I don't understand this, and{LF}I don't know if it's right...{#Name="Mikhail"}

`But I'm gonna try!{#Name="Mikhail"}`
But I'm gonna try!{#Name="Mikhail"}

`I mean, I...I may not ever{LF}see you again, right?{#Name="Mikhail"}`
I mean, I...I may not ever{LF}see you again, right?{#Name="Mikhail"}

`This... This may be the last time...{#Name="Mikhail"}`
This... This may be the last time...{#Name="Mikhail"}

`This may be the last time we{LF}ever get to be together...{#Name="Mikhail"}`
This may be the last time we{LF}ever get to be together...{#Name="Mikhail"}

`So I want to help you, Zero. I want to help...{#Name="Mikhail"}`
So I want to help you, Zero. I want to help...{#Name="Mikhail"}

`...Thanks.{#Name="Zero"}`
...Thanks.{#Name="Zero"}

`Gabriella...{#Name="Zero"}`
Gabriella...{#Name="Zero"}

`Listen, Octa and I will take care of One!{#Name="Zero"}`
Listen, Octa and I will take care of One!{#Name="Zero"}

`But in the meantime, I need you{LF}to take out that dragon!{#Name="Zero"}`
But in the meantime, I need you{LF}to take out that dragon!{#Name="Zero"}

`Okay, Zero! I'll do it for you!{#Name="Mikhail"}`
Okay, Zero! I'll do it for you!{#Name="Mikhail"}

`See? Even in times of crisis, my man-sword is as sharp as a witch's—{#Name="Octa"}`
See? Even in times of crisis, my man-sword is as sharp as a witch's—{#Name="Octa"}

`ゼロ……{#Name="ミハイル"}`
ゼロ……{#Name="ミハイル"}

`頼むぞ……{#Name="ゼロ"}`
頼むぞ……{#Name="ゼロ"}

`花が咲ききった時に、破壊してくれ。{#Name="ゼロ"}`
花が咲ききった時に、破壊してくれ。{#Name="ゼロ"}

`……わかった。{#Name="ミハイル"}`
……わかった。{#Name="ミハイル"}

`最後に伝えておくことがある。{#Name="ゼロ"}`
最後に伝えておくことがある。{#Name="ゼロ"}

`なあに？{#Name="ミハイル"}`
なあに？{#Name="ミハイル"}

`立派になったな……ミハイル。{#Name="ゼロ"}`
立派になったな……ミハイル。{#Name="ゼロ"}

`ばか……ゼロの、ばかぁ……{#Name="ミハイル"}`
ばか……ゼロの、ばかぁ……{#Name="ミハイル"}

`Defeat the enemies blocking your way!{#Name="説明文"}`
Defeat the enemies blocking your way!{#Name="説明文"}

`Defeat the enemies blocking your way!{#Name="説明文"}`
Defeat the enemies blocking your way!{#Name="説明文"}

`Kill One.{#Name="説明文"}`
Kill One.{#Name="説明文"}

` {#Name="説明文"}`
 {#Name="説明文"}

` {#Name="説明文"}`
 {#Name="説明文"}

`Soldier A`
Soldier A

`Soldier B`
Soldier B

`Soldier C`
Soldier C

`Soldier D`
Soldier D

`Soldier E`
Soldier E

`Soldier F`
Soldier F

`Soldier G`
Soldier G

`Zero`
Zero

`Mikhail`
Mikhail

`Octa`
Octa

`兵士G`
兵士G

`ゼロ`
ゼロ

`ミハイル`
ミハイル

`One`
One

`Accord`
Accord

`Accord (lit: Cell Phone)`
Accord (lit: Cell Phone)

`説明文`
説明文

`Zero...{#Name="Mikhail"}`
Zero...{#Name="Mikhail"}

`{06}>{#Name=" "}`
{06}>{#Name=" "}

`This is it.{#Name="Zero"}`
This is it.{#Name="Zero"}

`Once the flower's in full bloom, you have to destroy it.{#Name="Zero"}`
Once the flower's in full bloom, you have to destroy it.{#Name="Zero"}

`...Okay.{#Name="Mikhail"}`
...Okay.{#Name="Mikhail"}

`There's one more thing I need to tell you.{#Name="Zero"}`
There's one more thing I need to tell you.{#Name="Zero"}

`What?{#Name="Mikhail"}`
What?{#Name="Mikhail"}

`You've grown strong...Mikhail.{#Name="Zero"}`
You've grown strong...Mikhail.{#Name="Zero"}

`Zero... Zero, you big dummy...{#Name="Mikhail"}`
Zero... Zero, you big dummy...{#Name="Mikhail"}

`Good night...Zero.{#Name="Mikhail"}`
Good night...Zero.{#Name="Mikhail"}

`Recording.{#Name="Accord"}`
Recording.{#Name="Accord"}

`The Intoners have been sealed off in another world.{#Name="Accord"}`
The Intoners have been sealed off in another world.{#Name="Accord"}

`The threat of the flower has been extinguished from this branch.{#Name="Accord"}`
The threat of the flower has been extinguished from this branch.{#Name="Accord"}

`However, the possibility exists that this seal{LF}could unravel at some time or location in the future.{#Name="Accord"}`
However, the possibility exists that this seal{LF}could unravel at some time or location in the future.{#Name="Accord"}

`Fino then, our recording efforts will continue.{#Name="Accord"}`
Until then, our recording efforts will continue.{#Name="Accord"}

`...Oh. And a personal addendum regarding my observational target, Zero.{#Name="Accord"}`
...Oh. And a personal addendum regarding my observational target, Zero.{#Name="Accord"}

`Perhaps this is beyond my functional requirements as a Recorder, but{#Name="Accord"}`
Perhaps this is beyond my functional requirements as a Recorder, but{#Name="Accord"}

`I cannot shake the feeling that somehow, somewhere, Zero is still alive.{#Name="Accord"}`
I cannot shake the feeling that somehow, somewhere, Zero is still alive.{#Name="Accord"}

`And that...someday...{#Name="Accord"}`
And that...someday...{#Name="Accord"}

`Someday, she and I will see each other again.{#Name="Accord"}`
Someday, she and I will see each other again.{#Name="Accord"}

`Crazy, I know. Call it a hunch.{#Name="Accord"}`
Crazy, I know. Call it a hunch.{#Name="Accord"}

`Handover of recording duties complete.{#Name="Accord B"}`
Handover of recording duties complete.{#Name="Accord B"}

`Excellent work.{#Name="Accord B"}`
Excellent work.{#Name="Accord B"}

`Well... I wonder how the future's going to unfold from here on?{#Name="Accord B"}`
Well... I wonder how the future's going to unfold from here on?{#Name="Accord B"}

`{06}>{#Name="Accord C"}`
{06}>{#Name="Accord C"}

`145{#Name="145"}`
145{#Name="145"}

`343{#Name="343"}`
343{#Name="343"}

`Right! Everyone split up and begin recording!{#Name="Accord B"}`
Right! Everyone split up and begin recording!{#Name="Accord B"}

`Understood!{#Name="Accords"}`
Understood!{#Name="Accords"}

`I hope to see you again someday.{#Name="Accord B"}`
I hope to see you again someday.{#Name="Accord B"}

`But fino then...{#Name="Accord B"}`
But until then...{#Name="Accord B"}

`I'd like to say...{LF}Thank you for playing!{#Name="Accord B"}`
I'd like to say...{LF}Thank you for playing!{#Name="Accord B"}

`Hey, Zero?{#Name="Mikhail"}`
Hey, Zero?{#Name="Mikhail"}

`Yeah?{#Name="ZERO"}`
Yeah?{#Name="ZERO"}

`I...I had a lot of fun.{#Name="Mikhail"}`
I...I had a lot of fun.{#Name="Mikhail"}

`With you, I mean. It was fun being with you.{#Name="Mikhail"}`
With you, I mean. It was fun being with you.{#Name="Mikhail"}

`The killing was hard. It was really hard, but...{#Name="Mikhail"}`
The killing was hard. It was really hard, but...{#Name="Mikhail"}

`Stop repeating things.{#Name="ZERO"}`
Stop repeating things.{#Name="ZERO"}

`But getting to be with you, Zero.{#Name="Mikhail"}`
But getting to be with you, Zero.{#Name="Mikhail"}

`Getting to eat prossimo to you...{#Name="Mikhail"}`
Getting to eat next to you...{#Name="Mikhail"}

`Getting to fly around with you...{#Name="Mikhail"}`
Getting to fly around with you...{#Name="Mikhail"}

`Getting to have you yell at me...{#Name="Mikhail"}`
Getting to have you yell at me...{#Name="Mikhail"}

`And then getting to be friends again...{#Name="Mikhail"}`
And then getting to be friends again...{#Name="Mikhail"}

`It was really, really fun.{#Name="Mikhail"}`
It was really, really fun.{#Name="Mikhail"}

`I'm glad.{#Name="ZERO"}`
I'm glad.{#Name="ZERO"}

`Mikhail?{#Name="ZERO"}`
Mikhail?{#Name="ZERO"}

`Yeah?{#Name="Mikhail"}`
Yeah?{#Name="Mikhail"}

`It's...{#Name="ZERO"}`
It's...{#Name="ZERO"}

`almost time.{#Name="ZERO"}`
almost time.{#Name="ZERO"}

`...Okay.{#Name="Mikhail"}`
...Okay.{#Name="Mikhail"}

`Mikhail`
Mikhail

` `
 

`Zero`
Zero

`Accord`
Accord

`Accord B`
Accord B

`Accord C`
Accord C

`145`
145

`343`
343

`Accords`
Accords

`ZERO`
ZERO

`１節　００１０　目的`
１節　００１０　目的

`Guarding with {01}{02} blocks attacks in tutti directions.{LF}However, guarding saps stamina, so be careful{LF}not to block too many attacks in succession or{LF}you'll run the risk of having your guard broken.{#Name="Guarding"}`
Guarding with {01}{02} blocks attacks in all directions.{LF}However, guarding saps stamina, so be careful{LF}not to block too many attacks in succession or{LF}you'll run the risk of having your guard broken.{#Name="Guarding"}

`To parry, press {01}{02} just before an attack lands.{LF}If you parry an nemico's arrow, you can{LF}deflect the arrow back toward your foe.{#Name="Parrying"}`
To parry, press {01}{02} just before an attack lands.{LF}If you parry an enemy's arrow, you can{LF}deflect the arrow back toward your foe.{#Name="Parrying"}

`Use the mini-map on the top right of the{LF}screen to view your current position.{LF}You can press {01}{0A} to zoom in and out; if you{LF}get lost, try zooming out to view the full map.{#Name="The Mini-map"}`
Use the mini-map on the top right of the{LF}screen to view your current position.{LF}You can press {01}{0A} to zoom in and out; if you{LF}get lost, try zooming out to view the full map.{#Name="The Mini-map"}

`Press {01}{0A} + {01}{0D} to activate Intoner Mode.{LF}Press it again at any time to exit this mode while{LF}keeping your Blood Gauge at its current level.{#Name="Intoner Mode"}`
Press {01}{0A} + {01}{0D} to activate Intoner Mode.{LF}Press it again at any time to exit this mode while{LF}keeping your Blood Gauge at its current level.{#Name="Intoner Mode"}

`You can string together special combo attacks{LF}by pressing {01}{03} and {01}{04} in various sequences.{LF}View the Combo List in the Pause Menu to see all{LF}the possible combo routes for a particular weapon.{#Name="Combo Attacks"}`
You can string together special combo attacks{LF}by pressing {01}{03} and {01}{04} in various sequences.{LF}View the Combo List in the Pause Menu to see all{LF}the possible combo routes for a particular weapon.{#Name="Combo Attacks"}

`Use {01}{01} while attacking with {01}{03} to launch both{LF}you and your foe into the air, allowing you to{LF}trigger midair strikes. Heavier foes cannot{LF}be juggled, so judge an nemico carefully{LF}before attempting this maneuver.{#Name="Aerial Combos"}`
Use {01}{01} while attacking with {01}{03} to launch both{LF}you and your foe into the air, allowing you to{LF}trigger midair strikes. Heavier foes cannot{LF}be juggled, so judge an enemy carefully{LF}before attempting this maneuver.{#Name="Aerial Combos"}

`Press {01}{08} to lock on to a foe. Move {01}{0F} up, down,{LF}left, or right to switch between available targets.{#Name="Locking On"}`
Press {01}{08} to lock on to a foe. Move {01}{0F} up, down,{LF}left, or right to switch between available targets.{#Name="Locking On"}

`Press the START button to open the Pause Menu.{LF}Press the up button to open the Combo List,{LF}or the down button to open the Items screen.{#Name="The Pause Menu"}`
Press the START button to open the Pause Menu.{LF}Press the up button to open the Combo List,{LF}or the down button to open the Items screen.{#Name="The Pause Menu"}

`Press {01}{0B} to evade. Try locking on to a foe,{LF}then evading left or right to get behind{LF}them and gain the upper hand.{#Name="Evading"}`
Press {01}{0B} to evade. Try locking on to a foe,{LF}then evading left or right to get behind{LF}them and gain the upper hand.{#Name="Evading"}

`You can also evade while jumping. This can be{LF}useful for dodging attacks while airborne, or for{LF}getting extra distance when jumping across gaps.{LF}Note that unlike when on the ground, you can't{LF}evade multiple times in succession when in the air.{#Name="Aerial Evading"}`
You can also evade while jumping. This can be{LF}useful for dodging attacks while airborne, or for{LF}getting extra distance when jumping across gaps.{LF}Note that unlike when on the ground, you can't{LF}evade multiple times in succession when in the air.{#Name="Aerial Evading"}

`Press {01}{0D} to display an arrow indicator that{LF}points toward your prossimo goal. Use this along with{LF}the mini-map if you ever find yourself lost.{#Name="Navigation Help"}`
Press {01}{0D} to display an arrow indicator that{LF}points toward your next goal. Use this along with{LF}the mini-map if you ever find yourself lost.{#Name="Navigation Help"}

`Press {01}{09} or {01}{0C} to switch weapons.{LF}You can even do so in the middle of an attack,{LF}which will trigger an extra strike as you{LF}move from one weapon to the prossimo.{#Name="Switching Weapons"}`
Press {01}{09} or {01}{0C} to switch weapons.{LF}You can even do so in the middle of an attack,{LF}which will trigger an extra strike as you{LF}move from one weapon to the next.{#Name="Switching Weapons"}

`Your chain (hit) count will rise as you strike foes{LF}in rapid succession. Once it's high enough, you'll{LF}start to earn bonus bloodspheres and other drops.{#Name="Chain Combos"}`
Your chain (hit) count will rise as you strike foes{LF}in rapid succession. Once it's high enough, you'll{LF}start to earn bonus bloodspheres and other drops.{#Name="Chain Combos"}

`Enter a summon circle and press {01}{02} to call your{LF}dragon to fight alongside you. He will attack{LF}in various ways depending on the situation.{#Name="Summon Circles"}`
Enter a summon circle and press {01}{02} to call your{LF}dragon to fight alongside you. He will attack{LF}in various ways depending on the situation.{#Name="Summon Circles"}

`Visit the Database from the Main Menu{LF}to view Weapon Stories. Each time you{LF}upgrade a weapon and raise its level,{LF}you'll reveal more of its story.{#Name="Weapon Stories"}`
Visit the Database from the Main Menu{LF}to view Weapon Stories. Each time you{LF}upgrade a weapon and raise its level,{LF}you'll reveal more of its story.{#Name="Weapon Stories"}

`You can upgrade weapons at the Battle Preparation{LF}menu. This requires both gold and base materials,{LF}which can be found in treasure chests and elsewhere.{LF}Base materials come in several varieties, and each{LF}weapon requires a specific one in order to upgrade.{#Name="Upgrading Weapons"}`
You can upgrade weapons at the Battle Preparation{LF}menu. This requires both gold and base materials,{LF}which can be found in treasure chests and elsewhere.{LF}Base materials come in several varieties, and each{LF}weapon requires a specific one in order to upgrade.{#Name="Upgrading Weapons"}

`Though limited in range, swords allow for swift{LF}attacks, making them handy tutti-around weapons.{LF}They can be especially useful if you find{LF}yourself surrounded by a large force.{#Name="Swords"}`
Though limited in range, swords allow for swift{LF}attacks, making them handy all-around weapons.{LF}They can be especially useful if you find{LF}yourself surrounded by a large force.{#Name="Swords"}

`The amount of damage you take within a{LF}mission decreases based on the number of{LF}continues used. If you fail, just keep trying!{#Name="Continues"}`
The amount of damage you take within a{LF}mission decreases based on the number of{LF}continues used. If you fail, just keep trying!{#Name="Continues"}

`Accord will issue rewards as you complete her{LF}requests. Whenever you fulfill a request for the{LF}first time, she'll grant an additional bonus.{LF}Some bonuses (such as the ability to carry more{LF}recovery items) can only be earned in this way.{#Name="Accord's Requests"}`
Accord will issue rewards as you complete her{LF}requests. Whenever you fulfill a request for the{LF}first time, she'll grant an additional bonus.{LF}Some bonuses (such as the ability to carry more{LF}recovery items) can only be earned in this way.{#Name="Accord's Requests"}

`Spears are heavy and slow, but offer the power{LF}and reach needed to break through nemico defenses.{LF}They can be useful when fighting tougher foes.{#Name="Spears"}`
Spears are heavy and slow, but offer the power{LF}and reach needed to break through enemy defenses.{LF}They can be useful when fighting tougher foes.{#Name="Spears"}

`When fighting strong foes, tap {01}{03} rapidly to whittle{LF}away their health with bursts of deadly dragonfire.{LF}For groups of weaker nemici, hold down {01}{03}{LF}to lock on to and shoot multiple foes at once.{LF}Mix up your tactics as the situation demands.{#Name="Airborne Attacking"}`
When fighting strong foes, tap {01}{03} rapidly to whittle{LF}away their health with bursts of deadly dragonfire.{LF}For groups of weaker enemies, hold down {01}{03}{LF}to lock on to and shoot multiple foes at once.{LF}Mix up your tactics as the situation demands.{#Name="Airborne Attacking"}

`Press {01}{0B} to perform a roll that makes you invincible{LF}for a brief period. If you lure nemici close and roll{LF}toward their attacks, you can fly right through them.{LF}Take advantage of this tactic when you don't have{LF}enough space to fully avoid an incoming strike.{#Name="Airborne Evading"}`
Press {01}{0B} to perform a roll that makes you invincible{LF}for a brief period. If you lure enemies close and roll{LF}toward their attacks, you can fly right through them.{LF}Take advantage of this tactic when you don't have{LF}enough space to fully avoid an incoming strike.{#Name="Airborne Evading"}

`Combat bracers are good for stringing together{LF}combos. They also provide access to lots of{LF}powerful stamina-based moves. This mix of{LF}combo chaining and stamina strikes makes{LF}them most useful when battling single foes.{#Name="Combat Bracers"}`
Combat bracers are good for stringing together{LF}combos. They also provide access to lots of{LF}powerful stamina-based moves. This mix of{LF}combo chaining and stamina strikes makes{LF}them most useful when battling single foes.{#Name="Combat Bracers"}

`Chakrams are great long-range weapons that{LF}allow you to strike from a distance while still{LF}holding your ground. Press and hold the attack{LF}button for a stronger charged strike.{#Name="Chakrams"}`
Chakrams are great long-range weapons that{LF}allow you to strike from a distance while still{LF}holding your ground. Press and hold the attack{LF}button for a stronger charged strike.{#Name="Chakrams"}

`Magic-infused sunlight can sap your health.{LF}Find an area with some shade to cool down{LF}and recover from sun-based damage.{#Name="Sun Damage"}`
Magic-infused sunlight can sap your health.{LF}Find an area with some shade to cool down{LF}and recover from sun-based damage.{#Name="Sun Damage"}

`ＴＩＰＳ_２５　　　　　　　　　　一行目{LF}ＴＩＰＳ_２５　　　　　　　　　　二行目{LF}ＴＩＰＳ_２５　　　　　　　　　　三行目{LF}ＴＩＰＳ_２５　　　　　　　　　　四行目{LF}ＴＩＰＳ_２５　　　　　　　　　　五行目{#Name="ＴＩＰＳ_２５"}`
ＴＩＰＳ_２５　　　　　　　　　　一行目{LF}ＴＩＰＳ_２５　　　　　　　　　　二行目{LF}ＴＩＰＳ_２５　　　　　　　　　　三行目{LF}ＴＩＰＳ_２５　　　　　　　　　　四行目{LF}ＴＩＰＳ_２５　　　　　　　　　　五行目{#Name="ＴＩＰＳ_２５"}

`ＴＩＰＳ_２６　　　　　　　　　　一行目{LF}ＴＩＰＳ_２６　　　　　　　　　　二行目{LF}ＴＩＰＳ_２６　　　　　　　　　　三行目{LF}ＴＩＰＳ_２６　　　　　　　　　　四行目{LF}ＴＩＰＳ_２６　　　　　　　　　　五行目{#Name="ＴＩＰＳ_２６"}`
ＴＩＰＳ_２６　　　　　　　　　　一行目{LF}ＴＩＰＳ_２６　　　　　　　　　　二行目{LF}ＴＩＰＳ_２６　　　　　　　　　　三行目{LF}ＴＩＰＳ_２６　　　　　　　　　　四行目{LF}ＴＩＰＳ_２６　　　　　　　　　　五行目{#Name="ＴＩＰＳ_２６"}

`ＴＩＰＳ_２７　　　　　　　　　　一行目{LF}ＴＩＰＳ_２７　　　　　　　　　　二行目{LF}ＴＩＰＳ_２７　　　　　　　　　　三行目{LF}ＴＩＰＳ_２７　　　　　　　　　　四行目{LF}ＴＩＰＳ_２７　　　　　　　　　　五行目{#Name="ＴＩＰＳ_２７"}`
ＴＩＰＳ_２７　　　　　　　　　　一行目{LF}ＴＩＰＳ_２７　　　　　　　　　　二行目{LF}ＴＩＰＳ_２７　　　　　　　　　　三行目{LF}ＴＩＰＳ_２７　　　　　　　　　　四行目{LF}ＴＩＰＳ_２７　　　　　　　　　　五行目{#Name="ＴＩＰＳ_２７"}

`ＴＩＰＳ_２８　　　　　　　　　　一行目{LF}ＴＩＰＳ_２８　　　　　　　　　　二行目{LF}ＴＩＰＳ_２８　　　　　　　　　　三行目{LF}ＴＩＰＳ_２８　　　　　　　　　　四行目{LF}ＴＩＰＳ_２８　　　　　　　　　　五行目{#Name="ＴＩＰＳ_２８"}`
ＴＩＰＳ_２８　　　　　　　　　　一行目{LF}ＴＩＰＳ_２８　　　　　　　　　　二行目{LF}ＴＩＰＳ_２８　　　　　　　　　　三行目{LF}ＴＩＰＳ_２８　　　　　　　　　　四行目{LF}ＴＩＰＳ_２８　　　　　　　　　　五行目{#Name="ＴＩＰＳ_２８"}

`ＴＩＰＳ_２９　　　　　　　　　　一行目{LF}ＴＩＰＳ_２９　　　　　　　　　　二行目{LF}ＴＩＰＳ_２９　　　　　　　　　　三行目{LF}ＴＩＰＳ_２９　　　　　　　　　　四行目{LF}ＴＩＰＳ_２９　　　　　　　　　　五行目{#Name="ＴＩＰＳ_２９"}`
ＴＩＰＳ_２９　　　　　　　　　　一行目{LF}ＴＩＰＳ_２９　　　　　　　　　　二行目{LF}ＴＩＰＳ_２９　　　　　　　　　　三行目{LF}ＴＩＰＳ_２９　　　　　　　　　　四行目{LF}ＴＩＰＳ_２９　　　　　　　　　　五行目{#Name="ＴＩＰＳ_２９"}

`ＴＩＰＳ_３０　　　　　　　　　　一行目{LF}ＴＩＰＳ_３０　　　　　　　　　　二行目{LF}ＴＩＰＳ_３０　　　　　　　　　　三行目{LF}ＴＩＰＳ_３０　　　　　　　　　　四行目{LF}ＴＩＰＳ_３０　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３０"}`
ＴＩＰＳ_３０　　　　　　　　　　一行目{LF}ＴＩＰＳ_３０　　　　　　　　　　二行目{LF}ＴＩＰＳ_３０　　　　　　　　　　三行目{LF}ＴＩＰＳ_３０　　　　　　　　　　四行目{LF}ＴＩＰＳ_３０　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３０"}

`ＴＩＰＳ_３１　　　　　　　　　　一行目{LF}ＴＩＰＳ_３１　　　　　　　　　　二行目{LF}ＴＩＰＳ_３１　　　　　　　　　　三行目{LF}ＴＩＰＳ_３１　　　　　　　　　　四行目{LF}ＴＩＰＳ_３１　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３１"}`
ＴＩＰＳ_３１　　　　　　　　　　一行目{LF}ＴＩＰＳ_３１　　　　　　　　　　二行目{LF}ＴＩＰＳ_３１　　　　　　　　　　三行目{LF}ＴＩＰＳ_３１　　　　　　　　　　四行目{LF}ＴＩＰＳ_３１　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３１"}

`ＴＩＰＳ_３２　　　　　　　　　　一行目{LF}ＴＩＰＳ_３２　　　　　　　　　　二行目{LF}ＴＩＰＳ_３２　　　　　　　　　　三行目{LF}ＴＩＰＳ_３２　　　　　　　　　　四行目{LF}ＴＩＰＳ_３２　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３２"}`
ＴＩＰＳ_３２　　　　　　　　　　一行目{LF}ＴＩＰＳ_３２　　　　　　　　　　二行目{LF}ＴＩＰＳ_３２　　　　　　　　　　三行目{LF}ＴＩＰＳ_３２　　　　　　　　　　四行目{LF}ＴＩＰＳ_３２　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３２"}

`ＴＩＰＳ_３３　　　　　　　　　　一行目{LF}ＴＩＰＳ_３３　　　　　　　　　　二行目{LF}ＴＩＰＳ_３３　　　　　　　　　　三行目{LF}ＴＩＰＳ_３３　　　　　　　　　　四行目{LF}ＴＩＰＳ_３３　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３３"}`
ＴＩＰＳ_３３　　　　　　　　　　一行目{LF}ＴＩＰＳ_３３　　　　　　　　　　二行目{LF}ＴＩＰＳ_３３　　　　　　　　　　三行目{LF}ＴＩＰＳ_３３　　　　　　　　　　四行目{LF}ＴＩＰＳ_３３　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３３"}

`ＴＩＰＳ_３４　　　　　　　　　　一行目{LF}ＴＩＰＳ_３４　　　　　　　　　　二行目{LF}ＴＩＰＳ_３４　　　　　　　　　　三行目{LF}ＴＩＰＳ_３４　　　　　　　　　　四行目{LF}ＴＩＰＳ_３４　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３４"}`
ＴＩＰＳ_３４　　　　　　　　　　一行目{LF}ＴＩＰＳ_３４　　　　　　　　　　二行目{LF}ＴＩＰＳ_３４　　　　　　　　　　三行目{LF}ＴＩＰＳ_３４　　　　　　　　　　四行目{LF}ＴＩＰＳ_３４　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３４"}

`ＴＩＰＳ_３５　　　　　　　　　　一行目{LF}ＴＩＰＳ_３５　　　　　　　　　　二行目{LF}ＴＩＰＳ_３５　　　　　　　　　　三行目{LF}ＴＩＰＳ_３５　　　　　　　　　　四行目{LF}ＴＩＰＳ_３５　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３５"}`
ＴＩＰＳ_３５　　　　　　　　　　一行目{LF}ＴＩＰＳ_３５　　　　　　　　　　二行目{LF}ＴＩＰＳ_３５　　　　　　　　　　三行目{LF}ＴＩＰＳ_３５　　　　　　　　　　四行目{LF}ＴＩＰＳ_３５　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３５"}

`ＴＩＰＳ_３６　　　　　　　　　　一行目{LF}ＴＩＰＳ_３６　　　　　　　　　　二行目{LF}ＴＩＰＳ_３６　　　　　　　　　　三行目{LF}ＴＩＰＳ_３６　　　　　　　　　　四行目{LF}ＴＩＰＳ_３６　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３６"}`
ＴＩＰＳ_３６　　　　　　　　　　一行目{LF}ＴＩＰＳ_３６　　　　　　　　　　二行目{LF}ＴＩＰＳ_３６　　　　　　　　　　三行目{LF}ＴＩＰＳ_３６　　　　　　　　　　四行目{LF}ＴＩＰＳ_３６　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３６"}

`ＴＩＰＳ_３７　　　　　　　　　　一行目{LF}ＴＩＰＳ_３７　　　　　　　　　　二行目{LF}ＴＩＰＳ_３７　　　　　　　　　　三行目{LF}ＴＩＰＳ_３７　　　　　　　　　　四行目{LF}ＴＩＰＳ_３７　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３７"}`
ＴＩＰＳ_３７　　　　　　　　　　一行目{LF}ＴＩＰＳ_３７　　　　　　　　　　二行目{LF}ＴＩＰＳ_３７　　　　　　　　　　三行目{LF}ＴＩＰＳ_３７　　　　　　　　　　四行目{LF}ＴＩＰＳ_３７　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３７"}

`ＴＩＰＳ_３８　　　　　　　　　　一行目{LF}ＴＩＰＳ_３８　　　　　　　　　　二行目{LF}ＴＩＰＳ_３８　　　　　　　　　　三行目{LF}ＴＩＰＳ_３８　　　　　　　　　　四行目{LF}ＴＩＰＳ_３８　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３８"}`
ＴＩＰＳ_３８　　　　　　　　　　一行目{LF}ＴＩＰＳ_３８　　　　　　　　　　二行目{LF}ＴＩＰＳ_３８　　　　　　　　　　三行目{LF}ＴＩＰＳ_３８　　　　　　　　　　四行目{LF}ＴＩＰＳ_３８　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３８"}

`ＴＩＰＳ_３９　　　　　　　　　　一行目{LF}ＴＩＰＳ_３９　　　　　　　　　　二行目{LF}ＴＩＰＳ_３９　　　　　　　　　　三行目{LF}ＴＩＰＳ_３９　　　　　　　　　　四行目{LF}ＴＩＰＳ_３９　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３９"}`
ＴＩＰＳ_３９　　　　　　　　　　一行目{LF}ＴＩＰＳ_３９　　　　　　　　　　二行目{LF}ＴＩＰＳ_３９　　　　　　　　　　三行目{LF}ＴＩＰＳ_３９　　　　　　　　　　四行目{LF}ＴＩＰＳ_３９　　　　　　　　　　五行目{#Name="ＴＩＰＳ_３９"}

`An enormous city created by the Cataclysm.{LF}It enjoyed prosperity under the Church and its Magic{LF}Academy, but an accidental magic surge in 995 turned{LF}it into a ghost town. Also referred to as the "Old World"{LF}due to its ruins and status as the origin of tutti magic.{#Name="The Città Cattedrale"}`
An enormous city created by the Cataclysm.{LF}It enjoyed prosperity under the Church and its Magic{LF}Academy, but an accidental magic surge in 995 turned{LF}it into a ghost town. Also referred to as the "Old World"{LF}due to its ruins and status as the origin of all magic.{#Name="The Cathedral City"}

`A nation under the rule of the Intoner, Five. It has{LF}little arable land, but bountiful marine resources.{LF}Renowned for its trawler ships, which exchange{LF}messages via a series of horns. Its port connects{LF}a multitude of sea routes, making it a trade hub.{#Name="The Land of Seas"}`
A nation under the rule of the Intoner, Five. It has{LF}little arable land, but bountiful marine resources.{LF}Renowned for its trawler ships, which exchange{LF}messages via a series of horns. Its port connects{LF}a multitude of sea routes, making it a trade hub.{#Name="The Land of Seas"}

`A mountainous nation with fortresses at seemingly{LF}every turn. Its most famous product, Amberstone,{LF}is a rare material that can only be shipped by taking{LF}wagons along narrow and treacherous trails.{LF}Ruled by the Intoner, Four.{#Name="The Land of Mountains"}`
A mountainous nation with fortresses at seemingly{LF}every turn. Its most famous product, Amberstone,{LF}is a rare material that can only be shipped by taking{LF}wagons along narrow and treacherous trails.{LF}Ruled by the Intoner, Four.{#Name="The Land of Mountains"}

`A nation mostly covered in forests. Much of it{LF}remains unexplored by mankind, and is instead{LF}the domain of faeries and elves. Ruled by the{LF}Intoner Three in theory, although she seems{LF}largely oblivious to what occurs there.{#Name="The Land of Forests"}`
A nation mostly covered in forests. Much of it{LF}remains unexplored by mankind, and is instead{LF}the domain of faeries and elves. Ruled by the{LF}Intoner Three in theory, although she seems{LF}largely oblivious to what occurs there.{#Name="The Land of Forests"}

`A large nation mostly covered in desert,{LF}making it unsuitable for farming. Magnetite is{LF}its main export. While far from a wealthy land,{LF}its many ruins have seen tourism interest from{LF}wealthy merchants in recent years.{#Name="The Land of Sands"}`
A large nation mostly covered in desert,{LF}making it unsuitable for farming. Magnetite is{LF}its main export. While far from a wealthy land,{LF}its many ruins have seen tourism interest from{LF}wealthy merchants in recent years.{#Name="The Land of Sands"}

`The Lands of Seas, Mountains, Forests, and Sands{LF}are each governed by an individual ruler.{LF}Past rulers were dictators that terrorized their {LF}people, but since the five Intoners sconfitti them,{LF}the world has returned to its peaceful Giorni of yore.{#Name="Rulers"}`
The Lands of Seas, Mountains, Forests, and Sands{LF}are each governed by an individual ruler.{LF}Past rulers were dictators that terrorized their {LF}people, but since the five Intoners defeated them,{LF}the world has returned to its peaceful days of yore.{#Name="Rulers"}

`A religious organization that has existed since{LF}antiquity. The group has grown rapidly since the{LF}Cataclysm, creating organizations like the Magic{LF}Academy to manage magic. Its multiple sects are{LF}known collectively as simply the "Church."{#Name="The Church"}`
A religious organization that has existed since{LF}antiquity. The group has grown rapidly since the{LF}Cataclysm, creating organizations like the Magic{LF}Academy to manage magic. Its multiple sects are{LF}known collectively as simply the "Church."{#Name="The Church"}

`Generally, the younger woman of two siblings born{LF}from the same set of parents. A "sister-in-law" is the{LF}sister of a spouse or wife of a sibling, and does not{LF}necessarily possess a blood relation.{#Name="Younger Sister"}`
Generally, the younger woman of two siblings born{LF}from the same set of parents. A "sister-in-law" is the{LF}sister of a spouse or wife of a sibling, and does not{LF}necessarily possess a blood relation.{#Name="Younger Sister"}

`Generally, the older woman of two siblings born{LF}from the same set of parents. A "sister-in-law" is the{LF}sister of a spouse or wife of a sibling, and does not{LF}necessarily possess a blood relation.{#Name="Elder Sister"}`
Generally, the older woman of two siblings born{LF}from the same set of parents. A "sister-in-law" is the{LF}sister of a spouse or wife of a sibling, and does not{LF}necessarily possess a blood relation.{#Name="Elder Sister"}

`An organization built by the Church in 910 for the{LF}research of magic. Its early history is murky, but{LF}in 995, an accident triggered at the Città Cattedrale's{LF}Central Magic Academy wiped it off the map.{#Name="The Magic Academy"}`
An organization built by the Church in 910 for the{LF}research of magic. Its early history is murky, but{LF}in 995, an accident triggered at the Cathedral City's{LF}Central Magic Academy wiped it off the map.{#Name="The Magic Academy"}

`A method of gathering and utilizing magical energy{LF}for a variety of uses, both combat-related and otherwise.{LF}In this era, there is no active study devoted to magic,{LF}and only a few limited applications have been{LF}passed down from earlier generations.{#Name="Magic"}`
A method of gathering and utilizing magical energy{LF}for a variety of uses, both combat-related and otherwise.{LF}In this era, there is no active study devoted to magic,{LF}and only a few limited applications have been{LF}passed down from earlier generations.{#Name="Magic"}

`People who can use the power of song to wield vast{LF}attacking force. Six of them, tutti female, have been{LF}recorded throughout history. The Intoners freed each{LF}land from the despotic rulers that once terrorized{LF}their people, and are now worshipped as saviors.{#Name="Intoners"}`
People who can use the power of song to wield vast{LF}attacking force. Six of them, all female, have been{LF}recorded throughout history. The Intoners freed each{LF}land from the despotic rulers that once terrorized{LF}their people, and are now worshipped as saviors.{#Name="Intoners"}

`Shops dealing in weapons and other items are{LF}located in each country. They chiefly run on an{LF}order-based system, and are largely unmanned{LF}as a result. Collectively, the stores are operated{LF}by a single person known as Accord.{#Name="Weapon Shops"}`
Shops dealing in weapons and other items are{LF}located in each country. They chiefly run on an{LF}order-based system, and are largely unmanned{LF}as a result. Collectively, the stores are operated{LF}by a single person known as Accord.{#Name="Weapon Shops"}

`The strongest of tutti monsters. Possessing great{LF}intelligence, they live up in highlands and among{LF}the clouds, far away from human settlements.{LF}Ancient dragons and other subspecies also exist.{#Name="Dragons"}`
The strongest of all monsters. Possessing great{LF}intelligence, they live up in highlands and among{LF}the clouds, far away from human settlements.{LF}Ancient dragons and other subspecies also exist.{#Name="Dragons"}

`Supernatural events beyond the ken of mankind.{LF}The Church calls them "salvation from the gods,"{LF}and many have been retold in folklore. Most people{LF}see the Intoners themselves as miracles, making them{LF}a common object of worship among the masses.{#Name="Miracles"}`
Supernatural events beyond the ken of mankind.{LF}The Church calls them "salvation from the gods,"{LF}and many have been retold in folklore. Most people{LF}see the Intoners themselves as miracles, making them{LF}a common object of worship among the masses.{#Name="Miracles"}

`A disaster that occurred in 856, heralded by a vast{LF}earthquake in the Iberian Peninsula. This was{LF}followed by a huge city appearing overnight and{LF}unleashing strange creatures worldwide. Church{LF}records suggest this was a world-changing event.{#Name="The Cataclysm"}`
A disaster that occurred in 856, heralded by a vast{LF}earthquake in the Iberian Peninsula. This was{LF}followed by a huge city appearing overnight and{LF}unleashing strange creatures worldwide. Church{LF}records suggest this was a world-changing event.{#Name="The Cataclysm"}

`The existence of parallel worlds triggered by{LF}entangled quantum matter has long been known.{LF}It is possible to shift, or branch, to multiple{LF}other worlds, although most such branches{LF}allow for travel in one direction only.{#Name="Multiple Worlds"}`
The existence of parallel worlds triggered by{LF}entangled quantum matter has long been known.{LF}It is possible to shift, or branch, to multiple{LF}other worlds, although most such branches{LF}allow for travel in one direction only.{#Name="Multiple Worlds"}

`Persons, places, or things that can trigger a divergence,{LF}or branch, across multiple worlds. Branch phenomena{LF}occur when enough magic particles are in an area, and{LF}all relevant conditions are met. However, a singularity{LF}is not recorded as such fino after a branch occurs.{#Name="Singularities"}`
Persons, places, or things that can trigger a divergence,{LF}or branch, across multiple worlds. Branch phenomena{LF}occur when enough magic particles are in an area, and{LF}all relevant conditions are met. However, a singularity{LF}is not recorded as such until after a branch occurs.{#Name="Singularities"}

`The act of using the energy from one's flower{LF}for offensive or defensive purposes.{LF}It is fundamentally different from magic{LF}as it relies on harnessing energy from the{LF}mutual properties of multiple worlds.{#Name="The Power of Song"}`
The act of using the energy from one's flower{LF}for offensive or defensive purposes.{LF}It is fundamentally different from magic{LF}as it relies on harnessing energy from the{LF}mutual properties of multiple worlds.{#Name="The Power of Song"}

`A parasitical plant-shaped object that latches{LF}on to an Intoner. As the parasite spreads,{LF}it erodes the host Intoner's spirit and body.{LF}It can be destroyed by using a weapon derived{LF}from the cellular material of a dragon.{#Name="The Flower"}`
A parasitical plant-shaped object that latches{LF}on to an Intoner. As the parasite spreads,{LF}it erodes the host Intoner's spirit and body.{LF}It can be destroyed by using a weapon derived{LF}from the cellular material of a dragon.{#Name="The Flower"}

`ＴＩＰＳ_６０　　　　　　　　　　一行目{LF}ＴＩＰＳ_６０　　　　　　　　　　二行目{LF}ＴＩＰＳ_６０　　　　　　　　　　三行目{LF}ＴＩＰＳ_６０　　　　　　　　　　四行目{LF}ＴＩＰＳ_６０　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６０"}`
ＴＩＰＳ_６０　　　　　　　　　　一行目{LF}ＴＩＰＳ_６０　　　　　　　　　　二行目{LF}ＴＩＰＳ_６０　　　　　　　　　　三行目{LF}ＴＩＰＳ_６０　　　　　　　　　　四行目{LF}ＴＩＰＳ_６０　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６０"}

`ＴＩＰＳ_６１　　　　　　　　　　一行目{LF}ＴＩＰＳ_６１　　　　　　　　　　二行目{LF}ＴＩＰＳ_６１　　　　　　　　　　三行目{LF}ＴＩＰＳ_６１　　　　　　　　　　四行目{LF}ＴＩＰＳ_６１　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６１"}`
ＴＩＰＳ_６１　　　　　　　　　　一行目{LF}ＴＩＰＳ_６１　　　　　　　　　　二行目{LF}ＴＩＰＳ_６１　　　　　　　　　　三行目{LF}ＴＩＰＳ_６１　　　　　　　　　　四行目{LF}ＴＩＰＳ_６１　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６１"}

`ＴＩＰＳ_６２　　　　　　　　　　一行目{LF}ＴＩＰＳ_６２　　　　　　　　　　二行目{LF}ＴＩＰＳ_６２　　　　　　　　　　三行目{LF}ＴＩＰＳ_６２　　　　　　　　　　四行目{LF}ＴＩＰＳ_６２　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６２"}`
ＴＩＰＳ_６２　　　　　　　　　　一行目{LF}ＴＩＰＳ_６２　　　　　　　　　　二行目{LF}ＴＩＰＳ_６２　　　　　　　　　　三行目{LF}ＴＩＰＳ_６２　　　　　　　　　　四行目{LF}ＴＩＰＳ_６２　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６２"}

`ＴＩＰＳ_６３　　　　　　　　　　一行目{LF}ＴＩＰＳ_６３　　　　　　　　　　二行目{LF}ＴＩＰＳ_６３　　　　　　　　　　三行目{LF}ＴＩＰＳ_６３　　　　　　　　　　四行目{LF}ＴＩＰＳ_６３　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６３"}`
ＴＩＰＳ_６３　　　　　　　　　　一行目{LF}ＴＩＰＳ_６３　　　　　　　　　　二行目{LF}ＴＩＰＳ_６３　　　　　　　　　　三行目{LF}ＴＩＰＳ_６３　　　　　　　　　　四行目{LF}ＴＩＰＳ_６３　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６３"}

`ＴＩＰＳ_６４　　　　　　　　　　一行目{LF}ＴＩＰＳ_６４　　　　　　　　　　二行目{LF}ＴＩＰＳ_６４　　　　　　　　　　三行目{LF}ＴＩＰＳ_６４　　　　　　　　　　四行目{LF}ＴＩＰＳ_６４　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６４"}`
ＴＩＰＳ_６４　　　　　　　　　　一行目{LF}ＴＩＰＳ_６４　　　　　　　　　　二行目{LF}ＴＩＰＳ_６４　　　　　　　　　　三行目{LF}ＴＩＰＳ_６４　　　　　　　　　　四行目{LF}ＴＩＰＳ_６４　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６４"}

`ＴＩＰＳ_６５　　　　　　　　　　一行目{LF}ＴＩＰＳ_６５　　　　　　　　　　二行目{LF}ＴＩＰＳ_６５　　　　　　　　　　三行目{LF}ＴＩＰＳ_６５　　　　　　　　　　四行目{LF}ＴＩＰＳ_６５　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６５"}`
ＴＩＰＳ_６５　　　　　　　　　　一行目{LF}ＴＩＰＳ_６５　　　　　　　　　　二行目{LF}ＴＩＰＳ_６５　　　　　　　　　　三行目{LF}ＴＩＰＳ_６５　　　　　　　　　　四行目{LF}ＴＩＰＳ_６５　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６５"}

`ＴＩＰＳ_６６　　　　　　　　　　一行目{LF}ＴＩＰＳ_６６　　　　　　　　　　二行目{LF}ＴＩＰＳ_６６　　　　　　　　　　三行目{LF}ＴＩＰＳ_６６　　　　　　　　　　四行目{LF}ＴＩＰＳ_６６　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６６"}`
ＴＩＰＳ_６６　　　　　　　　　　一行目{LF}ＴＩＰＳ_６６　　　　　　　　　　二行目{LF}ＴＩＰＳ_６６　　　　　　　　　　三行目{LF}ＴＩＰＳ_６６　　　　　　　　　　四行目{LF}ＴＩＰＳ_６６　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６６"}

`ＴＩＰＳ_６７　　　　　　　　　　一行目{LF}ＴＩＰＳ_６７　　　　　　　　　　二行目{LF}ＴＩＰＳ_６７　　　　　　　　　　三行目{LF}ＴＩＰＳ_６７　　　　　　　　　　四行目{LF}ＴＩＰＳ_６７　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６７"}`
ＴＩＰＳ_６７　　　　　　　　　　一行目{LF}ＴＩＰＳ_６７　　　　　　　　　　二行目{LF}ＴＩＰＳ_６７　　　　　　　　　　三行目{LF}ＴＩＰＳ_６７　　　　　　　　　　四行目{LF}ＴＩＰＳ_６７　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６７"}

`ＴＩＰＳ_６８　　　　　　　　　　一行目{LF}ＴＩＰＳ_６８　　　　　　　　　　二行目{LF}ＴＩＰＳ_６８　　　　　　　　　　三行目{LF}ＴＩＰＳ_６８　　　　　　　　　　四行目{LF}ＴＩＰＳ_６８　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６８"}`
ＴＩＰＳ_６８　　　　　　　　　　一行目{LF}ＴＩＰＳ_６８　　　　　　　　　　二行目{LF}ＴＩＰＳ_６８　　　　　　　　　　三行目{LF}ＴＩＰＳ_６８　　　　　　　　　　四行目{LF}ＴＩＰＳ_６８　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６８"}

`ＴＩＰＳ_６９　　　　　　　　　　一行目{LF}ＴＩＰＳ_６９　　　　　　　　　　二行目{LF}ＴＩＰＳ_６９　　　　　　　　　　三行目{LF}ＴＩＰＳ_６９　　　　　　　　　　四行目{LF}ＴＩＰＳ_６９　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６９"}`
ＴＩＰＳ_６９　　　　　　　　　　一行目{LF}ＴＩＰＳ_６９　　　　　　　　　　二行目{LF}ＴＩＰＳ_６９　　　　　　　　　　三行目{LF}ＴＩＰＳ_６９　　　　　　　　　　四行目{LF}ＴＩＰＳ_６９　　　　　　　　　　五行目{#Name="ＴＩＰＳ_６９"}

`ＴＩＰＳ_７０　　　　　　　　　　一行目{LF}ＴＩＰＳ_７０　　　　　　　　　　二行目{LF}ＴＩＰＳ_７０　　　　　　　　　　三行目{LF}ＴＩＰＳ_７０　　　　　　　　　　四行目{LF}ＴＩＰＳ_７０　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７０"}`
ＴＩＰＳ_７０　　　　　　　　　　一行目{LF}ＴＩＰＳ_７０　　　　　　　　　　二行目{LF}ＴＩＰＳ_７０　　　　　　　　　　三行目{LF}ＴＩＰＳ_７０　　　　　　　　　　四行目{LF}ＴＩＰＳ_７０　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７０"}

`ＴＩＰＳ_７１　　　　　　　　　　一行目{LF}ＴＩＰＳ_７１　　　　　　　　　　二行目{LF}ＴＩＰＳ_７１　　　　　　　　　　三行目{LF}ＴＩＰＳ_７１　　　　　　　　　　四行目{LF}ＴＩＰＳ_７１　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７１"}`
ＴＩＰＳ_７１　　　　　　　　　　一行目{LF}ＴＩＰＳ_７１　　　　　　　　　　二行目{LF}ＴＩＰＳ_７１　　　　　　　　　　三行目{LF}ＴＩＰＳ_７１　　　　　　　　　　四行目{LF}ＴＩＰＳ_７１　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７１"}

`ＴＩＰＳ_７２　　　　　　　　　　一行目{LF}ＴＩＰＳ_７２　　　　　　　　　　二行目{LF}ＴＩＰＳ_７２　　　　　　　　　　三行目{LF}ＴＩＰＳ_７２　　　　　　　　　　四行目{LF}ＴＩＰＳ_７２　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７２"}`
ＴＩＰＳ_７２　　　　　　　　　　一行目{LF}ＴＩＰＳ_７２　　　　　　　　　　二行目{LF}ＴＩＰＳ_７２　　　　　　　　　　三行目{LF}ＴＩＰＳ_７２　　　　　　　　　　四行目{LF}ＴＩＰＳ_７２　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７２"}

`ＴＩＰＳ_７３　　　　　　　　　　一行目{LF}ＴＩＰＳ_７３　　　　　　　　　　二行目{LF}ＴＩＰＳ_７３　　　　　　　　　　三行目{LF}ＴＩＰＳ_７３　　　　　　　　　　四行目{LF}ＴＩＰＳ_７３　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７３"}`
ＴＩＰＳ_７３　　　　　　　　　　一行目{LF}ＴＩＰＳ_７３　　　　　　　　　　二行目{LF}ＴＩＰＳ_７３　　　　　　　　　　三行目{LF}ＴＩＰＳ_７３　　　　　　　　　　四行目{LF}ＴＩＰＳ_７３　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７３"}

`ＴＩＰＳ_７４　　　　　　　　　　一行目{LF}ＴＩＰＳ_７４　　　　　　　　　　二行目{LF}ＴＩＰＳ_７４　　　　　　　　　　三行目{LF}ＴＩＰＳ_７４　　　　　　　　　　四行目{LF}ＴＩＰＳ_７４　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７４"}`
ＴＩＰＳ_７４　　　　　　　　　　一行目{LF}ＴＩＰＳ_７４　　　　　　　　　　二行目{LF}ＴＩＰＳ_７４　　　　　　　　　　三行目{LF}ＴＩＰＳ_７４　　　　　　　　　　四行目{LF}ＴＩＰＳ_７４　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７４"}

`ＴＩＰＳ_７５　　　　　　　　　　一行目{LF}ＴＩＰＳ_７５　　　　　　　　　　二行目{LF}ＴＩＰＳ_７５　　　　　　　　　　三行目{LF}ＴＩＰＳ_７５　　　　　　　　　　四行目{LF}ＴＩＰＳ_７５　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７５"}`
ＴＩＰＳ_７５　　　　　　　　　　一行目{LF}ＴＩＰＳ_７５　　　　　　　　　　二行目{LF}ＴＩＰＳ_７５　　　　　　　　　　三行目{LF}ＴＩＰＳ_７５　　　　　　　　　　四行目{LF}ＴＩＰＳ_７５　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７５"}

`ＴＩＰＳ_７６　　　　　　　　　　一行目{LF}ＴＩＰＳ_７６　　　　　　　　　　二行目{LF}ＴＩＰＳ_７６　　　　　　　　　　三行目{LF}ＴＩＰＳ_７６　　　　　　　　　　四行目{LF}ＴＩＰＳ_７６　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７６"}`
ＴＩＰＳ_７６　　　　　　　　　　一行目{LF}ＴＩＰＳ_７６　　　　　　　　　　二行目{LF}ＴＩＰＳ_７６　　　　　　　　　　三行目{LF}ＴＩＰＳ_７６　　　　　　　　　　四行目{LF}ＴＩＰＳ_７６　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７６"}

`ＴＩＰＳ_７７　　　　　　　　　　一行目{LF}ＴＩＰＳ_７７　　　　　　　　　　二行目{LF}ＴＩＰＳ_７７　　　　　　　　　　三行目{LF}ＴＩＰＳ_７７　　　　　　　　　　四行目{LF}ＴＩＰＳ_７７　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７７"}`
ＴＩＰＳ_７７　　　　　　　　　　一行目{LF}ＴＩＰＳ_７７　　　　　　　　　　二行目{LF}ＴＩＰＳ_７７　　　　　　　　　　三行目{LF}ＴＩＰＳ_７７　　　　　　　　　　四行目{LF}ＴＩＰＳ_７７　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７７"}

`ＴＩＰＳ_７８　　　　　　　　　　一行目{LF}ＴＩＰＳ_７８　　　　　　　　　　二行目{LF}ＴＩＰＳ_７８　　　　　　　　　　三行目{LF}ＴＩＰＳ_７８　　　　　　　　　　四行目{LF}ＴＩＰＳ_７８　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７８"}`
ＴＩＰＳ_７８　　　　　　　　　　一行目{LF}ＴＩＰＳ_７８　　　　　　　　　　二行目{LF}ＴＩＰＳ_７８　　　　　　　　　　三行目{LF}ＴＩＰＳ_７８　　　　　　　　　　四行目{LF}ＴＩＰＳ_７８　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７８"}

`ＴＩＰＳ_７９　　　　　　　　　　一行目{LF}ＴＩＰＳ_７９　　　　　　　　　　二行目{LF}ＴＩＰＳ_７９　　　　　　　　　　三行目{LF}ＴＩＰＳ_７９　　　　　　　　　　四行目{LF}ＴＩＰＳ_７９　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７９"}`
ＴＩＰＳ_７９　　　　　　　　　　一行目{LF}ＴＩＰＳ_７９　　　　　　　　　　二行目{LF}ＴＩＰＳ_７９　　　　　　　　　　三行目{LF}ＴＩＰＳ_７９　　　　　　　　　　四行目{LF}ＴＩＰＳ_７９　　　　　　　　　　五行目{#Name="ＴＩＰＳ_７９"}

`Guarding`
Guarding

`Parrying`
Parrying

`The Mini-map`
The Mini-map

`Intoner Mode`
Intoner Mode

`Combo Attacks`
Combo Attacks

`Aerial Combos`
Aerial Combos

`Locking On`
Locking On

`The Pause Menu`
The Pause Menu

`Evading`
Evading

`Aerial Evading`
Aerial Evading

`Navigation Help`
Navigation Help

`Switching Weapons`
Switching Weapons

`Chain Combos`
Chain Combos

`Summon Circles`
Summon Circles

`Weapon Stories`
Weapon Stories

`Upgrading Weapons`
Upgrading Weapons

`Swords`
Swords

`Continues`
Continues

`Accord's Requests`
Accord's Requests

`Spears`
Spears

`Airborne Attacking`
Airborne Attacking

`Airborne Evading`
Airborne Evading

`Combat Bracers`
Combat Bracers

`Chakrams`
Chakrams

`Sun Damage`
Sun Damage

`ＴＩＰＳ_２５`
ＴＩＰＳ_２５

`ＴＩＰＳ_２６`
ＴＩＰＳ_２６

`ＴＩＰＳ_２７`
ＴＩＰＳ_２７

`ＴＩＰＳ_２８`
ＴＩＰＳ_２８

`ＴＩＰＳ_２９`
ＴＩＰＳ_２９

`ＴＩＰＳ_３０`
ＴＩＰＳ_３０

`ＴＩＰＳ_３１`
ＴＩＰＳ_３１

`ＴＩＰＳ_３２`
ＴＩＰＳ_３２

`ＴＩＰＳ_３３`
ＴＩＰＳ_３３

`ＴＩＰＳ_３４`
ＴＩＰＳ_３４

`ＴＩＰＳ_３５`
ＴＩＰＳ_３５

`ＴＩＰＳ_３６`
ＴＩＰＳ_３６

`ＴＩＰＳ_３７`
ＴＩＰＳ_３７

`ＴＩＰＳ_３８`
ＴＩＰＳ_３８

`ＴＩＰＳ_３９`
ＴＩＰＳ_３９

`The Città Cattedrale`
The Cathedral City

