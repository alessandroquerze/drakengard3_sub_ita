`Sì. Ecco, ora inizi a tirare fuori le parole difficili...{#Name="Cent"}`
Yeah. See, now you're breaking out the big words...{#Name="Cent"}

`Persino uno sveglio come me fatica a seguirti.{#Name="Cent"}`
Even a smart guy like me is having trouble following.{#Name="Cent"}

`Ma cielo, sei davvero bellissima...{#Name="Cent"}`
But goodness, you certainly are beautiful...{#Name="Cent"}

`Certo, potreeesti essere ancora più bella, se volessi.{#Name="Cent"}`
Of course, you cooould be even more lovely if you wanted.{#Name="Cent"}

`Magari una spuntatina ai capelli... Via gli occhiali...{LF}Una tinta veloce lì sopra...{#Name="Cent"}`
Maybe trim the hair... Take off the glasses...{LF}A quick dye job up top...{#Name="Cent"}

`Stai cercando di farla assomigliare a Two?{#Name="Dito"}`
Are you trying to make her look like Two?{#Name="Dito"}

`Iooo? Cosa? No. E' assurdo.{#Name="Cent"}`
Meee? What? No. That's absurd.{#Name="Cent"}

`Voglio solo che sia bellissima.{LF}Non è colpa mia se ho un tipo!{#Name="Cent"}`
I just want her to look beautiful.{LF}I can't help it if I have a type!{#Name="Cent"}

`E comunque chi diavolo è questa tizia?{#Name="Dito"}`
Who the hell is this chick, anyway?{#Name="Dito"}

`Hai intenzione di farla unire a noi, Zero?{#Name="Dito"}`
You planning to let her join us, Zero?{#Name="Dito"}

`Stai scherzando?{#Name="Zero"}`
Are you kidding?{#Name="Zero"}

`Bene. Non sopporto quel tipo.{#Name="Dito"}`
Good. I can't stand that type.{#Name="Dito"}

`Che tipo?{#Name="Mikhail"}`
What type?{#Name="Mikhail"}

`Lo sai! Quelle che sembrano tutte perfettine{LF}e roba simile.{#Name="Dito"}`
You know! The type who looks all prim and proper{LF}and whatever.{#Name="Dito"}

`Quelle che vanno in giro come se la loro merda non puzzasse.{#Name="Dito"}`
The type who walks around like their shit don't stink.{#Name="Dito"}

`...Quelle come Five, okay!? Non le sopporto.{LF}Mi fanno venire da vomitare.{#Name="Dito"}`
...The type like Five, okay!? Can't stand 'em.{LF}Makes me wanna puke.{#Name="Dito"}

`Sei messo male quanto Cent.{#Name="Zero"}`
You're as bad as Cent.{#Name="Zero"}

`Tenetela solo lontana dalla mia vista.{#Name="Dito"}`
Just keep her out of my sight.{#Name="Dito"}

`Lady Zero, forse sarebbe saggio{LF}mantenere le distanze da Mikhail.{#Name="Decadus"}`
Lady Zero, perhaps it would be wise to{LF}keep your distance from Mikhail.{#Name="Decadus"}

`Eh?{#Name="Mikhail"}`
Huh?{#Name="Mikhail"}

`No, va bene così.{#Name="Zero"}`
No, it's fine.{#Name="Zero"}

`Ma...{#Name="Decadus"}`
But...{#Name="Decadus"}

`Ho bisogno di lui.{#Name="Zero"}`
I need him.{#Name="Zero"}

`Eh!?{#Name="Mikhail"}`
Huh!?{#Name="Mikhail"}

`Beh, okay. Non è che abbia proprio bisogno di lui.{#Name="Zero"}`
Well, okay. I don't necessarily need him.{#Name="Zero"}

`Ma ho bisogno di un drago, e lui è l'unico che ho.{#Name="Zero"}`
But I need a dragon, and he's the only one I got.{#Name="Zero"}

`Seguirò la vostra guida, mia lady, qualunque{LF}direzione essa prenda.{#Name="Decadus"}`
I shall follow your lead, my lady, no matter{LF}where it may take me.{#Name="Decadus"}

`Ehi, Zero! Non ascoltare quello che ha detto quella strana ragazza, okay?{#Name="Mikhail"}`
Hey, Zero! Don't listen to what that weird girl said, okay?{#Name="Mikhail"}

`Intendi la parte in cui diceva che mi mangerai?{#Name="Zero"}`
You mean the bit about how you're gonna eat me?{#Name="Zero"}

`Sì! E' scortese! Io non ti mangerei mai, Zero!{LF}Mai!{#Name="Mikhail"}`
Yeah! That's mean! I'd never eat you, Zero!{LF}Never!{#Name="Mikhail"}

`Sei sicuro? Voglio dire, mangi già carne marcia.{LF}Non è che tu sia schizzinoso.{#Name="Zero"}`
You sure? I mean, you already eat rotten meat.{LF}It's not like you're picky.{#Name="Zero"}

`Assolutamente no! Scommetto che tu hai un sapore MOOOLTO peggiore della carne marcia!{#Name="Mikhail"}`
No way! I bet you taste WAAAY worse than rotten meat!{#Name="Mikhail"}

`Sai davvero come tirare su di morale una ragazza.{#Name="Zero"}`
You really know how to cheer a girl up.{#Name="Zero"}

`Senti, non ti mangerò, okay?{#Name="Mikhail"}`
Look, I'm not gonna eat you, okay?{#Name="Mikhail"}

`Ehi. Tu. Piantala con queste stronzate, okay?{#Name="Zero"}`
Hey. You. Knock off the bullshit, okay?{#Name="Zero"}

`Le... stronzate?{#Name="Accord"}`
The...bullshit?{#Name="Accord"}

`Ah. Devi riferirti alla mia affermazione{LF}secondo cui Mikhail ti avrebbe mangiata.{#Name="Accord"}`
Ah. You must be referring to my statement{LF}that Mikhail was going to eat you.{#Name="Accord"}

`Non c'è bisogno che tu la ripeta.{#Name="Zero"}`
You don't need to repeat it.{#Name="Zero"}

`...Ti spaventa?{#Name="Accord"}`
...Does this scare you?{#Name="Accord"}

`Spaventarmi? Ma col cazzo.{#Name="Zero"}`
Scare me? Hell no.{#Name="Zero"}

`Una morte vale l'altra.{#Name="Zero"}`
One death is as good as another.{#Name="Zero"}

`Non quello. Parlo dell'idea di perdere i tuoi amici.{#Name="Accord"}`
Not that. I'm talking about the idea of losing your friends.{#Name="Accord"}

`...Io non ho amici. Non ne ho mai avuti.{#Name="Zero"}`
...I don't have friends. I never have.{#Name="Zero"}

`Non ho davvero alcuna opinione in merito.{LF}Sono qui solo per osservare e registrare.{#Name="Accord"}`
I really have no opinion on the matter.{LF}I'm just here to observe and record.{#Name="Accord"}

`Cosa farai adesso?{#Name="Accord"}`
What will you do next?{#Name="Accord"}

`Rintraccerò Three e la ucciderò.{#Name="Zero"}`
Track down Three and kill her.{#Name="Zero"}

`Ah, e se mi segui, ucciderò anche te.{#Name="Zero"}`
Oh, and if you follow me, I'll kill you too.{#Name="Zero"}

`Non preoccuparti. Non ho il permesso di interferire in battaglia.{#Name="Accord"}`
Don't worry. I don't have permission to interfere in battle.{#Name="Accord"}

`Bene, allora.{#Name="Zero"}`
Well, good then.{#Name="Zero"}

`Abbi cura di te.{#Name="Accord"}`
Do take care of yourself.{#Name="Accord"}

`Questa Diramazione in cui ti trovi è gravemente danneggiata, e il mondo{LF}diventerà solo più caotico man mano che andrai avanti.{#Name="Accord"}`
This branch you're in is heavily damaged, and the world{LF}will only grow more chaotic as you proceed.{#Name="Accord"}

`Grazie per il... consiglio.{#Name="Zero"}`
Thanks for the...tip.{#Name="Zero"}

`Naturalmente, questo significa anche che c'è una possibilità maggiore{LF}di salvare il mondo...{#Name="Accord"}`
Of course, that also means there's a greater chance{LF}of saving the world as well...{#Name="Accord"}

`Le sorelle stanno crollando.{#Name="Accord"}`
The sisters are coming undone.{#Name="Accord"}

`Mikhail si è evoluto all'improvviso di due livelli in una volta.{#Name="Accord"}`
Mikhail suddenly evolved two levels at once.{#Name="Accord"}

`In mezzo a tutte queste anomalie,{#Name="Accord"}`
In the midst of all these anomalies,{#Name="Accord"}

`Zero si sta dirigendo verso La Terra delle Foreste.{#Name="Accord"}`
Zero is heading for the Land of Forests.{#Name="Accord"}

` `
 

`Accord`
Accord

`Mikhail`
Mikhail

`Octa`
Octa

`Dito`
Dito

`Zero`
Zero

`Cent`
Cent

`Decadus`
Decadus

`Procedere alla prossima missione?{#Name=" "}`
Proceed to the next mission?{#Name=" "}

`みんなで一つの場所に集まるデモ{#Name=" "}`
みんなで一つの場所に集まるデモ{#Name=" "}

`Sono... preoccupato.{#Name="Decadus"}`
I am...worried.{#Name="Decadus"}

`Per cosa?{#Name="Zero"}`
About what?{#Name="Zero"}

`Per Mikhail.{#Name="Decadus"}`
About Mikhail.{#Name="Decadus"}

`Non esserlo. Sta bene.{#Name="Zero"}`
Don't be. He's fine.{#Name="Zero"}

`Ma...{#Name="Decadus"}`
But...{#Name="Decadus"}

`Ti sto dicendo che sta bene.{#Name="Zero"}`
I'm telling you. He's fine.{#Name="Zero"}

`Molto bene.{#Name="Decadus"}`
Very well.{#Name="Decadus"}

`Sono certo che Mikhail sia al sicuro. Ma dovremmo comunque salvarlo.{#Name="Decadus"}`
I'm sure Mikhail is safe. But we should rescue him nonetheless.{#Name="Decadus"}

`Che scenetta, quella di prima, vero!{#Name="Octa"}`
That was quite the scene earlier, wasn't it!{#Name="Octa"}

`Prima il tuo drago si fa un pasto con Lady Three,{LF}poi viene rapito da un altro discepolo? Follia!{#Name="Octa"}`
First, your dragon makes a meal out of Lady Three,{LF}then he's stolen away by another disciple? Madness!{#Name="Octa"}

`......{#Name="Zero"}`
......{#Name="Zero"}

`Presumo che gli abbiate ordinato di mangiarla?{#Name="Octa"}`
You ordered him to eat her, I presume?{#Name="Octa"}

`No, proprio per niente.{#Name="Zero"}`
Nope, sure didn't.{#Name="Zero"}

`Oh, cielo... Che crudele scherzo del destino!{#Name="Octa"}`
Oh, my... Such a cruel twist of fate!{#Name="Octa"}

`Lady Three ha avuto una fine crudele. Forse tu e io{LF}dovremmo "commemorare" la sua dipartita, mmmmm?{#Name="Octa"}`
Lady Three suffered a cruel end. Perhaps you and I{LF}should "commemorate" her passing, mmmmm?{#Name="Octa"}

`Allora, sai quel tizio che ha rubato il tuo stupido drago?{LF}Era Cent.{#Name="Dito"}`
So you know that guy who stole your dumb dragon?{LF}That was Cent.{#Name="Dito"}

`E' il discepolo qui nella Terra delle Sabbie.{#Name="Dito"}`
He's the disciple here in the Land of Sands.{#Name="Dito"}

`Il discepolo di Two, eh?{#Name="Zero"}`
Two's disciple, huh?{#Name="Zero"}

`Già. Sembrano giovani, belli e follemente innamorati.{LF}Mi fanno venire da vomitare!{#Name="Dito"}`
Yep. They seem young, beautiful, and madly in love.{LF}Makes me wanna hurl!{#Name="Dito"}

`竜を２人の子ども代わりに育てていたりしてな{#Name="ゼロ"}`
竜を２人の子ども代わりに育てていたりしてな{#Name="ゼロ"}

`なんですか、その昔話みたいなほっこり感！{#Name="ディト"}`
なんですか、その昔話みたいなほっこり感！{#Name="ディト"}

`Questo Cent sembra proprio un gran rubacuori, vero?{LF}Probabilmente è meglio ucciderlo e basta quando fai fuori Two.{#Name="Dito"}`
This Cent guy seems like a real charmer, doesn't he?{LF}Probably better to just kill him when you take out Two.{#Name="Dito"}

`Secondo te cosa stanno progettando quei due, comunque?{#Name="Dito"}`
So what do you think those two are planning, anyway?{#Name="Dito"}

`Progettando? Come se uno dei due fosse abbastanza sveglio{LF}da avere un piano vero.{#Name="Zero"}`
Planning? As if either of them were smart enough{LF}to have an actual plan.{#Name="Zero"}

`Davvero? Pensavo che Cent avesse preso il tuo drago perché gliel'aveva ordinato Two.{#Name="Dito"}`
Really? I figured Cent took your dragon because Two ordered it.{#Name="Dito"}

`Deve avere qualche motivo per farlo, giusto? {#Name="Dito"}`
She's gotta have some reason for doing that, right? {#Name="Dito"}

`......{#Name="Zero"}`
......{#Name="Zero"}

`325{#Name="325"}`
325{#Name="325"}

`Zero ha sconfitto Three, ma nel frattempo ha perso Mikhail per mano del discepolo Cent.{#Name="Accord"}`
Zero defeated Three, but lost Mikhail to the disciple, Cent, in the process.{#Name="Accord"}

`Il gruppo si è diretto verso la casa di Cent nella Terra delle Sabbie{LF}per cercare di salvare il drago...{#Name="Accord"}`
The group headed for Cent's home in the Land of Sands{LF}to try and rescue the dragon...{#Name="Accord"}

` `
 

`Decadus`
Decadus

`Zero`
Zero

`Octa`
Octa

`Dito`
Dito

`ゼロ`
ゼロ

`ディト`
ディト

`325`
325

`Accord`
Accord

