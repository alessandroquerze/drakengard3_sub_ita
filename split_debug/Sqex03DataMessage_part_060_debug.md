`Prendetela! Fatela a pezzi!{LF}FATEEEEELA A PEZZI!{#Name="Four"}`
Get her! Tear her apart!{LF}TEEEEEEAR HER APAAART!{#Name="Four"}

`Non ascoltarla, Zero!{#Name="Mikhail"}`
Don't listen to her, Zero!{#Name="Mikhail"}

`E' fuori di testa! Farà impazzire anche te!{LF}Completamente fuori di testa!{#Name="Mikhail"}`
She's bonkers! She'll make you bonkers too!{LF}Totally bonkers!{#Name="Mikhail"}

`Smettila di dire fuori di testa!{#Name="Zero"}`
Stop saying bonkers already!{#Name="Zero"}

`No... Ti prego, no!{#Name="Four"}`
No... Please, no!{#Name="Four"}

`M-Ma...come può questo...{#Name="Four"}`
B-But...how could...{#Name="Four"}

`succedere...a...me...{#Name="Four"}`
this happen...to...me...{#Name="Four"}

`Non preoccuparti.{#Name="Zero"}`
Don't worry.{#Name="Zero"}

`Le tue sorelle ti raggiungeranno abbastanza presto.{#Name="Zero"}`
Your sisters will be joining you soon enough.{#Name="Zero"}

`Ehi. E' tutta tua.{#Name="Zero"}`
Hey. She's all yours.{#Name="Zero"}

`L'evoluzione di un drago...{#Name="Decadus"}`
The evolution of a dragon...{#Name="Decadus"}

`Santo cielo! Si è evoluto due volte in una volta sola!{#Name="Octa"}`
Good gracious! He's evolved twice in one go!{#Name="Octa"}

`Ora è tutto pronto. ...Michael.{#Name="Zero"}`
Everything is ready now. ...Michael.{#Name="Zero"}

`Uccidi Four.{#Name="説明文"}`
Kill Four.{#Name="説明文"}

`Zero!{#Name="Mikhail"}`
Zero!{#Name="Mikhail"}

`Lascia fare a me!{#Name="Zero"}`
Leave this to me!{#Name="Zero"}

`Four`
Four

`Zero`
Zero

`Octa`
Octa

`Mikhail`
Mikhail

`Decadus`
Decadus

`説明文`
説明文

`Zero è qui! Ci sta invadendo!{#Name="Soldier A"}`
Zero's here! She's invading!{#Name="Soldier A"}

`Copritemi! Vado dentro!{#Name="Soldier B"}`
Back me up! I'm going in!{#Name="Soldier B"}

`Ti copro io!{#Name="Soldier C"}`
I've got your back!{#Name="Soldier C"}

`Dove sono i medici!?{#Name="Soldier D"}`
Where are the medics!?{#Name="Soldier D"}

`Possiamo davvero uccidere un'Intoner!?{#Name="Soldier E"}`
Can we really kill an Intoner!?{#Name="Soldier E"}

`Ohhh! Che spettacolo tremendo!{#Name="Soldier F"}`
Ohhh! Such a dreadful sight!{#Name="Soldier F"}

`Aaaaaggghh! L'Intoner maledetta è qui!{#Name="Soldier G"}`
Aaaaaggghh! The accursed Intoner is here!{#Name="Soldier G"}

`Ugh! Come fanno a essere così dannatamente tosti!?{#Name="Soldier B"}`
Ugh! How can they be so damn tough!?{#Name="Soldier B"}

`Non sottovalutatela!{#Name="Soldier E"}`
Don't underestimate her!{#Name="Soldier E"}

`来るかッ？　来るかッ？{#Name="兵士C"}`
来るかッ？　来るかッ？{#Name="兵士C"}

`来たあああああッ！{#Name="兵士D"}`
来たあああああッ！{#Name="兵士D"}

`援護しますッ{#Name="兵士G"}`
援護しますッ{#Name="兵士G"}

`死にたいのか？　老いぼれにかまわず行けッ！{#Name="兵士F"}`
死にたいのか？　老いぼれにかまわず行けッ！{#Name="兵士F"}

`E' troppo... dannatamente forte!{#Name="Soldier A"}`
She's...too damn strong!{#Name="Soldier A"}

`Lo sapevamo quando abbiamo accettato questo incarico!{#Name="Soldier C"}`
We knew that when we took this job!{#Name="Soldier C"}

`Esatto! Sapevamo che stavamo combattendo un'Intoner!{#Name="Soldier D"}`
That's right! We knew we were fighting an Intoner!{#Name="Soldier D"}

`Dobbiamo fermarla qui!{#Name="Soldier E"}`
We need to stop her here!{#Name="Soldier E"}

`Facciamolo!{#Name="Soldier F"}`
Let's do this!{#Name="Soldier F"}

`La sua spada... Non riesco nemmeno a seguirla!{#Name="Soldier G"}`
Her sword... I can't even follow it!{#Name="Soldier G"}

`Non restate fermi! Vi uccideranno!{#Name="Soldier A"}`
Don't stand still! They'll kill you!{#Name="Soldier A"}

`Merda!{#Name="Soldier B"}`
Shit!{#Name="Soldier B"}

`Attento!{#Name="Soldier G"}`
Look out!{#Name="Soldier G"}

`Non posso andare oltre!{#Name="Soldier C"}`
I can't go any further!{#Name="Soldier C"}

`Tu! Ritirati!{#Name="Soldier F"}`
You! Fall back!{#Name="Soldier F"}

`No, lasciatemi! Concentratevi su Zero!{#Name="Soldier C"}`
No, leave me! Focus on Zero!{#Name="Soldier C"}

`Mostriamo loro di che pasta siamo fatti noi mortali!{#Name="Soldier B"}`
Show 'em what we mortals are made of!{#Name="Soldier B"}

`Mostriamo loro di che pasta siamo fatti noi mortali!{#Name="Soldier B"}`
Show 'em what we mortals are made of!{#Name="Soldier B"}

`Eccola! Caricaaaa!{#Name="Soldier C"}`
There she is! Chaaaarge!{#Name="Soldier C"}

`Aaaaaaaargh!{#Name="Soldier D"}`
Aaaaaaaargh!{#Name="Soldier D"}

`L'unico modo per vincere la paura è caricare in avanti! Fatelo!{#Name="Soldier C"}`
The only way to overcome your fear is to charge forward! Do it!{#Name="Soldier C"}

`Vieni qui, demonio, così posso ucciderti!{#Name="Soldier E"}`
Get over here, demon, so I can kill you!{#Name="Soldier E"}

`Attento a ciò che desideri, idiota! Ricorda, è un'Intoner!{#Name="Soldier F"}`
Watch what you wish for, fool! Remember, she's an Intoner!{#Name="Soldier F"}

`Un'Intoner oscura decisa a distruggere il mondo...{#Name="Soldier G"}`
A dark Intoner bent on destroying the world...{#Name="Soldier G"}

`14 aprile, anno 1000 d.C.{#Name="Zero"}`
April the 14th, 1000 A.D.{#Name="Zero"}

`Stavamo inseguendo Three nella Terra delle Foreste,{#Name="Zero"}`
We were pursuing Three in the Land of Forests,{#Name="Zero"}

`ma dopo aver ottenuto un'informazione vitale da Cent,{#Name="Zero"}`
but after obtaining a vital piece of info from Cent,{#Name="Zero"}

`decidemmo di tornare tra le montagne...{#Name="Zero"}`
we decided to head back into the mountains...{#Name="Zero"}

`Nnngh! Qui si gela!{#Name="Dito"}`
Nnngh! It's freezing here!{#Name="Dito"}

`Sarà meglio che tu non ti sbagli su questa storia, Cent.{#Name="Zero"}`
You better not be wrong about this, Cent.{#Name="Zero"}

`Io? Sbagliarmi? Ha! Che assurdità.{#Name="Cent"}`
Me? Wrong? Ha! How absurd.{#Name="Cent"}

`Il frutto più spettacolarmente delizioso{LF}del mondo è prooooprio più avanti!{#Name="Cent"}`
The most spectacularly delicious fruit{LF}in the world is juuuuuust ahead!{#Name="Cent"}

`Nessun frutto vale la pena di arrancare{LF}in mezzo a tutto questo ghiaccio di merda.{#Name="Dito"}`
No fruit is worth trompin'{LF}through all this ice and shit.{#Name="Dito"}

`Se avessi saputo che faceva così freddo,{LF}sarei rimasto nella foresta{LF}e avrei dato la caccia alle sorelle di Z.{#Name="Dito"}`
If I'd known it would be this cold,{LF}I'd have stayed in the forest{LF}and tracked down Z's sisters.{#Name="Dito"}

`Beh, non si può combattere a{LF}stomaco vuoto o...come si dice.{#Name="Zero"}`
Well, you can't fight on an empty{LF}stomach or...whatever.{#Name="Zero"}

`Yaaay! Come si dice! Come si dice è il mio preferito!{#Name="Mikhail"}`
Yaaay! Whatever! Whatever is my favorite!{#Name="Mikhail"}

`Ma sarà meglio che sia il miglior{LF}dannato frutto che abbia mai assaggiato.{#Name="Zero"}`
But this better be the best{LF}damn fruit I ever tasted.{#Name="Zero"}

`Non preoccuparti. E' delizioso oltre ogni paragone!{#Name="Cent"}`
Don't you worry. It is delicious beyond compare!{#Name="Cent"}

`Il frutto si chiama mendesian. Delizioso, no?{#Name="Cent"}`
The fruit is called a mendesian. Delightful, no?{#Name="Cent"}

`Mangiato da solo, ha un sapore pulito,{LF}fresco e profumato di primavera.{#Name="Cent"}`
Eaten alone, it has a clean,{LF}crisp taste redolent of spring.{#Name="Cent"}

`Ma si può anche trasformare in una ricca{LF}marmellata da spalmare su pane o carne!{#Name="Cent"}`
But you can also make it into a rich{LF}jam to spread on bread or meat!{#Name="Cent"}

`E nutriente? Oh, santo cielo, sì!{#Name="Cent"}`
And nutritious? Oh my heavens, yes!{#Name="Cent"}

`Molto più di un frutto normale, aggiungerei.{#Name="Cent"}`
Far more so than your typical fruit, I might add.{#Name="Cent"}

`Dimmi, fa bene anche al vecchio serpente nei pantaloni?{#Name="Octa"}`
Tell me, is it any good for the old trouser snake?{#Name="Octa"}

`Bene? E' semplicemente spettacolare!{#Name="Cent"}`
Good? It's nothing short of spectacular!{#Name="Cent"}

`Oh ho ho! Beh, questa sì che è una bella notizia.{#Name="Octa"}`
Oh ho ho! Well, there's a piece of fine news.{#Name="Octa"}

`Sento già la mia mem—ehm,{LF}le mie emozioni agitarsi al solo pensiero!{#Name="Octa"}`
I can already feel my mem—er,{LF}emotions stirring at the thought!{#Name="Octa"}

`Cerbero, eh?{#Name="Dito"}`
Cerberus, huh?{#Name="Dito"}

`Ehi, secondo voi ha un buon sapore?{#Name="Dito"}`
Hey, you think he tastes any good?{#Name="Dito"}

`Chi può dirlo? Dubito che molti abbiano mai{LF}avuto l'occasione di assaggiarne uno.{#Name="Octa"}`
Who can say? I doubt many ever{LF}had the chance to sample one.{#Name="Octa"}

`Ne ho mangiato uno una volta. Non male. Pieno di cartilagine.{#Name="Zero"}`
I ate one once. Not bad. Lots of gristle.{#Name="Zero"}

`La cartilagine era la parte migliore!{#Name="Mikhail"}`
The gristle was the best part!{#Name="Mikhail"}

`Nnngh... Lady Zero, la tua dieta{LF}è una cosa sconcertante.{#Name="Decadus"}`
Nnngh... Lady Zero, your diet{LF}is a bewildering thing.{#Name="Decadus"}

`Maledizione, fa freddo! Dov'è il tuo{LF}stupido mendesian?{#Name="Zero"}`
Goddammit, it's cold! Where's your{LF}stupid mendesian already?{#Name="Zero"}

`Oh, siamo vicini! Vicinissimi. {#Name="Cent"}`
Oh, we're close! Ever so close. {#Name="Cent"}

`Lo trasformerò in marmellata e lo mangerò{LF}a cucchiaiate!{#Name="Decadus"}`
I'm going to turn it into jam and eat it{LF}by the spoonful!{#Name="Decadus"}

`Io me lo spalmerò tutto addosso su...ehm...su di me!{#Name="Octa"}`
I'm gonna rub it all over my...uh...self!{#Name="Octa"}

`541{#Name="541"}`
541{#Name="541"}

`Non posso credere che ci stiamo davvero fidando{LF}di questo idiota...{#Name="Dito"}`
I can't believe we're actually trusting{LF}this idiot...{#Name="Dito"}

`Ah!{#Name="Cent"}`
Ah!{#Name="Cent"}

`Quel forziere lì!{#Name="Cent"}`
That treasure chest there!{#Name="Cent"}

`...Oh cielo.{#Name="Cent"}`
...Oh dear.{#Name="Cent"}

`...Ma che diavolo?{#Name="Zero"}`
...What the hell?{#Name="Zero"}

`Ah... Ah ha ha! Sììì, credo di essermi{LF}sbagliato un tantino su questa cosa...{#Name="Cent"}`
Ah... Ah ha ha! Yeaaah, I'm thinking I may{LF}have been a little off on this...{#Name="Cent"}

`Oh, mi sono appena ricordato! In realtà è{LF}nel forziere più avanti!{#Name="Cent"}`
Oh, I just remembered! It's actually{LF}in the chest up ahead!{#Name="Cent"}

`Ho un brutto presentimento.{#Name="Dito"}`
I got a bad feeling about this.{#Name="Dito"}

`Proprio qui! Questa volta è sicuramente questo!{#Name="Cent"}`
Right here! This is it for sure!{#Name="Cent"}

`Il forziere era vuoto.{#Name=" "}`
The treasure chest was empty.{#Name=" "}

`{06}>{#Name="Dito"}`
{06}>{#Name="Dito"}

`{06}>{#Name="Octa"}`
{06}>{#Name="Octa"}

`{06}>{#Name="Decadus"}`
{06}>{#Name="Decadus"}

`Okay. Decisione esecutiva.{#Name="Zero"}`
Okay. Executive decision.{#Name="Zero"}

`La prossima volta che vediamo un cerberus,{LF}gli diamo Cent in pasto.{#Name="Zero"}`
Next time we see a cerberus,{LF}we're gonna feed it Cent.{#Name="Zero"}

`仕方ないじゃないですかぁあ。{LF}食べたら美味しいのは、事実なんですよぉお。{#Name="セント"}`
仕方ないじゃないですかぁあ。{LF}食べたら美味しいのは、事実なんですよぉお。{#Name="セント"}

`私達は食べそびれたわけだから、{LF}その事実はどうでもいいんじゃないか？{#Name="デカート"}`
私達は食べそびれたわけだから、{LF}その事実はどうでもいいんじゃないか？{#Name="デカート"}

