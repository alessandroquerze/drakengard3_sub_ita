`あ、そうだ！　{LF}キングはぁあ、パスタにまぜても……{#Name="セント"}`
あ、そうだ！　{LF}キングはぁあ、パスタにまぜても……{#Name="セント"}

`だから！　現物がないのに{LF}料理方法を教えてくださらんで結構！{#Name="オクタ"}`
だから！　現物がないのに{LF}料理方法を教えてくださらんで結構！{#Name="オクタ"}

`487{#Name="487"}`
487{#Name="487"}

`Okay. Decisione esecutiva.{#Name="Zero"}`
Okay. Executive decision.{#Name="Zero"}

`La prossima volta che vediamo un cerbero,{LF}gli diamo in pasto Cent.{#Name="Zero"}`
Next time we see a cerberus,{LF}we're gonna feed it Cent.{#Name="Zero"}

`Ho un brutto presentimento.{#Name="Dito"}`
I got a bad feeling about this.{#Name="Dito"}

`So di averlo già detto, ma se non cambi{LF}direzione presto, finirà male.{#Name="Accord"}`
I know I've said it before, but if you don't change{LF}course soon, things are not going to end well.{#Name="Accord"}

`E' quello che dice il tuo libriccino?{#Name="Zero"}`
That what your little book says?{#Name="Zero"}

`Esatto.{#Name="Accord"}`
Indeed.{#Name="Accord"}

`...Quindi non hai intenzione di farmelo vedere?{#Name="Zero"}`
...So you're not gonna show it to me?{#Name="Zero"}

`Mi è stato ordinato di non interferire con il mondo.{#Name="Accord"}`
I've been instructed not to interfere with the world.{#Name="Accord"}

`Bugiarda.{#Name="Zero"}`
Liar.{#Name="Zero"}

`No, è vero!{#Name="Accord"}`
No, it's true!{#Name="Accord"}

`Ma... be', nessuno ha detto che non posso dare almeno qualche piccolo consiglio, no?{#Name="Accord"}`
But...well, nobody said I couldn't give little bits of advice, at least?{#Name="Accord"}

`Cosa diavolo sei, in ogni caso?{#Name="Zero"}`
What the hell are you, anyway?{#Name="Zero"}

`Siamo Recorder.{#Name="Accord"}`
We're Recorders.{#Name="Accord"}

`Recorder?{#Name="Zero"}`
Recorders?{#Name="Zero"}

`Esatto. Il nostro compito è registrare gli eventi del mondo,{LF}dal remoto passato al lontano futuro...{#Name="Accord"}`
Right. It's our job to record world events,{LF}from the ancient past to the distant future...{#Name="Accord"}

`Hai presente quel "Vecchio Mondo" di cui voi parlate sempre?{#Name="Accord"}`
You know the "Old World" you people are always going on about?{#Name="Accord"}

`Be', sono stata mandata da gente di lì.{#Name="Accord"}`
Well, I was sent by folks from there.{#Name="Accord"}

`Senti, io non so un cazzo del Vecchio Mondo.{#Name="Zero"}`
Listen, I don't know jack about the Old World.{#Name="Zero"}

`Ma se mi segui solo per fare registrazioni{LF}o roba del genere, puoi anche piantarla, cazzo.{#Name="Zero"}`
But if you're following me just to make recordings{LF}or whatever, you can knock it the fuck off.{#Name="Zero"}

`Be', certo, in teoria dovrei fare solo quello...{#Name="Accord"}`
Well, sure, that's all I'm supposed to be doing...{#Name="Accord"}

`Ma dopo tutti questi brutti finali in cui continui a imbatterti,{LF}sono davvero tentata di aiutarti.{#Name="Accord"}`
But after all these bad endings you keep encountering,{LF}I'm really tempted to help.{#Name="Accord"}

`Ora, cerchiamo di chiudere la questione in un modo che convenga a tutti, hmm?{#Name="Accord"}`
Now, let's try and wrap things up in a way that benefits all of us, hmm?{#Name="Accord"}

`Come se avessi bisogno che me lo dicessi tu. Posso chiudere la questione da sola.{#Name="Zero"}`
Like I need you to tell me. I can wrap things up myself.{#Name="Zero"}

`Sconfiggi il Cerbero grigio!{#Name="説明文"}`
Defeat the Gray Cerberus!{#Name="説明文"}

`Sconfiggi i nemici che ti bloccano la strada!{#Name="説明文"}`
Defeat the enemies blocking your way!{#Name="説明文"}

`Cerca il prezioso frutto di Cent.{#Name="説明文"}`
Search for Cent's precious fruit.{#Name="説明文"}

`次のシーケンスに移行しますか？{#Name=" "}`
次のシーケンスに移行しますか？{#Name=" "}

`Non... ce la faccio più... Zzz...{#Name="Dito"}`
Can't...take this anymore... Zzz...{#Name="Dito"}

`Stufato... Così caldo e buono... Zzz...{#Name="Dito"}`
Stew... So nice and warm... Zzz...{#Name="Dito"}

`Zzz... Mmmngh... C-Cosa? Oh, cielo!{#Name="Decadus"}`
Zzz... Mmmngh... H-Huh? Oh, my!{#Name="Decadus"}

`Zzz... Unnngh... Marmellata...tutta...sulla faccia...{#Name="Decadus"}`
Zzz... Unnngh... Jam...all over...face...{#Name="Decadus"}

`Oh ho ho... Ammirate il mio... tronco gigantesco... Zzzzzz...{#Name="Octa"}`
Oh ho ho... Behold my...massive trunk... Zzzzzz...{#Name="Octa"}

`Oh... Non avete...ancora visto niente... Zzzzzz...{#Name="Octa"}`
Oh... You haven't...seen anything yet... Zzzzzz...{#Name="Octa"}

`Zzz... Mrrmm... Che...buono...{#Name="Mikhail"}`
Zzz... Mrrmm... Yum...my...{#Name="Mikhail"}

`Zzz... Mrrmm... Zero... Zero...{#Name="Mikhail"}`
Zzz... Mrrmm... Zero... Zero...{#Name="Mikhail"}

`Zzz... Eh? Stai...andando a dormire...Lady Zero?{#Name="Cent"}`
Zzz... Huh? You're...going to sleep...Lady Zero?{#Name="Cent"}

`Che...strano... Zzz...{#Name="Cent"}`
How...strange... Zzz...{#Name="Cent"}

`Meglio che mi riposi anch'io...{#Name="Zero"}`
I'd better get some rest, too...{#Name="Zero"}

`Concedersi un sonno davvero necessario?{#Name=" "}`
Get some much-needed sleep?{#Name=" "}

`Ti ammazzo! Giuro che ti ammazzo!{#Name="Soldier C"}`
I'll kill you! I swear I'll kill you!{#Name="Soldier C"}

`Bene, adesso... abbiamo questa registrazione della Diramazione B.{#Name="Accord"}`
All right, next up...we've got this recording from Branch B.{#Name="Accord"}

`No, non quella- quella è la Diramazione C!{#Name="Accord"}`
No, not that one—that's Branch C!{#Name="Accord"}

`E QUELLA è la Diramazione A! Andiamo!{#Name="Accord"}`
And THAT'S Branch A! Come on!{#Name="Accord"}

`Ecco qua. Questa è la Diramazione B! Finalmente...{#Name="Accord"}`
There we go. That's Branch B! Finally...{#Name="Accord"}

`D'accordo! Diamo un'occhiata alla registrazione M2410_E0005_FL...{#Name="Accord"}`
Okay then! Let's have a look at recording M2410_E0005_FL...{#Name="Accord"}

`Mmm... sì. Non è nemmeno questa...{#Name="Cent"}`
Mmm...yeah. It's not this one either...{#Name="Cent"}

`Uhm... Forse sulla montagna accanto?{#Name="Cent"}`
Um... Maybe the next mountain over?{#Name="Cent"}

`Oooo forse nella nazione accanto?{#Name="Cent"}`
Orrrr maybe the next nation over?{#Name="Cent"}

`...Cent?{#Name="Zero"}`
...Cent?{#Name="Zero"}

`Sììì?{#Name="Cent"}`
Yeees?{#Name="Cent"}

`Ti stai inventando queste cazzate, vero?{#Name="Zero"}`
You're just making this shit up, aren't you?{#Name="Zero"}

`Io? No, no! Mai! No! ...No. {#Name="Cent"}`
Me? No, no! Never! No! ...No. {#Name="Cent"}

`E' sicuramente, probabilmente, da qualche parte{LF}su questo continente.{#Name="Cent"}`
It's definitely, probably, somewhere{LF}on this continent.{#Name="Cent"}

`Ora basta! Ti strapperò il{LF}cuore dal culo!{#Name="Dito"}`
That's it! I'm gonna pull your{LF}heart out your asshole!{#Name="Dito"}

`Oh, ehi! Adesso ricordo! E' proprio laggiù!{#Name="Cent"}`
Oh, hey! Now I remember! It's right over there!{#Name="Cent"}

`Malconci ed esausti dopo aver seguito{LF}Cent come un branco di idioti,{#Name="Zero"}`
Battered and exhausted after following{LF}Cent like a bunch of idiots,{#Name="Zero"}

`fummo costretti a ripararci{LF}sul fianco innevato di una montagna.{#Name="Zero"}`
we were forced to take shelter{LF}on a snowy mountainside.{#Name="Zero"}

`Spazzatura inutile ottenuta.{#Name="説明文"}`
Worthless trash obtained.{#Name="説明文"}

`...Ottenuta altra spazzatura ancora più inutile.{#Name="説明文"}`
...Even more worthless trash obtained.{#Name="説明文"}

`Soldato A`
Soldier A

`Soldato B`
Soldier B

`Soldato C`
Soldier C

`Soldato D`
Soldier D

`Soldato E`
Soldier E

`Soldato F`
Soldier F

`Soldato G`
Soldier G

`兵士C`
兵士C

`兵士D`
兵士D

`兵士G`
兵士G

`兵士F`
兵士F

`Zero`
Zero

`Dito`
Dito

`Cent`
Cent

`Mikhail`
Mikhail

`Octa`
Octa

`Decadus`
Decadus

`541`
541

` `
 

`セント`
セント

`デカート`
デカート

`オクタ`
オクタ

`487`
487

`Accord`
Accord

`説明文`
説明文

`Sfonda le linee nemiche.{#Name="説明文"}`
Break through the enemy lines.{#Name="説明文"}

`Sconfiggi i non morti!{#Name="説明文"}`
Defeat the Undead!{#Name="説明文"}

`Sconfiggi il Gigas non morto!{#Name="説明文"}`
Defeat the Undead Gigas!{#Name="説明文"}

`Oh Dio, è qui!{#Name="Soldier A"}`
Oh God, she's here!{#Name="Soldier A"}

`Chi?{#Name="Soldier B"}`
Who?{#Name="Soldier B"}

`Lei! Hai presente... quella!{#Name="Soldier A"}`
Her! You know...the one!{#Name="Soldier A"}

`Quella cosa? Parla chiaro, idiota!{#Name="Soldier B"}`
The one what? Speak English, you idiot!{#Name="Soldier B"}

`Che tu sia maledetta, demone! Ti combatterò finché avrò vita in corpo!{#Name="Soldier F"}`
Curse you, demon! I'll fight you as long as I've got life in me!{#Name="Soldier F"}

`La nostra foresta sarà perduta...{#Name="Soldier E"}`
Our forest will be lost...{#Name="Soldier E"}

`Non ce l'hanno ancora! Continuate a combattere!{#Name="Soldier B"}`
They don't have it yet! Keep fighting!{#Name="Soldier B"}

`Quella diabolica cantatrice è terrificante... Credo che vomiterò!{#Name="Soldier G"}`
That devilish songstress is horrifying... I think I'm gonna be sick!{#Name="Soldier G"}

`Riprenditi, uomo!{#Name="Soldier E"}`
Pull yourself together, man!{#Name="Soldier E"}

`Cazzo! Dobbiamo passare all'offensiva!{#Name="Soldier A"}`
Dammit! We have to go on the offensive!{#Name="Soldier A"}

`Siamo fuori formazione!{#Name="Soldier G"}`
We're out of formation!{#Name="Soldier G"}

`Tenete la posizione! La traditrice deve essere punita per i suoi crimini!{#Name="Soldier A"}`
Hold your ground! The traitor must be punished for her crimes!{#Name="Soldier A"}

`Soldato ferito in arrivo! C'è un medico qui?{#Name="Soldier D"}`
Wounded soldier coming through! Is there a medic here?{#Name="Soldier D"}

`Che spettacolo orribile...{#Name="Soldier F"}`
What a horrifying sight...{#Name="Soldier F"}

`E' un drago! E sta...attaccando!{#Name="Soldier G"}`
It's a dragon! And it's...attacking!{#Name="Soldier G"}

`Ci ritiriamo? Avanziamo!? Ci servono ordini!{#Name="Soldier E"}`
Do we retreat? Do we advance!? We need our orders!{#Name="Soldier E"}

