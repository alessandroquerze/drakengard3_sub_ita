`Allora salirò al cielo{LF}senza rimpianti! Oh ho ho!{#Name="Octa"}`
Then I shall ascend to heaven{LF}without regrets! Oh ho ho!{#Name="Octa"}

`Puoi anche ridere, ma sogno{LF}di vivere un giorno in pace.{#Name="Decadus"}`
You may laugh at this, but I dream{LF}of one day living in peace.{#Name="Decadus"}

`Eppure...l'amara verità non mi sfugge.{LF}Nel mio cuore so di essere nato per combattere.{#Name="Decadus"}`
Yet...the bitter truth does not escape me.{LF}I know in my heart that I was born for combat.{#Name="Decadus"}

`Sono un discepolo, creato solo per servire un'Intoner.{#Name="Decadus"}`
I am a disciple, created only to serve an Intoner.{#Name="Decadus"}

`Sono d'accordo. Completamente.{#Name="Octa"}`
I agree. Completely.{#Name="Octa"}

`Allora che ne dite di chiudere il becco{LF}e mettervi a uccidere!{#Name="Dito"}`
Then how about you both shut up{LF}and make with the killin'!{#Name="Dito"}

`Mi hai tolto le parole dalla mia splendida bocca!{#Name="Cent"}`
You stole the words right from my beautiful mouth!{#Name="Cent"}

`Bene, ci siamo.{#Name="Zero"}`
Well, this is it.{#Name="Zero"}

`Ci siamo cosa, mia signora?{#Name="Octa"}`
It's what, my lady?{#Name="Octa"}

`Non avrò più bisogno di voi.{#Name="Zero"}`
I won't be needing you anymore.{#Name="Zero"}

`Posso occuparmi del resto da sola.{#Name="Zero"}`
I can handle the rest by myself.{#Name="Zero"}

`M-Mia signora, vi prego, se siete preoccupata per noi, noi—{#Name="Decadus"}`
M-My Lady, please, if you're worried about us, we—{#Name="Decadus"}

`No.{#Name="Zero"}`
No.{#Name="Zero"}

`E' inevitabile. I discepoli non possono mantenere{LF}forma umana senza il potere di un'Intoner.{#Name="Zero"}`
It's unavoidable. Disciples can't remain{LF}in human form without an Intoner's power.{#Name="Zero"}

`Prima o poi sparirete.{#Name="Zero"}`
Sooner or later, you'll disappear.{#Name="Zero"}

`Quindi, prima che accada...{#Name="Zero"}`
So before that happens...{#Name="Zero"}

`Vi restituisco le vostre vecchie forme.{#Name="Zero"}`
I'm giving you your old forms back.{#Name="Zero"}

`...Addio.{#Name="Zero"}`
...Goodbye.{#Name="Zero"}

`Zero è davvero qui!? Ma come!?{#Name="Soldier F"}`
Zero's actually here!? But how!?{#Name="Soldier F"}

`Tutti i soldati, in posizione!{#Name="Soldier A"}`
All soldiers, take position!{#Name="Soldier A"}

`Oh ho! Pare che il nemico non veda l'ora di uscire{LF}di scena in un tripudio di gloria!{#Name="Octa"}`
Oh ho! It seems the enemy's anxious to go out {LF}in a blaze of glory!{#Name="Octa"}

`Dirigiti alla Cattedrale.{#Name="ディト"}`
Head for the Cathedral.{#Name="ディト"}

`Contatto perso con la seconda unità!{#Name="Soldier G"}`
Contact lost with second unit!{#Name="Soldier G"}

`Resistete e proteggete i cancelli! Combattete!{#Name="Soldier B"}`
Stand strong and protect the gates! Fight!{#Name="Soldier B"}

`Aaargh! Mi hanno colpito! Qualcuno...vendicatemi...{#Name="Soldier C"}`
Aaargh! I'm hit! Someone...avenge me...{#Name="Soldier C"}

`No! Aspetta! Sopravvivi!{LF}Ehi, ci serve un medico qui!{#Name="Soldier D"}`
No! Wait! Stay alive!{LF}Hey, we need a medic over here!{#Name="Soldier D"}

`Una volta eliminati questi soldati,{LF}la Cattedrale dovrebbe essere vicina.{#Name="Decadus"}`
Once we clear these soldiers out,{LF}the Cathedral should be close.{#Name="Decadus"}

`Hmm? Stai chiedendo aiuto al piccolo vecchio me?{#Name="Cent"}`
Hmm? Are you asking for help from little old me?{#Name="Cent"}

`Beh, ha senso usare{LF}la freccia migliore nella faretra.{#Name="Cent"}`
Well, it does make sense to use{LF}the best arrow in your quiver.{#Name="Cent"}

`Sapevo che prima o poi sareste venuti strisciando{LF}dal caro vecchio Cent!{#Name="Cent"}`
I knew you'd come crawling to{LF}dear old Cent eventually!{#Name="Cent"}

`Octa? Posso chiederti un favore?{#Name="Decadus"}`
Octa? Can I ask you a favor?{#Name="Decadus"}

`Mi ignori, eh? Una simile timidezza{LF}è davvero sconveniente...{#Name="Cent"}`
Ignoring me, eh? Such bashfulness{LF}really is unbecoming...{#Name="Cent"}

`E' morto? Ehi, ehi, è morto?{LF}Whoa, guarda tutto quel sangue!{#Name="Dito"}`
Is he dead? Hey, hey, is he dead?{LF}Whoa, look at all the blood!{#Name="Dito"}

`Ha ha! Fammi un favore e lascia perdere i medici.^{#Name="Dito"}`
Ha ha! Do me a favor and skip the medics,{LF}would you?{#Name="Dito"}

`Fantastico. Un altro scherzo della natura.{#Name="Zero"}`
Great. Another freak of nature.{#Name="Zero"}

`Bestie volanti. Imp, immagino.{#Name="Octa"}`
Flying beasts. Imps, I would imagine.{#Name="Octa"}

`Si dice che banchettino con il fegato degli umani.{#Name="Octa"}`
They say they feast on the livers of humans.{#Name="Octa"}

`Hanno proprio una bella abitudine.{#Name="Zero"}`
That's quite the habit they've got there.{#Name="Zero"}

`Oh ho! Pare che il nemico non veda l'ora{LF}di uscire di scena in un tripudio di gloria!{#Name="Octa"}`
Oh ho! It seems the enemy's anxious{LF}to go out in a blaze of glory!{#Name="Octa"}

`Allora non deludiamo quei bastardi!{#Name="Dito"}`
Well then, let's not disappoint the bastards!{#Name="Dito"}

`ゼロ`
ゼロ

`オクタ`
オクタ

`デカート`
デカート

`説明文`
説明文

`Soldato G`
Soldier G

`Soldato B`
Soldier B

`Soldato A`
Soldier A

`Soldato E`
Soldier E

`Soldato C`
Soldier C

`Soldato D`
Soldier D

`Soldato F`
Soldier F

`兵士Ａ`
兵士Ａ

`兵士Ｂ`
兵士Ｂ

`兵士Ｃ`
兵士Ｃ

`兵士Ｇ`
兵士Ｇ

`兵士Ｅ`
兵士Ｅ

`Dito`
Dito

`Cent`
Cent

`Octa`
Octa

`セント`
セント

`Decadus`
Decadus

`ディト`
ディト

`Zero`
Zero

`Mikhail`
Mikhail

`Ai tempi, non lo sapevo.{#Name="Mikhail"}`
Back then, I didn't know.{#Name="Mikhail"}

`Non sapevo che sconfiggere One non fosse la fine.{#Name="Mikhail"}`
I didn't know that defeating One wasn't the end.{#Name="Mikhail"}

`E non sapevo nemmeno...{#Name="Mikhail"}`
I also didn't know...{#Name="Mikhail"}

`la verità su Zero e me...{#Name="Mikhail"}`
the truth about Zero and me...{#Name="Mikhail"}

`スタッフロール{#Name=" "}`
スタッフロール{#Name=" "}

`Ａルートクリア{#Name=" "}`
Ａルートクリア{#Name=" "}

`Ｂルート解除{#Name=" "}`
Ｂルート解除{#Name=" "}

`Infiltrati nella Cattedrale.{#Name=" "}`
Infiltrate the Cathedral.{#Name=" "}

`Sconfiggi i soldati nemici!{#Name=" "}`
Defeat the enemy soldiers!{#Name=" "}

`Sconfiggi i soldati nemici!{#Name=" "}`
Defeat the enemy soldiers!{#Name=" "}

`Sconfiggi i soldati nemici!{#Name=" "}`
Defeat the enemy soldiers!{#Name=" "}

`Sconfiggi Gabriel!{#Name=" "}`
Defeat Gabriel!{#Name=" "}

`Tutte le unità, ritirata! Ritirataaa!{#Name="Soldier A"}`
All units, retreat! Retreeeat!{#Name="Soldier A"}

`Sei impazzito? A cosa servirebbe scappare?{#Name="Soldier B"}`
Are you mad? What good'll running do?{#Name="Soldier B"}

`Non possiamo sconfiggerla...{#Name="Soldier A"}`
We can't defeat her...{#Name="Soldier A"}

`Nemico avvistato! Ma che...?{#Name="Soldier G"}`
Enemy sighted! What the...?{#Name="Soldier G"}

`Che c'è?{#Name="Soldier E"}`
What is it?{#Name="Soldier E"}

`E' sola. Zero è da sola!{#Name="Soldier G"}`
She's alone. Zero's by herself!{#Name="Soldier G"}

`E' ferita!{#Name="Soldier F"}`
She's wounded!{#Name="Soldier F"}

`Forza! Possiamo ancora farcela!{#Name="Soldier C"}`
Come on! We can still do this!{#Name="Soldier C"}

`Chiunque possa combattere, in prima linea! Difendeteci!{#Name="Soldier B"}`
Anyone who can fight, get to the front! Defend us!{#Name="Soldier B"}

`Sono ancora in piedi!{#Name="Soldier D"}`
I'm still in this!{#Name="Soldier D"}

`Anch'io!{#Name="Soldier C"}`
Me, too!{#Name="Soldier C"}

`Quindi quella è l'Intoner maledetta...{#Name="Soldier E"}`
So that's the accursed Intoner...{#Name="Soldier E"}

`Uccidetela! Uccidetela! Uccideeeetela!{#Name="Soldier A"}`
Kill her! Kill her! Kiiiiiill!{#Name="Soldier A"}

`Non sembra indebolita!{#Name="Soldier G"}`
She doesn't seem to be weakened!{#Name="Soldier G"}

`Dannata Intoner...{#Name="Soldier F"}`
Damned Intoner...{#Name="Soldier F"}

`Aiuto! Sta sfondando!{#Name="Soldier E"}`
Help! She's breaking through!{#Name="Soldier E"}

`Fermatela, costi quel che costi! Per Lady One!{#Name="Soldier B"}`
Stop her, no matter what! For Lady One!{#Name="Soldier B"}

`Aaaaahh!{#Name="Soldier F"}`
Aaaaahh!{#Name="Soldier F"}

`Basta! Stiamo sprecando vite di buoni soldati!{#Name="Soldier A"}`
Enough already! We're wasting good soldiers' lives!{#Name="Soldier A"}

`戦なんて、最初から全部無駄だ{#Name="兵士Ｆ"}`
戦なんて、最初から全部無駄だ{#Name="兵士Ｆ"}

`……前進！{#Name="兵士Ａ"}`
……前進！{#Name="兵士Ａ"}

`Se muoio qui, non avrò rimpianti!{#Name="Soldier C"}`
If I die here, I regret nothing!{#Name="Soldier C"}

`Lady One deve restare al sicuro! Altrimenti il mondo intero è in pericolo!{#Name="Soldier E"}`
Lady One must remain safe! If not, the whole world is in danger!{#Name="Soldier E"}

`ああ…ワン様を守りぬこう。それが、世界を守ることになる！{#Name="兵士Ｅ"}`
ああ…ワン様を守りぬこう。それが、世界を守ることになる！{#Name="兵士Ｅ"}

`Tutti i nostri soldati sono feriti!{#Name="Soldier F"}`
All our soldiers are wounded!{#Name="Soldier F"}

`Niente ritirata! Continuate ad avanzare!{#Name="Soldier B"}`
No retreat! Keep going!{#Name="Soldier B"}

`Porca merda, è Zero! E' qui!{#Name="Soldier G"}`
Holy shit, it's Zero! She's here!{#Name="Soldier G"}

`C'è qualcun altro?{#Name="Soldier A"}`
Anyone else?{#Name="Soldier A"}

`No, signore! Sembra essere sola!{#Name="Soldier G"}`
No, sir! She appears to be alone!{#Name="Soldier G"}

