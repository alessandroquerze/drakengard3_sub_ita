`Galgaliel sconfitti!+2`
Galgaliels

`Galgaliel sconfitti!+3`
Galgaliels

`Galgaliel sconfitti!+4`
Galgaliels

`Ezrael sconfitto!+1`
Ezrael

`Raphael sconfitto!+2`
Raphael

`Abdiel sconfitto!+1`
Abdiel

`Abdiel sconfitto!+2`
Abdiel

`Abdiel sconfitto!+3`
Abdiel

`Grossi Granchi Succosi`
Big, Juicy Crabs

`Piccoli Granchi Succosi`
Little Juicy Crabs

`Michael`
Michael

`Aeronavi sconfitte!+1`
Airships

`Barche Volanti sconfitte!+1`
Air Skiffs

`Navi Pirata degli Elfi Malvagi sconfitte!+1`
Evil Elf Pirate Ships

`-`
-

`One`
One

`One`
One

`Two`
Two

`Three`
Three

`Four`
Four

`Five`
Five

`ゼロ最終`
ゼロ最終

`Cent`
Cent

`Statue di Three`
Three Statues

`バリケード小`
バリケード小

`バリケード中`
バリケード中

`バリケード大`
バリケード大

`バリケード大 2`
バリケード大 2

`Cannoni distrutti!+1`
Cannons

`Cannoni Pesanti distrutti!+1`
Heavy Cannons

`Ipercannoni distrutti!+1`
Hyper Cannons

`戦艦大砲台`
戦艦大砲台

`戦艦長距離大砲台`
戦艦長距離大砲台

`ハイパー砲台透明`
ハイパー砲台透明

`ハイパー砲台 (アルマロス周囲)`
ハイパー砲台 (アルマロス周囲)

`ハイパー砲台 (アルマロス上部)`
ハイパー砲台 (アルマロス上部)

`大砲台(4020)`
大砲台(4020)

`浮遊石S`
浮遊石S

`ガーゴイルキューブ（OBJ）`
ガーゴイルキューブ（OBJ）

`壊せる人面木`
壊せる人面木

`壊せる蔦柱`
壊せる蔦柱

`壊せる木0`
壊せる木0

`壊せる木1`
壊せる木1

`壊せる蔦壁小`
壊せる蔦壁小

`壊せる蔦壁大`
壊せる蔦壁大

`引っ込む蔦`
引っ込む蔦

`Mezzi da Sbarco sconfitti!+1`
Landing Crafts

`Navi da Guerra sconfitti!+1`
Battleships

`木汎用`
木汎用

`ZH00 ゼロ家柱`
ZH00 ゼロ家柱

`アイテムボックス（木箱）`
アイテムボックス（木箱）

`ワイバーンの岩`
ワイバーンの岩

`宝箱`
宝箱

`爆弾（クエスト用）`
爆弾（クエスト用）

`Forzieri Metallici`
Metallic Chests

`Cannoni distrutti!+1`
Cannons

`Ipercannoni distrutti!+1`
Hyper Cannons

`Navi da Guerra sconfitte!+1`
Battleships

`Navi da Guerra sconfitte!+2`
Battleships

`Soldati sconfitti!+25`
Soldiers

`Torri Meridionali distrutte!+1`
Southern Towers

`Torri Orientali distrutte!+1`
Eastern Towers

`Torri Occidentali distrutte!+1`
Western Towers

`Torri Settentrionali distrutte!+1`
Northern Towers

`Soldati sconfitti!+26`
Soldiers

`Soldati Sconfitti!`
Soldiers

`Ogre Potenziati eliminati!+1`
Enhanced Ogres

`Imp Potenziati sconfitti!+1`
Enhanced Imps

`Cerberi Potenziati sconfitti!+1`
Enhanced Cerberuses

`Soldatesse sconfitte!+1`
Female Soldiers

`Attivo`
On

`Disattivo`
Off

`Max`
Max

`Min`
Min

`Impostazioni Controller`
Controller Settings

`Impostazioni Video`
Visual Settings

`Impostazioni Audio`
Audio Settings

`カメラ操作`
カメラ操作

`画面表示`
画面表示

`Ripristina Impostazioni Predefinite`
Restore Defaults

`Regola le impostazioni relative al controller.`
Adjust settings related to the controller.

`Regola le impostazioni relative alla schermata di gioco.`
Adjust settings related to the game screen.

`Regola le impostazioni relative al volume audio.`
Adjust settings related to audio volume.

`カメラの操作方式について設定します`
カメラの操作方式について設定します

`画面の表示物について設定します`
画面の表示物について設定します

`Ripristina tutte le impostazioni alla configurazione predefinita.`
Restore all settings to the default configuration.

`通常攻撃`
通常攻撃

`固有攻撃`
固有攻撃

`ジャンプ`
ジャンプ

`回避ステップ`
回避ステップ

`ロックオン`
ロックオン

`武器チェンジ`
武器チェンジ

`魔法`
魔法

`籠手チェンジ`
籠手チェンジ

`ロックオン方式`
ロックオン方式

`切り替えタイプ`
切り替えタイプ

`ホールドタイプ`
ホールドタイプ

`Comandi Battaglia Aerea`
Air Battle Controls

`Normale`
Normal

`Invertito`
Inverted

`Luminosità`
Brightness

`Volume Musica`
Music Volume

`Volume Effetti`
SFX Volume

`Volume Voci`
Voice Volume

`音質`
音質

