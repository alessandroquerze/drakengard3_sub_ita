`La tua fede vacilla! La tua blasfemia sarà punita!{#Name="Soldier E"}`
Your faith is lacking! Your blasphemy will be punished!{#Name="Soldier E"}

`Sì! La fede è tutto ciò che conta in questo mondo!{#Name="Soldier B"}`
Yes! Faith is all that matters in this world!{#Name="Soldier B"}

`Inchinati davanti a Lady Two! Implora il suo giusto perdono!{#Name="Soldier C"}`
Bow before Lady Two! Beg her righteous forgiveness!{#Name="Soldier C"}

`Questo mondo è oltre ogni salvezza! Nessuno di noi può più salvarlo.{#Name="Soldier D"}`
This world is beyond help! None of us can save it now.{#Name="Soldier D"}

`Oh, Dio, sono morto... Mi uccideranno!{#Name="Soldier G"}`
Oh, God, I'm so dead... They're gonna kill me!{#Name="Soldier G"}

`Credi in Lady Two e lei ti resusciterà nell'abbondanza dell'aldilà!{#Name="Soldier D"}`
Believe in Lady Two and she will resurrect you in the bounty of the afterlife!{#Name="Soldier D"}

`Credo in Lady Two, lo giuro! Ci credo, ci creeeedo!{#Name="Soldier G"}`
I believe in Lady Two, I swear it! I believe, I belieeeeve!{#Name="Soldier G"}

`Quelli che devono ancora risvegliarsi sono sempre pronti a gemere e urlare. Che pena!{#Name="Soldier F"}`
Those who have yet to awaken are always quick to wail and scream. How pathetic!{#Name="Soldier F"}

`Il vero mondo è immerso nella bellezza innocente di Lady Two!{#Name="Soldier F"}`
The true world is bathed in the innocent beauty of Lady Two!{#Name="Soldier F"}

`Se Two e One sono entrambe in città,{LF}mi farà risparmiare un po' di tempo per ucciderle.{#Name="Zero"}`
If Two and One are both in the city,{LF}that'll save me some killin' time.{#Name="Zero"}

`Lascia che... Lascia che ti aiuti anch'io.{#Name="Mikhail"}`
Let me... Let me help you out, too.{#Name="Mikhail"}

`Certo. Vola su e controlla più avanti, okay?{#Name="Zero"}`
Sure. Fly up and scout ahead, okay?{#Name="Zero"}

`C-Certo, Zero! Ci penso io!{#Name="Mikhail"}`
S-Sure, Zero! You got it!{#Name="Mikhail"}

`Aw, guardati mentre cerchi di tenerlo al sicuro.{#Name="Dito"}`
Aw, look at you trying to keep him safe.{#Name="Dito"}

`Sei proprio una tenerona, Z. Lo sai?{#Name="Dito"}`
You're just a big ol' cupcake, Z. You know that?{#Name="Dito"}

`Se Two e One sono entrambe in città,{LF}mi farà risparmiare un po' di tempo per ucciderle.{#Name="Zero"}`
If Two and One are both in the city,{LF}that'll save me some killin' time.{#Name="Zero"}

`Il miracolo è ancora vivo! Credete in Lady Two!{#Name="Soldier A"}`
The miracle is still alive! Believe in Lady Two!{#Name="Soldier A"}

`Se solo Zero si inchinasse alla saggezza divina di Lady Two!{#Name="Soldier C"}`
If only Zero would bow to Lady Two's divine wisdom!{#Name="Soldier C"}

`Lady Two è indistruttibile! Invincibile! Immortale!{#Name="Soldier F"}`
Lady Two is indestructible! Invincible! Immortal!{#Name="Soldier F"}

`Fuoco! Fuuuuuoco!{#Name="Soldier E"}`
Fire! Fiiiiiire!{#Name="Soldier E"}

`Si muovono troppo in fretta!{#Name="Soldier G"}`
They're moving too fast!{#Name="Soldier G"}

`Dov'è la vostra fede in Lady Two!?{LF}Lei ci proteggerà!{#Name="Soldier E"}`
Where's your faith in Lady Two!?{LF}She will protect us!{#Name="Soldier E"}

`Sono il 1597esimo uomo in fila per{LF}ricevere la salvezza di Lady Two!{#Name="Soldier D"}`
I'm the 1597th man in line to{LF}receive Lady Two's salvation!{#Name="Soldier D"}

`Chi sono questi tizi?{#Name="Zero"}`
Who are these guys?{#Name="Zero"}

`Un fanatismo simile ricorda i sintomi{LF}d'astinenza di certi medicinali.{#Name="Octa"}`
Such fanaticism resembles the withdrawal{LF}symptoms of certain medications.{#Name="Octa"}

`Cosa, quindi sono dipendenti?{LF}Da cosa sono dipendenti?{#Name="Zero"}`
What, so they're addicts?{LF}What are they addicted to?{#Name="Zero"}

`Dal suo canto, immagino.{#Name="Cent"}`
Her song, I'd imagine.{#Name="Cent"}

`Ehi, Cent ha appena detto qualcosa di sensato!{LF}Fategli un applauso, gente!{#Name="Dito"}`
Hey, Cent actually made sense!{LF}Give him a hand, everyone!{#Name="Dito"}

`Con Lady Two sparita, anche il potere{LF}del suo canto svanisce.{#Name="Decadus"}`
With Lady Two gone, the power{LF}of her song also vanishes.{#Name="Decadus"}

`Portando così al triste stato{LF}di quello che abbiamo davanti.{#Name="Decadus"}`
Thus leading to the sad state{LF}of affairs before us.{#Name="Decadus"}

`Stupido cane. Ti farò a pezzi!{#Name="Zero"}`
Stupid dog. I'm gonna rip you apart!{#Name="Zero"}

`Oh, per dio! Non un altro!{#Name="Zero"}`
Goddammit! Not another one!{#Name="Zero"}

`Temo che i nostri nemici abbiano individuato{LF}la debolezza di Lady Zero.{#Name="Decadus"}`
I fear our enemies have picked{LF}up on Lady Zero's weakness.{#Name="Decadus"}

`Debolezza?{#Name="Zero"}`
Weakness?{#Name="Zero"}

`Il vostro disgusto nel compiere lo stesso{LF}compito ancora e ancora e ancora.{#Name="Decadus"}`
Your dislike of performing the same{LF}task over and over and over again.{#Name="Decadus"}

`Chi te l'ha chiesto, stupido sottomesso!{#Name="Zero"}`
Who asked you, you freaky gimp!{#Name="Zero"}

`Mi hai chiamato, Zero? Oh ragazzi, oh ragazzi, oh ragazzi!{#Name="Mikhail"}`
Did you call me, Zero? Oh boy, oh boy, oh boy!{#Name="Mikhail"}

`Non fare quella faccia felice. Fai solo quello{LF}per cui sei qui. ...E{LF}quando hai finito, riposati un po'.{#Name="Zero"}`
Don't look so happy. Just do what{LF}you're here to do already. ...And{LF}when you're done, get some rest.{#Name="Zero"}

`Grazie, Zero! Ce la metterò tutta, okay?{#Name="Mikhail"}`
Thanks, Zero! I'll try double hard, okay?{#Name="Mikhail"}

`Possiamo raggiungere Cathedral City{LF}da questo sentiero più avanti?{#Name="Cent"}`
Can we get to the Cathedral City{LF}from this path up ahead?{#Name="Cent"}

`Hmm...{#Name="Accord"}`
Hmm...{#Name="Accord"}

`Draghi che regrediscono...{LF}Discepoli che abbandonano le loro Intoner...{#Name="Accord"}`
Dragons devolving...{LF}Disciples abandoning their Intoners...{#Name="Accord"}

`Una diramazione così strana merita uno studio molto attento.{#Name="Accord"}`
A branch this strange merits very careful study.{#Name="Accord"}

`Ci sono parecchi non morti da queste parti...{#Name="Octa"}`
Rather a lot of undead around here...{#Name="Octa"}

`Probabilmente stanno riciclando i{LF}corpi dei soldati caduti.{#Name="Decadus"}`
They're likely recycling the{LF}bodies of fallen soldiers.{#Name="Decadus"}

`Forse anche questo è un altro sintomo{LF}dell'assenza di Lady Two.{#Name="Octa"}`
Perhaps this is yet another symptom{LF}of Lady Two's absence.{#Name="Octa"}

`説明文`
説明文

`（リザーブ）`
（リザーブ）

`Soldato A`
Soldier A

`Soldato B`
Soldier B

`Soldato C`
Soldier C

`Soldato G`
Soldier G

`Soldato F`
Soldier F

`Soldato E`
Soldier E

`Soldato D`
Soldier D

`Zero`
Zero

`Mikhail`
Mikhail

`Dito`
Dito

`Octa`
Octa

`Cent`
Cent

`Decadus`
Decadus

`Accord`
Accord

`Cerca Three.{#Name=" "}`
Search for Three.{#Name=" "}

`Sconfiggi gli Ogre della sabbia!{#Name=" "}`
Defeat the Sand Ogres!{#Name=" "}

`Sconfiggi i nemici che ti bloccano la strada!{#Name=" "}`
Defeat the enemies blocking your way!{#Name=" "}

`595{#Name="595"}`
595{#Name="595"}

`60{#Name="60"}`
60{#Name="60"}

`253{#Name="253"}`
253{#Name="253"}

`307{#Name="307"}`
307{#Name="307"}

`379{#Name="379"}`
379{#Name="379"}

`487{#Name="487"}`
487{#Name="487"}

`271{#Name="271"}`
271{#Name="271"}

`433{#Name="433"}`
433{#Name="433"}

`415{#Name="415"}`
415{#Name="415"}

`109{#Name="109"}`
109{#Name="109"}

`あのアコールなる女が言うには{#Name="オクタ"}`
あのアコールなる女が言うには{#Name="オクタ"}

`649{#Name="649"}`
649{#Name="649"}

`235{#Name="235"}`
235{#Name="235"}

`60{#Name="60"}`
60{#Name="60"}

`いざとなったら食べさせるさ{#Name="ゼロ"}`
いざとなったら食べさせるさ{#Name="ゼロ"}

`私を{#Name="ゼロ"}`
私を{#Name="ゼロ"}

`181{#Name="181"}`
181{#Name="181"}

`60{#Name="60"}`
60{#Name="60"}

`127{#Name="127"}`
127{#Name="127"}

`271{#Name="271"}`
271{#Name="271"}

`325{#Name="325"}`
325{#Name="325"}

`217{#Name="217"}`
217{#Name="217"}

`307{#Name="307"}`
307{#Name="307"}

`109{#Name="109"}`
109{#Name="109"}

`Scommetterei che Lady Three si nasconde qui dentro.{#Name="Octa"}`
I would wager Lady Three is hiding in here.{#Name="Octa"}

`Perché?{#Name="Zero"}`
Why's that?{#Name="Zero"}

`Le sono sempre piaciuti i posti bui come questo.{#Name="Octa"}`
She always did enjoy dark places like this.{#Name="Octa"}

`Questo, e queste rovine sono l'unico rifugio qui vicino.{#Name="Decadus"}`
That, and these ruins are the only nearby refuge.{#Name="Decadus"}

`Giusto. Diamo un'occhiata.{#Name="Zero"}`
Right. Let's give it a look.{#Name="Zero"}

`Non vi avvicinerete a Lady Three!{#Name="Soldier B"}`
You're not getting near Lady Three!{#Name="Soldier B"}

`Oh sì, invece!{#Name="Zero"}`
Oh yes I am!{#Name="Zero"}

`Resta saldo, soldato!{#Name="Soldier A"}`
Stand firm, soldier!{#Name="Soldier A"}

`Sissignore!{#Name="Soldier G"}`
Yes, sir!{#Name="Soldier G"}

`Oh ho ho! Che coraggio sciocco!{#Name="Octa"}`
Oh ho ho! Such foolish courage!{#Name="Octa"}

`La nostra missione è proteggere Lady Three!{#Name="Soldier F"}`
Our mission is to guard Lady Three!{#Name="Soldier F"}

`Allora ti suggerisco di trovarti un altro lavoro!{#Name="Decadus"}`
Then I suggest you find another line of work!{#Name="Decadus"}

`どきなさい{#Name="デカート"}`
どきなさい{#Name="デカート"}

`どかない！　スリイ様をお守りするのが我らの仕事だッ{#Name="兵士F"}`
どかない！　スリイ様をお守りするのが我らの仕事だッ{#Name="兵士F"}

`Allora ti suggerisco di trovarti un altro lavoro!{#Name="Decadus"}`
Then I suggest you find another line of work!{#Name="Decadus"}

`Ughh! Mi sto stancando di queste teste di cazzo!{#Name="Zero"}`
Ughh! I'm getting sick of these shitheads!{#Name="Zero"}

