`Procedere alla prossima missione?{#Name=" "}`
Proceed to the next mission?{#Name=" "}

`みんなで一つの場所に集まるデモ{#Name=" "}`
みんなで一つの場所に集まるデモ{#Name=" "}

`Sembra che Lady Two riponga un'enorme fiducia nel suo discepolo.{#Name="Decadus"}`
It appears Lady Two places enormous trust in her disciple.{#Name="Decadus"}

`Perché lo dici davanti a Zero? Geloso?{#Name="Dito"}`
Why ya sayin' that in front of Zero? Jealous?{#Name="Dito"}

`N-No! Non era affatto mia intenzione insinuare che—{#Name="Decadus"}`
N-No! It was hardly my intention to imply that—{#Name="Decadus"}

`Non me ne frega proprio niente.{#Name="Zero"}`
I really don't care.{#Name="Zero"}

`...Oh. Molto bene, allora.{#Name="Decadus"}`
...Oh. Very well then.{#Name="Decadus"}

`Peccato, pervertito. Niente calci nelle palle per te oggi!{LF}Ha ha ha ha!{#Name="Dito"}`
Too bad, freak. No ball kicks for you today!{LF}Ha ha ha ha!{#Name="Dito"}

`Uuungh...{#Name="Decadus"}`
Uuungh...{#Name="Decadus"}

`Per i suoi soldati e per il suo discepolo, un'Intoner è una presenza divina.{#Name="Decadus"}`
To her soldiers and to her disciple, an Intoner is a godlike presence.{#Name="Decadus"}

`Che strano che Mikhail debba essere la tua rovina...{#Name="Octa"}`
How odd that Mikhail is to be your downfall...{#Name="Octa"}

`Non credere mai a ciò che ti dice il nemico.{#Name="Zero"}`
Never believe what your enemy tells you.{#Name="Zero"}

`Ma abbiamo visto tutti cosa ha fatto a Lady Three, un'altra Intoner.{#Name="Octa"}`
But we all saw what he did to Lady Three, a fellow Intoner.{#Name="Octa"}

`Vai a dormire e basta, ti va?{#Name="Zero"}`
Go to sleep already, would you?{#Name="Zero"}

`竜の行為は暴走ではないと？{#Name="オクタ"}`
竜の行為は暴走ではないと？{#Name="オクタ"}

`……もう寝ろ{#Name="ゼロ"}`
……もう寝ろ{#Name="ゼロ"}

`Puoi anche liquidarle, ma le mie preoccupazioni su Mikhail sono sincere...{#Name="Octa"}`
You may dismiss it, but my concerns about Mikhail are genuine...{#Name="Octa"}

`Ehi, quindi non sei preoccupata per niente?{LF}Per l'avvertimento di Cent, intendo.{#Name="Dito"}`
Hey, so you're not worried at all?{LF}About Cent's warning, I mean?{#Name="Dito"}

`Non mi ricordo nemmeno cosa abbia detto.{#Name="Zero"}`
I don't even remember what he said.{#Name="Zero"}

`Ha detto che quello stupido drago sarebbe stato la tua rovina!{#Name="Dito"}`
He said that dumb dragon would be your downfall!{#Name="Dito"}

`No, non sono preoccupata. ...Tu lo sei?{#Name="Zero"}`
No, I'm not worried. ...Are you?{#Name="Zero"}

`Io? Diavolo, no. La mia rovina è iniziata il giorno in cui ti ho incontrata!{#Name="Dito"}`
Me? Hell, no. My downfall started the day I met you!{#Name="Dito"}

`Carino.{#Name="Zero"}`
Cute.{#Name="Zero"}

`Non che importi a qualcuno, ma ho un bisogno tremendo di farmi un bagno...{#Name="Dito"}`
Not that anyone cares, but I need a bath something fierce...{#Name="Dito"}

`Allora, chi sa dov'è il Santuario delle Sabbie?{LF}...Qualcuno? ...Alzata di mani?{LF}...Prooooprio come pensavo.{#Name="Dito"}`
So who knows where the Shrine of Sands is?{LF}...Anyone? ...Show of hands?{LF}...Thaaat's what I thought.{#Name="Dito"}

`Immagino che dovremo solo vagare in giro come idioti{LF}finché non ci finiremo dentro per caso.{#Name="Zero"}`
Guess we'll just have to wander around like idiots{LF}until we stumble into it.{#Name="Zero"}

`Ho sentito dire che le rovine sotterranee qui vicino{LF}vengono usate come caserme dalle truppe nemiche.{#Name="Decadus"}`
I heard tell the nearby underground ruins are{LF}being used as barracks for the enemy troops.{#Name="Decadus"}

`Mi basta. Inizieremo da lì.{#Name="Zero"}`
Good enough. We'll start there.{#Name="Zero"}

`E io cosa devo fare?{#Name="Mikhail"}`
So what should I do?{#Name="Mikhail"}

`Tu vola in alto e fai una ricognizione, chiaro?{#Name="Zero"}`
You fly up and scout ahead, yeah?{#Name="Zero"}

`Ci penso io, Zero! Ci penso io!{#Name="Mikhail"}`
You got it, Zero! You got it!{#Name="Mikhail"}

`じゃあ、まずはそこからかな{#Name="ゼロ"}`
じゃあ、まずはそこからかな{#Name="ゼロ"}

`ボクはどうすればいいの？{#Name="ミハイル"}`
ボクはどうすればいいの？{#Name="ミハイル"}

`キミは上空からの情報収集を頼む{#Name="ゼロ"}`
キミは上空からの情報収集を頼む{#Name="ゼロ"}

`了解だよ！了解だよ！{#Name="ミハイル"}`
了解だよ！了解だよ！{#Name="ミハイル"}

`初回会話{#Name=" "}`
初回会話{#Name=" "}

`ループ会話{#Name=" "}`
ループ会話{#Name=" "}

`Mi sono appena ricordato una cosa. Un golem mi ha detto{LF}che Two è nel Santuario delle Sabbie!{#Name="Mikhail"}`
I just remembered something. A golem told me{LF}that Two is in the Shrine of Sands!{#Name="Mikhail"}

`Un golem te l'ha "detto"? I golem non parlano, scemo.{LF}Sono di pietra.{#Name="Zero"}`
A golem "told" you? Golems can't talk, dummy.{LF}They're stone.{#Name="Zero"}

`Beh, non ha parlato o roba del genere.{LF}E' stato più come se la sua anima mi avesse parlato.{#Name="Mikhail"}`
Well, he didn't talk or whatever.{LF}It was more like his soul spoke to me.{#Name="Mikhail"}

`L'anima non esiste.{#Name="Zero"}`
There's no such thing as a soul.{#Name="Zero"}

`Mi sono appena ricordato una cosa. Un golem mi ha detto{LF}che Two è nel Santuario delle Sabbie!{#Name="Mikhail"}`
I just remembered something. A golem told me{LF}that Two is in the Shrine of Sands!{#Name="Mikhail"}

`Cent ha affermato che il drago di Zero sarebbe stato la sua rovina.{#Name="Accord"}`
Cent claimed Zero's dragon would be her downfall.{#Name="Accord"}

`Qual era il significato dietro questa profezia sinistra?{#Name="Accord"}`
What was the meaning behind this ominous prophecy?{#Name="Accord"}

`In cerca di informazioni, Zero si diresse verso{LF}le rovine sotto la Terra delle Sabbie...{#Name="Accord"}`
In search of information, Zero traveled to{LF}the ruins beneath the Land of Sands...{#Name="Accord"}

` `
 

`Decadus`
Decadus

`Dito`
Dito

`Zero`
Zero

`Octa`
Octa

`オクタ`
オクタ

`ゼロ`
ゼロ

`Mikhail`
Mikhail

`ミハイル`
ミハイル

`Accord`
Accord

`初回会話{#Name="ディト"}`
初回会話{#Name="ディト"}

`ループ会話{#Name="ディト"}`
ループ会話{#Name="ディト"}

`初回会話{#Name="デカート"}`
初回会話{#Name="デカート"}

`ループ会話{#Name="デカート"}`
ループ会話{#Name="デカート"}

`初回会話{#Name="オクタ"}`
初回会話{#Name="オクタ"}

`ループ会話{#Name="オクタ"}`
ループ会話{#Name="オクタ"}

`次のステージが最後である説明。{#Name="セント"}`
次のステージが最後である説明。{#Name="セント"}

`ループ会話{#Name="セント"}`
ループ会話{#Name="セント"}

`初回会話{#Name="ミハイル"}`
初回会話{#Name="ミハイル"}

`ループ会話{#Name="ミハイル"}`
ループ会話{#Name="ミハイル"}

`次のミッションに移動しますか？{#Name=" "}`
次のミッションに移動しますか？{#Name=" "}

`みんなで一つの場所に集まるデモ{#Name=" "}`
みんなで一つの場所に集まるデモ{#Name=" "}

`ディト`
ディト

`デカート`
デカート

`オクタ`
オクタ

`セント`
セント

`ミハイル`
ミハイル

` `
 

`Procedere alla prossima missione?{#Name=" "}`
Proceed to the next mission?{#Name=" "}

`みんなで一つの場所に集まるデモ{#Name=" "}`
みんなで一つの場所に集まるデモ{#Name=" "}

`Hmm... Lo sento. Sììì. Lo seeento.{#Name="Octa"}`
Hmm... I can feel it. Yeees. Feeeeeel it.{#Name="Octa"}

`Non ora, Octa. Ho mal di testa.{#Name="Zero"}`
Not now, Octa. I've got a headache.{#Name="Zero"}

`Oh ho! No, mia signora, per una volta i miei pensieri sono altrove.{LF}Questa volta... ho la sensazione che Mikhail sia stato maledetto.{#Name="Octa"}`
Oh ho! No, my lady—for once, my thoughts are elsewhere.{LF}This time...I have a feeling Mikhail has been cursed.{#Name="Octa"}

`Pensi che una maledizione sia responsabile della sua regressione?{#Name="Decadus"}`
You think a curse is responsible for his devolution?{#Name="Decadus"}

`Figlio di... Allora chi è il bastardo responsabile?{#Name="Zero"}`
Son of a... Then who's the bastard responsible?{#Name="Zero"}

`Beh, fammi pensare... Hmmm... HMMM...{#Name="Dito"}`
Well, lemme think... Hmmm... HMMM...{#Name="Dito"}

`Oh, non lo so. Forse...{LF}L'INTONER CHE STIAMO CERCANDO DI UCCIDERE!?{#Name="Dito"}`
Oh, I dunno. Maybe...{LF}THE INTONER WE'VE BEEN TRYING TO KILL!?{#Name="Dito"}

`Two... la farò a pezzi!{#Name="Zero"}`
Two... I'm gonna crush her!{#Name="Zero"}

`Su, su, persino io mi prendo una pausa dalle ricerche carnali{LF}di tanto in tanto. {#Name="Octa"}`
Now, now, even I take a break from fleshly pursuits{LF}from time to time. {#Name="Octa"}

`Huh. Sì. Affascinante. Credo di essere stato{LF}con Lady Two in qualche momento del passato.{#Name="Cent"}`
Huh. Yes. Fascinating. I think I may have been{LF}with Lady Two sometime in the past.{#Name="Cent"}

`Avevo una specie di ruolo con lei.{LF}Tipo un servitore. O forse un maggiordomo.{#Name="Cent"}`
I had some kind of role with her.{LF}Like a servant. Or perhaps a butler.{#Name="Cent"}

`O magari un discepolo?{#Name="Dito"}`
Or a disciple, maybe?{#Name="Dito"}

`Oh, dai?{#Name="Zero"}`
Oh yeah?{#Name="Zero"}

`Beh, suppongo che si potrebbe dire, più o meno, forse?{#Name="Cent"}`
Well, I suppose you could kind of, sort of, maybe say that?{#Name="Cent"}

`Dov'è Lady Two adesso?{#Name="Decadus"}`
Where is Lady Two now?{#Name="Decadus"}

`E come diavolo farei a saperlo?{#Name="Cent"}`
How on earth would I know?{#Name="Cent"}

`L'ho vista l'ultima volta nelle rovine del deserto, maaa...{LF}cielo, è stato moltissimo tempo fa. {#Name="Cent"}`
I last saw her in the desert ruins, buuut...{LF}goodness, that was ages upon ages ago. {#Name="Cent"}

`Perché l'hai lasciata?{#Name="Zero"}`
Why'd you leave her?{#Name="Zero"}

`Oh, lo sai! Ogni uccello deve librarsi dalla{LF}gabbia dell'iniquità e... cose così.{#Name="Cent"}`
Oh, you know! Every bird has to soar from{LF}the cage of iniquity and...things.{#Name="Cent"}

`Non capisco metà delle stronzate che dici.{#Name="Zero"}`
I don't understand half the crap you say.{#Name="Zero"}

`Cielo. Non pronunciavo il nome{LF}"Lady Two" da parecchio tempo...{#Name="Cent"}`
Goodness. I haven't uttered the name{LF}"Lady Two" in quite some time...{#Name="Cent"}

`Ehi, Mikhail! Sei più bambinone di prima, eh?{#Name="Dito"}`
Hey, Mikhail! You're more of a big baby now than ever, huh?{#Name="Dito"}

`Non c'è bisogno di tanta crudeltà, Dito.{#Name="Decadus"}`
There is no need for such cruelty, Dito.{#Name="Decadus"}

`Sono tornato a essere inutile?{#Name="Mikhail"}`
Am I back to being useless?{#Name="Mikhail"}

`Affatto. Hai solo bisogno di riposo.{#Name="Decadus"}`
Not at all. You just need rest.{#Name="Decadus"}

`Bleh. Mi stai facendo venire il voltastomaco!{#Name="Dito"}`
Bleh. You're makin' me puke over here!{#Name="Dito"}

`La compassione vuota come quella finisce solo{LF}per far stare peggio la gente.{#Name="Dito"}`
Empty sympathy like that just ends up{LF}making people feel worse.{#Name="Dito"}

`181{#Name="181"}`
181{#Name="181"}

`343{#Name="343"}`
343{#Name="343"}

`Prima stavo morendo di freddo, ma qualcuno{LF}si è preoccupato del vecchio Dito? Noooooo...{#Name="Dito"}`
I was freezin' to death earlier, but did anyone{LF}worry about ol' Dito? Noooooo...{#Name="Dito"}

`Tutto questo è alquanto preoccupante.{#Name="Decadus"}`
This is all a tad worrisome.{#Name="Decadus"}

