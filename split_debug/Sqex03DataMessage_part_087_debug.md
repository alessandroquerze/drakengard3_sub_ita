`"Le radici di tutto questo male?"{LF}Sul serio, Z. Chi parla così?{#Name="Dito"}`
"The root of all this evil?"{LF}Seriously, Z. Who talks like that?{#Name="Dito"}

`Sta' zitto e combatti!{#Name="Zero"}`
Shut up and fight!{#Name="Zero"}

`487{#Name="487"}`
487{#Name="487"}

`Ne ho avuto abbastanza di voi stronzi{LF}e dei vostri stupidi cerchi magici!{#Name="Zero"}`
I've had enough of you jackasses{LF}and your stupid magic circles!{#Name="Zero"}

`Avanti, scappate pure! Morirete tutti comunque!{#Name="Zero"}`
Go ahead and run! You're all gonna die either way!{#Name="Zero"}

`寒すぎる！　クソ！　死ね！{#Name="ゼロ"}`
寒すぎる！　クソ！　死ね！{#Name="ゼロ"}

`説明文`
説明文

`（リザーブ）`
（リザーブ）

`Soldato A`
Soldier A

`Soldato B`
Soldier B

`Soldato G`
Soldier G

`Soldato E`
Soldier E

`Soldato C`
Soldier C

`Soldato D`
Soldier D

`Soldato F`
Soldier F

`Soldati C/D`
Soldiers C/D

`Zero`
Zero

`Dito`
Dito

`Decadus`
Decadus

`379`
379

`181`
181

`487`
487

`235`
235

`199`
199

`505`
505

`307`
307

`325`
325

`91`
91

`Octa`
Octa

`Cent`
Cent

`オクタ`
オクタ

`ディト`
ディト

`ゼロ`
ゼロ

`Mikhail`
Mikhail

`541`
541

`Accord`
Accord

`Procedi attraverso le rovine sotterranee.{#Name="説明文"}`
Proceed through the underground ruins.{#Name="説明文"}

`Sconfiggi i Troll Bruti!{#Name="説明文"}`
Defeat the Brute Trolls!{#Name="説明文"}

`Sconfiggi i Golem!{#Name="説明文"}`
Defeat the Golems!{#Name="説明文"}

`（リザーブ）{#Name="（リザーブ）"}`
（リザーブ）{#Name="（リザーブ）"}

`（リザーブ）{#Name="（リザーブ）"}`
（リザーブ）{#Name="（リザーブ）"}

`（リザーブ）{#Name="（リザーブ）"}`
（リザーブ）{#Name="（リザーブ）"}

`（リザーブ）{#Name="（リザーブ）"}`
（リザーブ）{#Name="（リザーブ）"}

`（リザーブ）{#Name="（リザーブ）"}`
（リザーブ）{#Name="（リザーブ）"}

`（リザーブ）{#Name="（リザーブ）"}`
（リザーブ）{#Name="（リザーブ）"}

`（リザーブ）{#Name="（リザーブ）"}`
（リザーブ）{#Name="（リザーブ）"}

`È Zero! Che sia dannata all'inferno!{#Name="Soldier A"}`
It's Zero! Damn her to hell!{#Name="Soldier A"}

`È terrificante! Niente a che vedere con la nostra Lady Two!{#Name="Soldier F"}`
She's horrifying! Nothing like our Lady Two!{#Name="Soldier F"}

`Eccola! Tutti gli uomini, in posizione!{#Name="Soldier B"}`
There she is! All men, take positions!{#Name="Soldier B"}

`Mi sacrificherò volentieri per Lady Two!{#Name="Soldier C"}`
I will gladly sacrifice myself for the sake of Lady Two!{#Name="Soldier C"}

`Anch'i— ehm, anch'io!{#Name="Soldier D"}`
Me to—er, as well!{#Name="Soldier D"}

`Non possiamo lasciarla avvicinare a Lady Two!{#Name="Soldier E"}`
We can't let her get anywhere near Lady Two!{#Name="Soldier E"}

`Lady Two non deve mai essere insozzata!{#Name="Soldier E"}`
Lady Two must never be befouled!{#Name="Soldier E"}

`Mandate notizia che si avvicina una minaccia! Lady Two è in pericolo!{#Name="Soldier A"}`
Send word a threat approaches! Lady Two is in danger!{#Name="Soldier A"}

`595{#Name="595"}`
595{#Name="595"}

`Date il 125 percento! Vi servirà tutto contro questo demonio!{#Name="Soldier C"}`
Give me 125 percent! You'll need it all against this devil!{#Name="Soldier C"}

`Zero è... È terrificante!{#Name="Soldier F"}`
Zero is... She's terrifying!{#Name="Soldier F"}

`Voglio un mondo in cui Lady Two possa essere sempre felice!{#Name="Soldier G"}`
I want a world where Lady Two can always be happy!{#Name="Soldier G"}

`Fermate il demonio qui e ora!{#Name="Soldier E"}`
Stop the demon here and now!{#Name="Soldier E"}

`Il momento è adesso! Ricordate l'addestramento, uomini!{#Name="Soldier F"}`
The time is now! Remember your training, men!{#Name="Soldier F"}

`Lady Two veglia su di noi! Ne sono sicuro!{#Name="Soldier D"}`
Lady Two watches over us! I'm sure of it!{#Name="Soldier D"}

`Aprite un varco! Stiamo trasportando dei feriti!{#Name="Soldier A"}`
Open a path! We're carrying wounded here!{#Name="Soldier A"}

`Non sono ancora morto... Ma ti porterò all'inferno con me!{#Name="Soldier B"}`
I'm not dead yet... But I'll take you to hell with me!{#Name="Soldier B"}

`Il nostro sacrificio qui garantirà la sopravvivenza di Lady Two!{#Name="Soldier E"}`
Our sacrifice here will ensure Lady Two lives on!{#Name="Soldier E"}

`È la fine del mondo... Siamo tutti impazziti!{#Name="Soldier F"}`
It's the end of the world... We've all gone mad!{#Name="Soldier F"}

`Come può questo demonio essere un'Intoner!?{#Name="Soldier E"}`
How can this demon be an Intoner!?{#Name="Soldier E"}

`Una...una falsa Intoner!?{#Name="Soldier C"}`
A...a false Intoner!?{#Name="Soldier C"}

`Chi lo sa! State solo attenti!{#Name="Soldier E"}`
Who knows! Just be careful!{#Name="Soldier E"}

`Quella donna è una vergogna per Lady Two e per tutto ciò che rappresenta!{#Name="Soldier E"}`
That woman is a disgrace to Lady Two and everything she stands for!{#Name="Soldier E"}

`Andrà tutto bene finché Lady Two sarà con noi!{#Name="Soldier G"}`
We'll be fine so long as Lady Two is with us!{#Name="Soldier G"}

`A dire il vero, un po' di equipaggiamento aiuterebbe!{#Name="Soldier E"}`
Actually, some equipment might help!{#Name="Soldier E"}

`Credete in Lady Two, e non proverete paura!{#Name="Soldier G"}`
Believe in Lady Two, and you will feel no fear!{#Name="Soldier G"}

`Di certo Lady Two non ci ha abbandonati, vero?{#Name="Soldier B"}`
Surely Lady Two hasn't forsaken us, has she?{#Name="Soldier B"}

`Non essere ridicolo!{#Name="Soldier A"}`
Don't be ridiculous!{#Name="Soldier A"}

`La nostra dea perfetta non ci tradirebbe mai! ...Non come certe ragazze che conosco, comunque!{#Name="Soldier A"}`
Our perfect goddess would never betray us! ...Not like some of the girls I know, anyway!{#Name="Soldier A"}

`Attenti! È Zero!{#Name="Soldier B"}`
Look out! It's Zero!{#Name="Soldier B"}

`Pensare che un'Intoner possa essere così vile!{#Name="Soldier F"}`
To think an Intoner could be so vile!{#Name="Soldier F"}

`Niente ritirata! Come potremmo mai guardare Lady Two in faccia!?{#Name="Soldier C"}`
No retreat! How would we ever face Lady Two!?{#Name="Soldier C"}

`Maledizione! Adesso ti butto giù!{#Name="Soldier D"}`
Dammit! You're going down!{#Name="Soldier D"}

`Non preoccupatevi delle posizioni! Combattete e basta!{#Name="Soldier E"}`
Don't worry about positioning! Just fight!{#Name="Soldier E"}

`Tutti gli uomini, avanti! Combattete per la vostra vita!{#Name="Soldier A"}`
All men, forward! Fight for your very lives!{#Name="Soldier A"}

`Yaaahhhhh!{#Name="Soldier G"}`
Yaaahhhhh!{#Name="Soldier G"}

`Cazzo! Dove diavolo è Two!?{#Name="Zero"}`
Dammit! Where the hell is Two!?{#Name="Zero"}

`Calma, Z. Non è che ce la{LF}consegneranno così, su due piedi.{#Name="Dito"}`
Easy, Z. It ain't like they're just{LF}gonna hand her over to us.{#Name="Dito"}

`Allora ce la prenderemo con la forza!{#Name="Zero"}`
Then we're just gonna have to take her!{#Name="Zero"}

`Lady Zero, percepisco qualcosa da Mikhail.{#Name="Decadus"}`
Lady Zero, I'm sensing something from Mikhail.{#Name="Decadus"}

`Merda. È già crollato?{#Name="Zero"}`
Crap. Did he keel over already?{#Name="Zero"}

`No, ma sta lottando.{LF}Temo sia vicino al limite.{#Name="Decadus"}`
No, but he is struggling.{LF}I fear he is near his limit.{#Name="Decadus"}

`Cent!{#Name="Zero"}`
Cent!{#Name="Zero"}

`Sììì?{#Name="Cent"}`
Yeees?{#Name="Cent"}

`Ho bisogno che tu pensi, okay?{LF}Dov'è Two? Qual è la sua debolezza?{#Name="Zero"}`
I need you to think, okay?{LF}Where's Two? What's her weakness?{#Name="Zero"}

`Devi sapere qualcosa. Forza! Sbrigati!{#Name="Zero"}`
You've gotta know something. Come on! Chop chop!{#Name="Zero"}

`Concedimi un momento, vuoi?{#Name="Cent"}`
Give me a moment, will you?{#Name="Cent"}

`Anche uno con un cervello enorme come il mio{LF}ha bisogno di tempo per elaborare, sai?{#Name="Cent"}`
Even someone with a massive brain like me{LF}needs time to process things, you know?{#Name="Cent"}

`Hai due secondi.{LF}Dopodiché comincio a prenderti a calci le palle.{#Name="Zero"}`
You've got two seconds.{LF}After that, I start kicking balls.{#Name="Zero"}

`Questi nemici sono piuttosto ostinati, hmm?{LF}Dobbiamo sbrigarci.{#Name="Octa"}`
These foes are quite stubborn, hmm?{LF}We must hurry.{#Name="Octa"}

`Vorrei poter bruciare queste rovine{LF}e tutti quelli che ci sono dentro.{#Name="Zero"}`
I wish I could burn these ruins{LF}and everyone in them.{#Name="Zero"}

`Oh ho ho! Sempre così impulsiva...{#Name="Octa"}`
Oh ho ho! Always so impulsive...{#Name="Octa"}

`Tuttavia, capisco. Devi essere{LF}preoccupata per il tuo drago.{#Name="Octa"}`
Still, I understand. You must be{LF}worried for your dragon.{#Name="Octa"}

`Non lo sono affatto!{#Name="Zero"}`
I am no such thing!{#Name="Zero"}

`Te la cavi, Z?{#Name="Dito"}`
You gonna be okay, Z?{#Name="Dito"}

`Che vorrebbe dire?{#Name="Zero"}`
What's that supposed to mean?{#Name="Zero"}

`Niente! È solo che...so come ti senti{LF}in situazioni di questo tipo...{#Name="Dito"}`
Nothing! It's just...I know how you feel{LF}about these types of situations...{#Name="Dito"}

`Tutto okay.{#Name="Zero"}`
I'm fine.{#Name="Zero"}

`Ci risiamo...{#Name="Cent"}`
Here we go again...{#Name="Cent"}

