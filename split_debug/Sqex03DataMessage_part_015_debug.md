` `
 

`Accord`
Accord

`Zero`
Zero

`Michael`
Michael

`ゼロ`
ゼロ

`Procedere alla prossima missione?{#Name=" "}`
Proceed to the next mission?{#Name=" "}

`みんなで一つの場所に集まるデモ{#Name=" "}`
みんなで一つの場所に集まるデモ{#Name=" "}

`Hm? Oh, vuoi sapere di più su di me?{LF}Beh, sono esattamente ciò che sembro!{#Name="Cent"}`
Hm? Oh, you wanna know more about me?{LF}Well, I'm exactly what I seem!{#Name="Cent"}

`Un magnifico, splendido esempio di uomo.{#Name="Cent"}`
A fine, beautiful example of a man.{#Name="Cent"}

`Trovo che il mio bell'aspetto spesso metta a disagio la gente comune,{LF}ma che ci vuoi fare?{#Name="Cent"}`
I find my good looks often turn off the little people,{LF}but what can you do?{#Name="Cent"}

`Posso prenderlo a pugni? Tipo, in faccia?{#Name="Dito"}`
Can I punch him? Like, in the face?{#Name="Dito"}

`Oh, sì! A proposito di pugni... è l'ora della curiosità!{#Name="Cent"}`
Oh, yes! Speaking of punching...it's trivia time!{#Name="Cent"}

`Avete presente l'espressione "porgere l'altra guancia", sì?{#Name="Cent"}`
You've heard the expression "turn the other cheek," yes?{#Name="Cent"}

`Beeene, non è solo un'espressione!{LF}Oh cielo, no.{#Name="Cent"}`
Weeell, it's not just an expression!{LF}Oh my, no.{#Name="Cent"}

`In realtà deriva da un antico testo medico su{LF}come regolare le linee della mandibola e le strutture ossee!{#Name="Cent"}`
It actually comes from an ancient medical text concerning{LF}how to adjust jaw lines and bone structures!{#Name="Cent"}

`Uh, davvero?{#Name="Dito"}`
Uh, really?{#Name="Dito"}

`Certo che no. Sta sparando stronzate.{#Name="Zero"}`
Of course not. He's full of shit.{#Name="Zero"}

`Dai, solo un pugno!{#Name="Dito"}`
Come on, just one punch!{#Name="Dito"}

`Allora? Siamo pronti per la grande partenza?{#Name="Cent"}`
Well? We ready for the big departure?{#Name="Cent"}

`Hmm... Che strano. Non riesco a smettere di tremare.{#Name="Octa"}`
Hmm... How odd. I can't seem to stop shaking.{#Name="Octa"}

`Che c'è? Nervoso?{#Name="Zero"}`
What's wrong? Nervous?{#Name="Zero"}

`Per quanto sia vergognoso ammetterlo...{#Name="Octa"}`
As shameful as it is to admit...{#Name="Octa"}

`Ma sa, my lady, i dottori dicono che una notte appassionata di{LF}incastri amorosi è la cura perfetta per simili mali! Mmm?{#Name="Octa"}`
But you know, my lady, doctors say a passionate night of{LF}log-jamming is the perfect cure for such ailments! Mmm?{#Name="Octa"}

`Lascia perdere.{#Name="Zero"}`
Forget it.{#Name="Zero"}

`Allora mi concederebbe tre giorni?{#Name="Octa"}`
Then give me three days, would you?{#Name="Octa"}

`Ho bisogno di 72 ore filate di piacere solitario prima{LF}di essere pronto al combattimento.{#Name="Octa"}`
I need 72 straight hours of self-pleasure before{LF}I'm ready for combat.{#Name="Octa"}

`No. Non se ne parla.{#Name="Zero"}`
Nope. Not happening.{#Name="Zero"}

`Siamo pronti ad andare? Oh ho ho ho!{#Name="Octa"}`
Ready to go yet? Oh ho ho ho!{#Name="Octa"}

`Cavolo, questa banda sta sfuggendo di mano.{LF}Parlano tutti bla-bla-bla per tutto il dannato tempo.{#Name="Dito"}`
Man, this crew's getting outta hand.{LF}Everyone's all yap-yap-yap all the freakin' time.{#Name="Dito"}

`Io personalmente apprezzo la compagnia.{LF}La trovo... vivace.{#Name="Decadus"}`
I personally enjoy the companionship.{LF}I find it...lively.{#Name="Decadus"}

`Vivace? Serve solo a farci notare più facilmente{LF}da qualche mostro!{#Name="Dito"}`
Lively? All it does is make us more likely to be spotted{LF}by some monster!{#Name="Dito"}

`Finché Lady Zero è al sicuro, nient'altro ha importanza.{#Name="Decadus"}`
So long as Lady Zero is safe, nothing else matters.{#Name="Decadus"}

`Oh, ora ho capito. Vuoi solo dare a Zero{LF}qualche scudo di carne in più, eh?{#Name="Dito"}`
Oh, now I get it. You just wanna give Zero{LF}a few more meat-shields, huh?{#Name="Dito"}

`Più discepoli significa più gente da maltrattare per lei.{LF}È così, mostriciattolo?{#Name="Dito"}`
More disciples means more people for her to abuse.{LF}That how it is, freakshow?{#Name="Dito"}

`Le amicizie finte mi fanno schifo.{#Name="Dito"}`
Pretend friendships make me sick.{#Name="Dito"}

`Il nostro viaggio è davvero diventato lungo.{#Name="Decadus"}`
Our journey has become long indeed.{#Name="Decadus"}

`Ho sempre saputo che lo sarebbe stato.{#Name="Zero"}`
I always knew it would.{#Name="Zero"}

`Naturalmente, my lady. Solo lei poteva prevedere simili cose.{#Name="Decadus"}`
Of course, my lady. Such things only you could have foreseen.{#Name="Decadus"}

`Prego che stanotte possa godere di un sonno profondo e ristoratore.{#Name="Decadus"}`
I pray you enjoy a deep and restful sleep tonight.{#Name="Decadus"}

`Resterò al suo fianco e la proteggerò{LF}da qualunque cosa possa accadere.{#Name="Decadus"}`
I shall remain by your side and guard you{LF}from whatever may occur.{#Name="Decadus"}

`Già. Farò del mio meglio.{#Name="Zero"}`
Right. I'll do my best.{#Name="Zero"}

`Sono ai suoi ordini, my lady.{LF}Farò qualunque cosa desideri.{#Name="Decadus"}`
I am yours to command, my lady.{LF}I shall do anything you desire.{#Name="Decadus"}

`È rimasta solo una sorella, eh, Zero?{#Name="Mikhail"}`
There's just one sister left, huh, Zero?{#Name="Mikhail"}

`Finalmente siamo arrivati a One.{#Name="Zero"}`
Finally down to One.{#Name="Zero"}

`Quindi immagino che ora la ucciderai, eh?{#Name="Mikhail"}`
So I guess you're gonna kill her now, huh?{#Name="Mikhail"}

`Hai qualche problema?{#Name="Zero"}`
You got a problem with that?{#Name="Zero"}

`È solo che... ormai abbiamo ucciso un sacco di persone, sai?{#Name="Mikhail"}`
It's just that...we've killed a lot of people now, you know?{#Name="Mikhail"}

`E allora?{#Name="Zero"}`
And?{#Name="Zero"}

`Beh, quando provo a dormire, a volte io...{LF}Riesco ancora a sentirli urlare.{#Name="Mikhail"}`
Well, when I try to sleep, sometimes I...{LF}I can still hear them screaming.{#Name="Mikhail"}

`Tu, Zero? Tu li senti urlare?{#Name="Mikhail"}`
Do you, Zero? Do you hear them scream?{#Name="Mikhail"}

`No.{#Name="Zero"}`
No, I don't.{#Name="Zero"}

`Wow, Zero. Sei davvero forte.{#Name="Mikhail"}`
Wow, Zero. You sure are strong.{#Name="Mikhail"}

`Uccideremo ancora...{#Name="Mikhail"}`
We're going to kill again...{#Name="Mikhail"}

`Andiamo?{#Name="Zero"}`
Shall we?{#Name="Zero"}

`Lady One è nella Cathedral City, protetta{LF}da una potente rete difensiva.{#Name="Octa"}`
Lady One's in the Cathedral City, which is protected{LF}by a strong defensive network.{#Name="Octa"}

`Perciò, credo che un'invasione aerea sarebbe la nostra strategia migliore.{#Name="Octa"}`
Thus, I believe an aerial invasion would be our best strategy.{#Name="Octa"}

`Huh. Beh, almeno servi a qualcosa.{#Name="Zero"}`
Huh. Well, at least you're good for something.{#Name="Zero"}

`Ma my lady, lei sa già che sono di grande utilità{LF}durante queste lunghe, buie notti!{#Name="Octa"}`
But my lady, you already know that I'm of great use{LF}during these long, dark nights!{#Name="Octa"}

`ははっ……{#Name="ゼロ"}`
ははっ……{#Name="ゼロ"}

`Quindi... combattiamo di nuovo?{#Name="Mikhail"}`
So...we're fighting again?{#Name="Mikhail"}

`Sì.{#Name="Zero"}`
Yep.{#Name="Zero"}

`Okay...{#Name="Mikhail"}`
Okay...{#Name="Mikhail"}

`Aggrappati a me, Zero.{#Name="Mikhail"}`
Hang on to me, Zero.{#Name="Mikhail"}

`73{#Name="73"}`
73{#Name="73"}

`451{#Name="451"}`
451{#Name="451"}

`343{#Name="343"}`
343{#Name="343"}

`271{#Name="271"}`
271{#Name="271"}

`361{#Name="361"}`
361{#Name="361"}

`163{#Name="163"}`
163{#Name="163"}

`60{#Name="60"}`
60{#Name="60"}

`109{#Name="109"}`
109{#Name="109"}

`199{#Name="199"}`
199{#Name="199"}

`Andiamo?{#Name="Zero"}`
Shall we?{#Name="Zero"}

`Lady One è nella Cathedral City, protetta{LF}da una potente rete difensiva.{#Name="Octa"}`
Lady One's in the Cathedral City, which is protected{LF}by a strong defensive network.{#Name="Octa"}

`Perciò, credo che un'invasione aerea sarebbe la nostra strategia migliore.{#Name="Octa"}`
Thus, I believe an aerial invasion would be our best strategy.{#Name="Octa"}

`Huh. Beh, almeno servi a qualcosa.{#Name="Zero"}`
Huh. Well, at least you're good for something.{#Name="Zero"}

`Ma my lady, lei sa già che sono di grande utilità{LF}durante queste lunghe, buie notti!{#Name="Octa"}`
But my lady, you already know that I'm of great use{LF}during these long, dark nights!{#Name="Octa"}

`Heh.{#Name="Zero"}`
Heh.{#Name="Zero"}

`Quindi... combattiamo di nuovo?{#Name="Mikhail"}`
So...we're fighting again?{#Name="Mikhail"}

`Sì.{#Name="Zero"}`
Yep.{#Name="Zero"}

`Okay...{#Name="Mikhail"}`
Okay...{#Name="Mikhail"}

`Aggrappati a me, Zero.{#Name="Mikhail"}`
Hang on to me, Zero.{#Name="Mikhail"}

` `
 

`Cent`
Cent

`Dito`
Dito

`Zero`
Zero

`Octa`
Octa

`Decadus`
Decadus

`Mikhail`
Mikhail

`ゼロ`
ゼロ

`73`
73

`451`
451

`343`
343

`271`
271

`361`
361

`163`
163

`60`
60

`109`
109

`199`
199

`-`
-

`2-colpi`
2-hit

`3-colpi`
3-hit

`4-colpi`
4-hit

`5-colpi`
5-hit

`6-colpi`
6-hit

`7-colpi`
7-hit

