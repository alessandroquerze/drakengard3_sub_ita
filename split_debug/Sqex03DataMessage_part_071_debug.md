 `Quel bambinone dei tuoi finisce spesso{LF}legato, eh.{#Name="Dito"}`
That big baby of yours sure{LF}does get tied up a lot.{#Name="Dito"}

`Se Lady Three lo prende,{LF}potrebbe fare esperimenti sul suo corpo.{#Name="Octa"}`
If Lady Three catches him, she{LF}may experiment on his body.{#Name="Octa"}

`E-Esperimenti sul corpo? Ungh... Sarebbe{LF}deliz... Ehm, voglio dire... detestabile...{#Name="Decadus"}`
B-Body experiments? Ungh... That would{LF}be delectib... Er, I mean...detestable...{#Name="Decadus"}

`Scusa se ho combinato un altro guaio, Zero. Cercherò di fare meglio...{#Name="Mikhail"}`
Sorry I messed up again, Zero. I'll be better...{#Name="Mikhail"}

`Sei stranamente composto.{#Name="Zero"}`
You're awfully reserved.{#Name="Zero"}

`Eh?{#Name="Mikhail"}`
Huh?{#Name="Mikhail"}

`Non stai dicendo "Aww!" o "Mi sono impegnato tantissimo!"{LF}o roba simile.{#Name="Zero"}`
You're not going "Aww!" or "I tried super hard!"{LF}or whatever.{#Name="Zero"}

`Oh, posso farlo!? Ehi, allora, indovina un po'?{LF}Indovina, eh?{#Name="Mikhail"}`
Oh, can I!? Hey, in that case, guess what?{LF}Guess what, huh?{#Name="Mikhail"}

`Zitto, scemo. L'edera ti si conficcherà{LF}ancora più a fondo. Sta' fermo e zitto{LF}finché non riesco a tirarti giù.{#Name="Zero"}`
Shut it, dummy. The ivy's just gonna dig{LF}deeper into you. Hold still and be quiet{LF}until I can cut you down.{#Name="Zero"}

`Non credo che tu stia dicendo la verità.{#Name="Three"}`
I don't think you're telling the truth.{#Name="Three"}

`Mia signora, io—{#Name="Octa"}`
My lady, I—{#Name="Octa"}

`Te ne sei andato perché la tua lussuria aveva bisogno{LF}di una partner più disponibile.{#Name="Three"}`
You left because your lust needed{LF}a more eager partner.{#Name="Three"}

`Il tuo desiderio senza limiti non faceva più{LF}scorrere il mio miele...{#Name="Three"}`
Your limitless desire no longer{LF}caused my honey to flow...{#Name="Three"}

`E così sei diventato irrequieto.{#Name="Three"}`
And so you grew restless.{#Name="Three"}

`Questo... Be', potrebbe essere stato un motivo, ma...{#Name="Octa"}`
That's... Well, it may have been a reason, but...{#Name="Octa"}

`Allora dimmi, Three...{LF}Che cosa fa scorrere il vecchio miele, di questi tempi?{#Name="Zero"}`
So tell me, Three...{LF}What does make the old honey flow these days?{#Name="Zero"}

`Hmm?{#Name="Three"}`
Hmm?{#Name="Three"}

`Oh, per favore. Non fare la finta tonta con me.{LF}So che hai dei desideri.{#Name="Zero"}`
Oh, please. Don't play dumb with me.{LF}I know you have desires.{#Name="Zero"}

`Io...{#Name="Three"}`
I...{#Name="Three"}

`Non provo più amore per il mio discepolo.{LF}Ora il mio amore è riservato ai miei soldati.{#Name="Three"}`
I no longer hold any love for my disciple.{LF}My love is reserved for my soldiers now.{#Name="Three"}

`I miei dolci, adorati soldatini.{#Name="Three"}`
My sweet, beloved little soldiers.{#Name="Three"}

`Dovete fuggire subito!{LF}Non possiamo più fermare Lady Three!{#Name="Octa"}`
You must flee at once!{LF}We cannot stop Lady Three now!{#Name="Octa"}

`Pff! Non ho paura di una svitata fallita{LF}che se ne sta chiusa in camera a fare bambole tutto il giorno.{#Name="Zero"}`
Pfft! I'm not scared of some whack-job loser{LF}who hides in her room making dolls all day.{#Name="Zero"}

`Non sono bambole, mia signora.{LF}Sono soldati su cui ha operato!{#Name="Octa"}`
They're not dolls, my lady.{LF}They're soldiers she's operated on!{#Name="Octa"}

`C-Cosa?{#Name="Decadus"}`
Wh-What?{#Name="Decadus"}

`Oh, è terribile. ...LO ADORO!{#Name="Dito"}`
Oh, that's terrible. ...I LOVE IT!{#Name="Dito"}

`Zero! Ehi! Ascolta!{#Name="Mikhail"}`
Zero! Hey! Listen!{#Name="Mikhail"}

`Taci. Sono nel mezzo di una cosa, adesso.{#Name="Zero"}`
Hush. I'm in the middle of something right now.{#Name="Zero"}

`Anch'io! Anch'io sono nel mezzo di una cosa!{LF}Una cosa strana!{#Name="Mikhail"}`
Me too! I'm in the middle of something!{LF}Something weird!{#Name="Mikhail"}

`Che c'è?{#Name="Zero"}`
What is it?{#Name="Zero"}

`Ogni volta che guardo Three, è come se...{LF}come se il corpo mi diventasse tutto caldo...{#Name="Mikhail"}`
Whenever I look at Three, it's like...{LF}like, my body gets all hot...{#Name="Mikhail"}

`Mi viene questo strano dolore al petto e alla pancia,{LF}e comincio a sentirmi tutto formicolante...{#Name="Mikhail"}`
There's this weird pain in my chest and tummy,{LF}and I start to get all tingly...{#Name="Mikhail"}

`Che devo fare, Zero? Sono malato?{#Name="Mikhail"}`
What should I do, Zero? Am I sick?{#Name="Mikhail"}

`Sì, si chiama "amore". Probabilmente terminale.{#Name="Zero"}`
Yeah, it's called "love." Probably terminal.{#Name="Zero"}

`No, Zero! Non è così! È una cosa seria!{LF}È... È più come...{#Name="Mikhail"}`
No, Zero! It's not like that! This is serious!{LF}It's... It's more like...{#Name="Mikhail"}

`ミハ……イル{#Name="ゼロ"}`
ミハ……イル{#Name="ゼロ"}

`ミハイル……{#Name="ゼロ"}`
ミハイル……{#Name="ゼロ"}

`……ミハイル！！{#Name="ゼロ"}`
……ミハイル！！{#Name="ゼロ"}

`Ahia! Zero, questi esplodono!{LF}Ti esplodono addosso!{#Name="Mikhail"}`
Ow! Zero, these guys explode!{LF}They explode all over you!{#Name="Mikhail"}

`Fa' attenzione! Non che ti si attacchino addosso!{#Name="Zero"}`
Pay attention! Don't let 'em latch on to you!{#Name="Zero"}

`Se il dolore è così forte, scrollateli di dosso!{#Name="Zero"}`
If the pain's that bad, shake 'em off!{#Name="Zero"}

`Aaah! Gli si sono staccate le teste! Si sono staccate!{#Name="Mikhail"}`
Aaah! Their heads came off! They came off!{#Name="Mikhail"}

`Bene! Ora schiaccia quei bastardi!{#Name="Zero"}`
Good! Now crush those bastards!{#Name="Zero"}

`Quell'attacco sembra consumare parecchia energia.{LF}Aspetta un'apertura e poi colpisci!{#Name="Zero"}`
That attack looks like it uses up a lot of power.{LF}Wait for an opening and then strike!{#Name="Zero"}

`Aspetta un'apertura e poi colpisci!{#Name="Zero"}`
Wait for an opening and then strike!{#Name="Zero"}

`Zerooo! Qui dentro è troppo stretto per volare!{#Name="Mikhail"}`
Zerooo! It's too small for me to fly in here!{#Name="Mikhail"}

`Ughhh! Drago del cazzo...{#Name="Zero"}`
Ughhh! Piece of shit dragon...{#Name="Zero"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`テスト{#Name="テスト"}`
テスト{#Name="テスト"}

`Mikhail`
Mikhail

`Zero`
Zero

`Octa`
Octa

`Three`
Three

`Dito`
Dito

`Decadus`
Decadus

`? ? ?`
? ? ?

`Cent`
Cent

`145`
145

`73`
73

`説明文`
説明文

`目的文`
目的文

`Soldato F`
Soldier F

`Soldato E`
Soldier E

`Soldato A`
Soldier A

`Soldato C`
Soldier C

`Soldato D`
Soldier D

`Soldato G`
Soldier G

`Soldato B`
Soldier B

`ゼロ`
ゼロ

`テスト`
テスト

`Fatti strada in questa foresta inquietante.{#Name="説明文"}`
Make your way through this eerie forest.{#Name="説明文"}

`Sconfiggi i necromanti!{#Name="説明文"}`
Defeat the Necromancers!{#Name="説明文"}

`Sconfiggi l'armata di Armisael!{#Name="説明文"}`
Defeat the army of Armisael!{#Name="説明文"}

`Sconfiggi i nemici che ti bloccano la strada!{#Name="説明文"}`
Defeat the enemies blocking your way!{#Name="説明文"}

