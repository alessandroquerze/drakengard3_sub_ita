`Cambia pagina{#Name=" "}`
Change Page{#Name=" "}

`Cambia modalità{#Name=" "}`
Change Mode{#Name=" "}

`Cambia categoria{#Name=" "}`
Change Category{#Name=" "}

`Cambia pulsanti{#Name=" "}`
Change Buttons{#Name=" "}

`Gioca{#Name=" "}`
Play{#Name=" "}

`Flusso della storia{#Name=" "}`
Story Flow{#Name=" "}

`Lista combo CONFIRMED NOT USED 1126.{#Name=" "}`
Combo List CONFIRMED NOT USED 1126.{#Name=" "}

`キャンプイベントをスキップしますか？{#Name=" "}`
キャンプイベントをスキップしますか？{#Name=" "}

`Tornare al menu principale?{#Name=" "}`
Return to the Main Menu?{#Name=" "}

`このストーリーをプレイするための解放条件をクリアしていません{#Name=" "}`
このストーリーをプレイするための解放条件をクリアしていません{#Name=" "}

`このストーリーを解放するには、武器の収集率を１００％にする必要があります。{LF}現在{04}{#Name=" "}`
このストーリーを解放するには、武器の収集率を１００％にする必要があります。{LF}現在{04}{#Name=" "}

`未読です{#Name=" "}`
未読です{#Name=" "}

`{04}{#Name=" "}`
{04}{#Name=" "}

`La porta è chiusa.{#Name="Zero"}`
The door's shut.{#Name="Zero"}

`Hanno chiuso il cancello? Grazie mille, stronzi.{#Name="Zero"}`
They closed the gate? Thanks a lot, assholes.{#Name="Zero"}

`*Merda. E' chiuso.{#Name="Zero"}`
Shit. It's closed.{#Name="Zero"}

`*Dev'essere stato il nemico a chiuderlo.{#Name="Zero"}`
The enemy must've closed it.{#Name="Zero"}

`*Chi continua a chiuderli? Che senso ha?{#Name="Zero"}`
Who keeps closing these? What's the point?{#Name="Zero"}

`*Devono averlo chiuso.{#Name="Zero"}`
Guess they closed it.{#Name="Zero"}

`Heh. Pensano che QUESTO possa fermarmi?{#Name="Zero"}`
Heh. They think THIS is gonna stop me?{#Name="Zero"}

`Non ho bisogno di tornare indietro.{#Name="Zero"}`
I don't need to go back.{#Name="Zero"}

`Ho chiuso con questo posto.{#Name="Zero"}`
I'm done with this place.{#Name="Zero"}

`Devo continuare ad andare avanti...{#Name="Zero"}`
Gotta keep moving forward...{#Name="Zero"}

`C'è una sola strada da prendere, e non è questa.{#Name="Zero"}`
There's only one way to go, and this ain't it.{#Name="Zero"}

`Non serve andare da questa parte.{#Name="Zero"}`
No need to go this way.{#Name="Zero"}

`x {04}{#Name=" "}`
x {04}{#Name=" "}

`Tutte le armi sono state potenziate al massimo.{LF}Hai sbloccato un nuovo costume bonus per Zero!{LF}Accedici dal menu "Cambia costumi/musica".{#Name=" "}`
All weapons have been fully upgraded.{LF}You've unlocked a new bonus outfit for Zero!{LF}Access it from the "Change Outfits/Music" menu.{#Name=" "}

`Dati di salvataggio{#Name=" "}`
Save Data{#Name=" "}

`Vuoi rimuovere i vecchi dati di salvataggio{LF}per liberare più spazio?{#Name=" "}`
Would you like to remove old save data{LF}to secure more free space?{#Name=" "}

`Continuare la partita senza salvare?{#Name=" "}`
Continue the game without saving?{#Name=" "}

` {04}{#Name=" "}`
 {04}{#Name=" "}

`Chiudi{#Name=" "}`
Close{#Name=" "}

`Sei riuscito a raccogliere tutte le armi.{LF}La Diramazione D è ora sbloccata!{#Name=" "}`
You've managed to collect every weapon.{LF}Branch D is now unlocked!{#Name=" "}

`La porta è chiusa.{#Name="Zero"}`
The door's shut.{#Name="Zero"}

`Hanno chiuso il cancello? Grazie mille, stronzi.{#Name="Zero"}`
They closed the gate? Thanks a lot, assholes.{#Name="Zero"}

`*Merda. E' chiuso.{#Name="Zero"}`
Shit. It's closed.{#Name="Zero"}

`*Dev'essere stato il nemico a chiuderlo.{#Name="Zero"}`
The enemy must've closed it.{#Name="Zero"}

`*Chi continua a chiuderli? Che senso ha?{#Name="Zero"}`
Who keeps closing these? What's the point?{#Name="Zero"}

`*Devono averlo chiuso.{#Name="Zero"}`
Guess they closed it.{#Name="Zero"}

`Heh. Pensano che QUESTO possa fermarmi?{#Name="Zero"}`
Heh. They think THIS is gonna stop me?{#Name="Zero"}

`Non ho bisogno di tornare indietro.{#Name="Zero"}`
I don't need to go back.{#Name="Zero"}

`Ho chiuso con questo posto.{#Name="Zero"}`
I'm done with this place.{#Name="Zero"}

`Devo continuare ad andare avanti...{#Name="Zero"}`
Gotta keep moving forward...{#Name="Zero"}

`C'è una sola strada da prendere, e non è questa.{#Name="Zero"}`
There's only one way to go, and this ain't it.{#Name="Zero"}

`Non serve andare da questa parte.{#Name="Zero"}`
No need to go this way.{#Name="Zero"}

` `
 

`Zero`
Zero

` {#Name="ディト"}`
 {#Name="ディト"}

`ディト`
ディト

`Personaggi`
Characters

`Armi`
Weapons

`Visualizza informazioni dettagliate sul personaggio.`
View detailed character information.

`Visualizza storie delle armi e altre informazioni sulle armi.`
View weapon stories and other weapon information.

`Archivio`
Database

`{04}`
{04}

`? ? ?`
? ? ?

`Spade`
Swords

`Lance`
Spears

`Bracciali da combattimento`
Combat Bracers

`Chakram`
Chakrams

`Armi DLC`
DLC Weapons

`{04}`
{04}

`Seleziona/ruota modello CONFIRMED NOT USED 1126`
Select/Rotate Model CONFIRMED NOT USED 1126

`Visualizza animazione personaggio. CONFIRMED NOT USED 1126`
View character animation. CONFIRMED NOT USED 1126

`Seleziona un'arma per visualizzarne la storia e altri dettagli.`
Select a weapon to view its weapon story and additional details.

`Seleziona un personaggio da visualizzare.`
Select a character to view.

`Seleziona un'animazione da visualizzare. CONFIRMED NOT USED 1126`
Select an animation to view. CONFIRMED NOT USED 1126

`デモ`
デモ

`Una Intoner dotata di abilità di combattimento sovrumane. E' aperta{LF}e disinibita con gli uomini, ma di natura violenta e pigra.{LF}Dal suo occhio destro cresce uno strano fiore.{#Name="Zero"}`
A gifted Intoner with superhuman battle skills. She is open{LF}and uninhibited with men, yet violent and lazy by nature.{LF}She has a strange flower growing out of her right eye.{#Name="Zero"}

`Un drago fieramente orgoglioso, considerato il più potente{LF}del mondo. Estremamente intelligente e forte oltre ogni misura,{LF}è l'unico vero confidente che Zero abbia mai avuto.{#Name="Michael"}`
A fiercely proud dragon, said to be the most powerful in{LF}the world. Vastly intelligent and strong beyond measure,{LF}he is the only true confidant Zero has ever had.{#Name="Michael"}

`La forma post-reincarnazione di Michael che viaggia{LF}con Zero. Idealista e pacifista, pensa che tutti{LF}i conflitti possano essere risolti solo con le parole.{#Name="Mikhail (Stage 1)"}`
The post-reincarnation form of Michael that travels{LF}with Zero. An idealist and a pacifist, he thinks all{LF}conflicts can be solved with words alone.{#Name="Mikhail (Stage 1)"}

`La forma post-reincarnazione di Michael che viaggia{LF}con Zero. Idealista e pacifista, pensa che tutti{LF}i conflitti possano essere risolti solo con le parole.{#Name="Mikhail (Stage 2)"}`
The post-reincarnation form of Michael that travels{LF}with Zero. An idealist and a pacifist, he thinks all{LF}conflicts can be solved with words alone.{#Name="Mikhail (Stage 2)"}

`La forma post-reincarnazione di Michael che viaggia{LF}con Zero. Idealista e pacifista, pensa che tutti{LF}i conflitti possano essere risolti solo con le parole.{#Name="Mikhail (Stage 3)"}`
The post-reincarnation form of Michael that travels{LF}with Zero. An idealist and a pacifist, he thinks all{LF}conflicts can be solved with words alone.{#Name="Mikhail (Stage 3)"}

`Un discepolo armato di lancia con un'indole estremamente sadica{LF}e la capacità di percepire rapidamente le debolezze altrui.{LF}Prova grande piacere dalle urla dei suoi nemici.{#Name="Dito"}`
A spear-wielding disciple with an extreme sadistic streak{LF}and the ability to quickly sense the weaknesses of others.{LF}He derives great pleasure from the screams of his foes.{#Name="Dito"}

`Un discepolo ben piazzato che combatte a pugni. Il più{LF}assennato dei suoi compagni, si comporta sempre da gentiluomo, {LF}ma in realtà è un masochista che gode del dolore senza limiti.{#Name="Decadus"}`
A well-built disciple who fights with his fists. The most{LF}sensible of his colleagues, he always behaves as a gentleman, {LF}but is actually a masochist who enjoys pain to no end.{#Name="Decadus"}

`Un discepolo dall'aspetto anziano che combatte con i chakram.{LF}Vanta un appetito sessuale apparentemente insaziabile,{LF}e si dice porti con sé un'anatomia... notevole.{#Name="Octa"}`
An elderly-looking disciple who fights with chakrams.{LF}He boasts a seemingly insatiable sexual appetite,{LF}and is said to carry some...impressive anatomy.{#Name="Octa"}

`Un discepolo narcisista che combatte con lame gemelle. Nonostante{LF}la sua estrema sicurezza di sé, la "conoscenza" che sceglie{LF}di impartire è spesso falsa e finisce solo per irritare tutti.{#Name="Cent"}`
A narcissistic disciple who fights with twin blades. In spite of{LF}his extreme self-confidence, the "knowledge" that he chooses{LF}to impart is often false and just ends up annoying everyone.{#Name="Cent"}

`La più giovane delle Intoner, e sovrana della Terra dei Mari. Avida{LF}e superficiale, desidera tutto ciò che vede (inclusa Zero), ma{LF}perde interesse appena lo ottiene. Ama i doppi sensi sessuali.{#Name="Five"}`
The youngest Intoner, and ruler of the Land of Seas. Greedy{LF}and shallow, she desires all she sees (including Zero), but{LF}loses interest once she gets it. Fond of sexual innuendo.{#Name="Five"}

`L'unica Intoner vergine, e sovrana della Terra dei Monti.{LF}Finge un'aria di superiorità per nascondere il suo profondo{LF}complesso d'inferiorità e le sue inibizioni.{#Name="Four"}`
The lone virgin Intoner, and ruler of the Land of Mountains.{LF}She fakes a superior air in order to hide her deep-seated{LF}inferiority complex and inhibitions.{#Name="Four"}

`La svogliata Intoner che governa la Terra delle Foreste. Ha{LF}gusti sessuali contorti e una personalità distaccata e bizzarra.{LF}Le piace sperimentare sugli umani per trasformarli in bambole.{#Name="Three"}`
The listless Intoner who rules the Land of Forests. She has{LF}twisted sexual tastes and a detached, bizarre personality.{LF}She enjoys experimenting on humans to turn them into dolls.{#Name="Three"}

`Una Intoner brillante e allegra che governa la Terra delle Sabbie.{LF}E' innamorata di Cent, anche se un incidente sfortunato{LF}le ha spezzato la mente, lasciandola in uno stato infantile.{#Name="Two"}`
A bright and cheerful Intoner who rules the Land of Sands.{LF}She is in love with Cent, though an unfortunate incident{LF}shattered her mind, leaving her in a childlike state.{#Name="Two"}

`La Intoner che governa Cathedral City. E' la più{LF}matura e intelligente delle sorelle, e anche l'unica{LF}che comprende pienamente le vere intenzioni di Zero.{#Name="One"}`
The Intoner who rules the Cathedral City. She's the most{LF}mature and intelligent of the sisters, and also the only{LF}one who fully grasps Zero's true intentions.{#Name="One"}

`Il fratello gemello di One, creato da One stessa. Fu nascosto{LF}come misura di sicurezza per distruggere Zero e le altre Intoner.{LF}Idolatra One sia come sorella sia come madre.{#Name="One's Brother"}`
One's twin brother, created by One herself. He was hidden{LF}away as a failsafe to destroy Zero and the other Intoners.{LF}He idolizes One as both a sister and a mother.{#Name="One's Brother"}

`Una negoziante della Terra dei Mari che porta con sé una grande{LF}valigia. In quanto osservatrice e Recorder,{LF}considera Zero una "singolarità" di massimo livello nel suo lavoro.{#Name="Accord"}`
A shopkeeper from the Land of Seas who carries a large{LF}suitcase around with her. As an observer and Recorder,{LF}she views Zero as a top-level "singularity" in her work.{#Name="Accord"}

`Un angelo evocato da Dito usando il potere della canzone di Five.{#Name="Phanuel"}`
An angel summoned by Dito using the power of Five's song.{#Name="Phanuel"}

`Un angelo evocato da Decadus usando il potere della canzone di Four.{#Name="Armaros"}`
An angel summoned by Decadus using the power of Four's song.{#Name="Armaros"}

`Un angelo evocato da Octa usando il potere della canzone di Three.{#Name="Armisael"}`
An angel summoned by Octa using the power of Three's song.{#Name="Armisael"}

`Un angelo evocato da Cent usando il potere della canzone di Two.{#Name="Egregori"}`
An angel summoned by Cent using the power of Two's song.{#Name="Egregori"}

`Un drago trasformato in angelo da One.{#Name="Gabriel"}`
A dragon that was transformed into an angel by One.{#Name="Gabriel"}

`Un angelo evocato da Two e Cent.{#Name="Raphael"}`
An angel summoned by Two and Cent.{#Name="Raphael"}

`Un angelo evocato da One.{#Name="Abdiel"}`
An angel summoned by One.{#Name="Abdiel"}

`Un angelo draconico evocato da Four dopo che la sua mente si era spezzata.{#Name="Zophiel"}`
A dragon angel summoned by Four after her mind was broken.{#Name="Zophiel"}

`Un angelo evocato da Five dopo che la sua mente si era spezzata.{#Name="Galgaliel"}`
An angel summoned by Five after her mind was broken.{#Name="Galgaliel"}

`Un angelo — e un antico drago — richiamato da Three{LF}dopo che la sua mente si era spezzata.{#Name="Ezrael"}`
An angel—and an ancient dragon—called forth by Three{LF}after her mind was broken.{#Name="Ezrael"}

`Zero`
Zero

`Michael`
Michael

`Mikhail (Stage 1)`
Mikhail (Stage 1)

`Mikhail (Stage 2)`
Mikhail (Stage 2)

`Mikhail (Stage 3)`
Mikhail (Stage 3)

`Dito`
Dito

`Decadus`
Decadus

`Octa`
Octa

`Cent`
Cent

`Five`
Five

`Four`
Four

`Three`
Three

